// SupplierCredForm —— #155 area B: renders a generic credential form from derived auth forms.
...
// auth -> supplier-status message. No hand-written form per supplier.
...
// **creates** the supplier (written into the manifest); values are the credentials stored
// **into** that supplier **after** it's created. Without them, filling in this form has
...
// uncontrolled inputs.
export function SupplierCredForm({ auth, onScheme, values, scopes }: {
  auth: AuthForms;
  ...
  return (
    <div data-testid="supplier-cred-form" className="mt-4 border-t border-(--color-rule)/60 pt-4">
...
function CredNote({ note }: { note: string }) {
  return note === ''
    ? null
    : <p data-testid="supplier-status" className="mono text-[12px] text-(--color-accent)">{note}</p>;
}
...
function SchemeSelectMaybe({
  ...
  const t = useTranslations('adminShell.supplierCred');
...
//
// But this used to render a `supplier-connect-button` **button with no onClick** (F-C-24). The
...
// the **card's** job (only the card has the supplier id); the ingest form never had the ability
...
function NeedsDanceNote() {
  ...
  return (
    <p
      data-testid="supplier-needs-dance"
...
function ScopeField(
  ...
  return (
    <div>
      ...
      <div data-testid={`supplier-field-${field.key}`} className="flex flex-wrap gap-1.5">
        {all.map((s) => (
          <label key={s} className="mono text-[11px] flex items-center gap-1">
            <input
              type="checkbox" defaultChecked data-testid={`supplier-scope-${s}`}
