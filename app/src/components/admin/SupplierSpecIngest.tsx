// SupplierSpecIngest —— #155 area A: spec ingestion. owner pastes / uploads / URL-fetches an
// OpenAPI spec -> backend validates it (same 3.0 parser, normalized) -> shows a supplier
// candidate (seam title) or a human-readable rejection reason.
...
import {
  useSupplierIngest, type AuthForms, type AuthScheme,
} from '@/lib/admin/use-supplier-ingest';
import {
  isAssemblable, type AssembleInput, type AssembleState,
} from '@/lib/admin/use-supplier-upload';
import { FilePicker } from '@/components/admin/atoms/FilePicker';
import { SupplierCredForm } from '@/components/admin/SupplierCredForm';
import { SupplierCard } from '@/components/admin/sections/suppliers/SupplierCard';

// SupplierSpecIngest —— **there is exactly one form for adding a supplier** (F-C-21). There
// used to be a second, differently-shaped `{spec, binding}` textarea under the seam card;
...
//
// onAssemble —— assembly (create the supplier + store credentials). Only clickable once
...
// the "bring your own spec" path). The protocol form under the seam card collapses in
...
export function SupplierSpecIngest({ onAssemble, onCandidate, assemble }: {
...
// supplier-connect-button (the one `ConnectMaybe` renders with no onClick) coexist with the
// card's real, working one — a dead button sitting next to a live one.
function AssembledCard({ id }: { id: string }) {
  return <SupplierCard entry={{ id, seam: '', kind: 'openapi' }} />;
}
...
function IngestForm({ onAssemble, onCandidate, assembleError }: {
  ...
  const hook = useSupplierIngest();
  const bindingRef = useRef('');
  const schemeRef = useRef('');
  const credsRef = useRef<Record<string, string>>({});
  // scopesRef —— the oauth2 scopes checked. Stored into the new supplier alongside the
  ...
  // the supplier gets created but the credential form fails to derive.
...
function ExposeRow({ show, checked, onChange }: {
  ...
  return show ? (
    <label className="mt-4 flex items-start gap-2 cursor-pointer">
      <input
        type="checkbox"
        data-testid="supplier-expose-agent-tools"
        checked={checked}
...
function AssembleFailure({ message }: { message: string }) {
  return message === '' ? null : (
    <p
      data-testid="supplier-assemble-error"
...
function UselessWarning({ show }: { show: boolean }) {
  ...
  return show ? (
    <p
      data-testid="supplier-assemble-useless"
      className="mono text-[12px] text-(--color-accent) mt-3"
    >
  ...
  return show ? (
    <div className="mt-4">
      <button
        ...
        data-testid="supplier-assemble-button"
...
function BaseUrlRow({ onText }: { onText: (t: string) => void }) {
  ...
  return (
    <label className="block mt-2">
      ...
      <input
        type="text"
        data-testid="supplier-spec-base-url"
...
// "maps operations to a seam contract" was this field's **only** explanation — it vanished
...
function BindingTextarea({ onText }: { onText: (t: string) => void }) {
  ...
  return (
    <label className="block mt-2">
      ...
      <textarea
        data-testid="supplier-binding-input"
        onChange={(e) => onText(e.target.value)}
...
function CredFormMaybe({ auth, onScheme, values, scopes }: {
  ...
  return auth === null
    ? null
    : <SupplierCredForm auth={auth} onScheme={onScheme} values={values} scopes={scopes} />;
}
...
function SpecTextarea({ onText, onBlur }: { onText: (t: string) => void; onBlur: () => void }) {
  return (
    <textarea
      data-testid="supplier-spec-input"
      onChange={(e) => onText(e.target.value)}
      onBlur={onBlur}
...
function SubmitButton({ onClick }: { onClick: () => void }) {
  ...
  return (
    <button
      type="button" onClick={onClick}
      data-testid="supplier-spec-submit"
      className="sm-btn sm-btn-solid sm-btn-sm"
    >
      {t('useThisSpec')}
    </button>
  ...
  return (
    <div className="flex gap-2 mt-3 items-end">
      <label className="block flex-1">
        ...
        <input
          type="text"
          data-testid="supplier-spec-url-input"
          value={url}
      ...
      <button
        ...
        data-testid="supplier-spec-fetch-button"
...
function SpecError({ message }: { message: string }) {
  return message === '' ? null : (
    <p
      data-testid="supplier-spec-error"
      className="mono text-[12px] text-(--color-accent) mt-3"
    >
      {message}
    </p>
...
function SpecCandidate({ title }: { title: string }) {
  ...
  return (
    <div
      data-testid="supplier-candidate"
