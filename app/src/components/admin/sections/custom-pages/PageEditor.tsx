// PageEditor — the custom-page editor at its own route /admin/edit/<slug> (owner: "点进去有编辑器，
...
// render on the right. The list page (/admin/custom-pages) is now just the list and links here.
//
...
import { IMPORTABLE_MODULES, STARTER, type ImportableModule } from '@/lib/admin/custom-page-imports';
import {
  loadDraft, stageFiles, shipFilesLive, previewView, usePinnedPreviewSrc,
  useCustomPages, type BuildView, type CustomPageSummary, type DraftFiles,
    ...
    <div data-testid="custom-page-editor">
      ...
      <div className="grid gap-4 lg:grid-cols-2 items-start">
        <div className="min-w-0">
          ...
          <div className="border border-(--color-rule) rounded-b-[3px] overflow-hidden" data-testid="custom-page-source">
            <CodeMirror
              value={files[active] ?? ''}
...
function EditorHeader(
  ...
  return (
    <div className="mb-4">
      ...
      >
        <span data-testid="custom-page-back">{t('backToList')}</span>
      </Link>
      {isNew
...
function FileTab(
  ...
  return (
    <button
      type="button" onClick={() => onSwitch(path)} data-testid={`custom-page-file-${path}`}
...
// PreviewPane — the page's live render, resolved from the shared custom-pages list by slug. Its own
  ...
  return (
    <div className="flex items-center gap-3 mt-3">
      <button
        ...
        data-testid="custom-page-build" className="sm-btn sm-btn-sm disabled:opacity-40"
      >
        {t('buildPreview')}
      </button>
      <button
        type="button" onClick={onPublish} disabled={disabled}
        data-testid="custom-page-publish" className="sm-btn sm-btn-solid sm-btn-sm disabled:opacity-40"
      >
        {t('publish')}
      </button>
...
function WidgetPanel() {
  const t = useTranslations('adminPages.customPages');
  return (
    <details className="mt-4" data-testid="custom-page-imports">
...
function EditorPreview({ page }: { page: CustomPageSummary }) {
  const t = useTranslations('adminPages.customPages');
  const view = previewView(page);
  const src = usePinnedPreviewSrc(view.buildID, view.src);
  return src === '' ? <PreviewEmpty /> : (
    <div className="border border-(--color-rule) rounded-sm overflow-hidden" data-testid="custom-page-staging">
      <div className="flex items-baseline justify-between px-3 py-1.5 border-b border-(--color-rule)/60">
        ...
        <span data-testid="custom-page-staging-state" className="mono text-[9.5px] tracking-[0.14em] uppercase text-(--color-muted)">
          {view.status}
        </span>
      </div>
      <iframe
        key={view.buildID} data-testid="custom-page-staging-frame" src={src}
