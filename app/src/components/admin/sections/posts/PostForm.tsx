...
export function PostForm(props: Props) {
  ...
  return (
    <div className="bg-(--color-paper) border border-(--color-rule) max-w-[720px] w-full max-h-[90vh] overflow-y-auto p-7 flex flex-col gap-4">
      ...
      <PostFormBody values={values} setValues={setValues} props={props} toast={toast} />
      <PostFormFooter
  ...
  return (
    <>
      ...
      <CoverImagePicker value={values.coverAsset}
        onChange={(v) => set('coverAsset', v)} toast={toast} />
      <PostBodyField value={values.bodyMD}
        onChange={(v) => set('bodyMD', v)} assetURLs={props.assetURLs} />
    </>
  );
}
...
function useSubmitHandler(
  ...
  return useCallback(async () => {
    await runSubmit(values, props, toast);
  }, [values, props, toast]);
}
...
async function runSubmit(
  ...
  isValid(values) && await doSubmit(values, props, toast);
}
...
async function doSubmit(
  ...
  const ok = await props.onSubmit(values);
