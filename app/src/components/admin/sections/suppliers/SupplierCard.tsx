// SupplierCard — one supplier card (built-in or uploaded): derived credentials form
// (supplier-field-{key}) + oauth2's redirect-uri (read-only) / scope checklist +
// Connect/Disconnect + status/error. Unification: one card for any kind/auth. Logic lives
// in use-supplier-card; this file only renders + wires up (eslint: no if, complexity <=3).
...
import type { CatalogEntry } from '@/lib/admin/use-supplier-catalog';
import { credFieldLabel } from '@/lib/admin/cred-field-label';

export function SupplierCard({ entry }: { entry: CatalogEntry }) {
  return (
    <li
      data-testid={`supplier-row-${entry.id}`}
      className="crosshair border border-(--color-rule) rounded-sm bg-(--color-surface)/30 p-4"
    >
      <span className="ch-tl" /><span className="ch-br" />
      <SupplierCardBody entry={entry} />
    </li>
  );
}

// SupplierCardBody — a card's body: status + derived credentials form + Connect/Disconnect.
//
// **Built-in and owner-uploaded suppliers share this same body** (F-C-47). The uploaded
// family used to render only "seam / origin / kind / status / delete" — so the product
// let an owner create a supplier **with no way to connect it**, while that section's own
// intro read "upload your own (OpenAPI / protocol) supplier". The ability was never
// missing: `/{id}/credential-form`, `/{id}/credentials`, `/{id}/connect` all exist for any
// id, and the form is already **derived by the backend from the supplier's own declaration**
...
export function SupplierCardBody({ entry }: { entry: CatalogEntry }) {
  const hook = useSupplierCard(entry.id);
  return (
    <>
      ...
      <SupplierOps ops={entry.owner_ops ?? []} onRan={hook.reloadStatus} />
    </>
  );
}
...
//
// It always rendered `seam`, and **an uploaded supplier with no seam contract has
// an empty seam string** — a vendor like GitHub, which doesn't map onto calendar/mail,
...
function CardHead(
  ...
  const t = useTranslations('adminIntegrations.supplierCard');
  return (
    <div className="flex items-center justify-between mb-3">
      {/* The card name needs more weight than the fields inside it. It used to be `text-sm`,
          ...
          seam cards just hadn't caught up yet. */}
      <span
      ...
      <span data-testid="supplier-status" className="mono text-[11px] text-(--color-muted)">
...
function ScopeShortfallNote({ missing }: { missing: readonly string[] }) {
  const t = useTranslations('adminShell.supplierCard');
  return missing.length === 0 ? null : (
    <p
      data-testid="supplier-scope-shortfall"
...
function UnreadableNote({ reason }: { reason: string }) {
  return reason === '' ? null : (
    <p
      data-testid="supplier-unreadable"
      className="mb-3 mono text-[11px] text-(--color-accent) reading-tight"
    >
      {reason}
    </p>
...
// Uncontrolled: the connection uses whichever scheme was set when the supplier was assembled
...
function Fields({ hook }: { hook: SupplierCardHook }) {
  return (
...
// port?)" — those two need the supplier to **declare** its own grouping and field
...
// StoredCredsNote — "this supplier already has credentials saved".
//
// The backend **never returns** a credential's value, only `has_credentials: true`
// (verified in supplier-security: credential-form only returns field names, not even a
...
function StoredCredsNote({ show }: { show: boolean }) {
  ...
  return show ? (
    <p
      data-testid="supplier-creds-stored"
...
function RedirectUri({ id, authType }: { id: string; authType: string }) {
  const uri = useSupplierRedirectURI(id);
  return authType === 'oauth2' ? (
    <input
      data-testid="supplier-redirect-uri"
      readOnly
      value={uri}
...
function Scopes({ hook }: { hook: SupplierCardHook }) {
    ...
    <div className="space-y-1 mb-3">
      {hook.scopes.map((scope) => (
        <label key={scope} className="flex items-center gap-2 mono text-[11px] text-(--color-muted)">
          <input
            type="checkbox"
            data-testid={`supplier-scope-${scope}`}
...
function Actions({ hook }: { hook: SupplierCardHook }) {
  ...
  return (
    <div className="flex gap-2">
      {/* Disabled until the derived form comes back: before that, Connect doesn't know
          ...
          sends an oauth2 supplier down the non-dance path — the owner reads
...
function DisconnectButton({ hook }: { hook: SupplierCardHook }) {
  const t = useTranslations('adminIntegrations.supplierCard');
  return hook.connected ? (
    <button
      type="button" onClick={hook.disconnect}
      data-testid="supplier-disconnect-button"
...
function ErrorLine({ error }: { error: string }) {
  return error === '' ? null : (
    <p data-testid="supplier-error" className="mt-2 mono text-[11px] text-(--color-accent)">
      {error}
    </p>
  );
}
