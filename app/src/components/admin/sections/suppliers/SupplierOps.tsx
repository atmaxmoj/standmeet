// SupplierOps — on one card, the owner operations this supplier **declares itself**
// (backend internal/plugin/adapters/owner_op.go).
//
// Why this is a generic block instead of a hardcoded "send test email" button on the mail
// card: the declaration is already data — smtp's manifest lists suppliers.mail_test_send,
// the catalog carries it along, and this file just renders it as given. Adding an action
// means adding an entry to that supplier's manifest; this file doesn't change at all.
...
import type { OwnerOp, OwnerOpField } from '@/lib/admin/use-supplier-catalog';
import { useSupplierOp, type SupplierOpHook } from '@/lib/admin/use-supplier-op';
...
export function SupplierOps({ ops, onRan }: { ops: readonly OwnerOp[]; onRan: () => void }) {
  return ops.length === 0 ? null : (
    <div className="mt-3 border-t border-(--color-rule)/60 pt-3 space-y-3">
      {ops.map((op) => <SupplierOpBlock key={op.name} op={op} onRan={onRan} />)}
    </div>
  );
}

function SupplierOpBlock({ op, onRan }: { op: OwnerOp; onRan: () => void }) {
  const hook = useSupplierOp(op, onRan);
  return (
    <div data-testid={`supplier-op-${hook.segment}`} className="space-y-2">
...
function OpFields({ fields, hook }: { fields: readonly OwnerOpField[]; hook: SupplierOpHook }) {
  return (
    <div className="space-y-2">
      {fields.map((f) => (
        <input
          key={f.key}
          data-testid={`supplier-op-field-${f.key}`}
...
function RunButton({ hook }: { hook: SupplierOpHook }) {
  const t = useTranslations('adminIntegrations.supplierOp');
  return (
    <button
      type="button" onClick={hook.run} disabled={hook.running}
      data-testid="supplier-op-run"
...
function OpResult({ hook }: { hook: SupplierOpHook }) {
  return hook.outcome === null ? null : (
    <p data-testid="supplier-op-result" className="mono text-[11px] text-(--color-accent)">
      <OpResultText outcome={hook.outcome} />
    </p>
  );
}

function OpResultText({ outcome }: { outcome: NonNullable<SupplierOpHook['outcome']> }) {
  const t = useTranslations('adminIntegrations.supplierOp');
  return <>{resultSentence(outcome, t)}</>;
}

type Translate = ReturnType<typeof useTranslations<'adminIntegrations.supplierOp'>>;

function resultSentence(
  outcome: NonNullable<SupplierOpHook['outcome']>, t: Translate,
): string {
...
// supplier — check your inbox to confirm it arrived". It lived in this generic component,
