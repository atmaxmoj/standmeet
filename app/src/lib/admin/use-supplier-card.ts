// use-supplier-card —— the assembly lifecycle for one supplier card
...
// exchanges the token → redirects back to /admin/suppliers → the card
...
const FormSchema = z.object({
  ...
  // supplier **supports**). The two must be kept separate before a
...
export interface SupplierCardHook {
  authType: string;
  ...
  connected: boolean;
  /** The backend says this supplier already has credentials stored. The value itself never comes back — the card says "present" on that basis, rather than showing empty boxes. */
   ...
   * supplier down the bearer path instead, getting back "The connection
   ...
   * as the readiness signal. What `auth_type` a supplier declares is its
...
// useSupplierStatus —— the "status" group for a card: connected or not /
...
function useSupplierStatus(id: string) {
  const [connected, setConnected] = useState(false);
  // hasCredentials —— the backend has always returned this
  // (`supplier-security` verified it: status only ever returns
  ...
  const loadStatus = useCallback(() => {
    void adminAPI.get(`/suppliers/${id}/status`, StatusSchema)
      .then((s) => {
...
export function useSupplierCard(id: string): SupplierCardHook {
  ...
  // formLoaded —— whether the derived form has been fetched back. See SupplierCardHook.ready.
  const [formLoaded, setFormLoaded] = useState(false);
  const status = useSupplierStatus(id);
  ...
  const loadForm = useCallback(() => {
    ...
    setFormLoaded(false);
    void adminAPI.get(`/suppliers/${id}/credential-form`, FormSchema)
      .then((f) => {
      ...
      .catch(() => setError('Couldn’t load this supplier’s setup form. Reload and retry.'));
  }, [id]);
  ...
  // — Connect must wait for it to land: the supplier's row in the database
  ...
  const saveCreds = useCallback(() => {
    ...
    // create the supplier's row, and connect's UPDATE reports "connected"
    // as soon as it hits a row — so "connected" would get said even when
    // nothing was filled in (this is exactly what supplier-connect-receipt
    ...
    return adminAPI.postVoid(`/suppliers/${id}/credentials`, {
  ...
  const connect = useCallback(() => {
    ...
    // supplier reading a failure sentence that belongs to the non-dance path.
  ...
  const disconnect = useCallback(() => {
    void adminAPI.postVoid(`/suppliers/${id}/disconnect`, {})
