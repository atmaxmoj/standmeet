// use-supplier-catalog —— the built-in supplier catalog (manifests
// assembled in from outside). GET /api/admin/suppliers/catalog → connectable
// built-in cards (google-calendar / smtp / …). Kept separate from
// use-supplier-list (owner-created): built-ins don't go into List (List's
// reuse-by-seam callers would otherwise pick up built-ins by mistake).

import { z } from 'zod';

import { useLatestList } from '@/lib/admin/use-latest-list';

// OwnerOpFieldSchema / OwnerOpSchema —— the owner operations a supplier
// declares in its own manifest; the backend has already derived its
// input_schema into flat fields (see adapters.OwnerOp.Fields). The frontend
...
const CatalogEntrySchema = z.object({
  ...
  seam: z.string(),
  kind: z.string(),
  // title —— the vendor name carried by an uploaded supplier (F-C-56). Built-in entries in the catalog don't have this: their name is just the seam.
...
const CatalogSchema = z.object({ suppliers: z.array(CatalogEntrySchema).nullish() });

export type CatalogEntry = z.infer<typeof CatalogEntrySchema>;

export interface SupplierCatalogHook {
...
export function useSupplierCatalog(): SupplierCatalogHook {
  const { items: entries, loaded, loadError, refresh } = useLatestList<CatalogEntry>(
    '/suppliers/catalog', CatalogSchema,
  );
