// use-supplier-ingest —— #155 area A client logic for spec ingestion (paste/URL
// fetch → backend validation → candidate / human-readable error). The logic
// lives here (not presentation); SupplierSpecIngest.tsx only renders + wires up.
...
export interface SupplierIngestHook {
  error: string;
...
async function runValidate(
  ...
  return adminAPI.post('/suppliers/validate-spec', body, VerdictSchema)
    .then(verdictToState)
    .catch(() => ({
...
export function useSupplierIngest(): SupplierIngestHook {
