// use-supplier-op —— runs an owner operation **a supplier declares for
// itself** (see backend supplier/owner_op.go).
//
...
import type { OwnerOp } from '@/lib/admin/use-supplier-catalog';
...
// declaredOpPrefix (routes/admin/block_conn.go) — the route `/suppliers/ops/<segment>` was already public.
const OP_PREFIX = 'suppliers.';
...
const OpResultSchema = z.object({
  ...
  // accepted by the {kind} supplier — check your inbox to confirm": mail
  // phrasing baked into the generic layer, nonsense for a different seam
...
export interface SupplierOpHook {
  segment: string;
  running: boolean;
...
// design** (see the comment in SupplierOps), and they'd been collapsed
...
export function useSupplierOp(op: OwnerOp, onRan: () => void): SupplierOpHook {
  ...
  const run = useCallback(() => {
    setRunning(true);
    setOutcome(null);
    void adminAPI.post(`/suppliers/ops/${segment}`, { ...values.current }, OpResultSchema)
      .then((r) => setOutcome({
