// use-supplier-upload —— #155 area G upload orchestration: paste a spec
// (+optional binding) to assemble an openapi supplier. A name clash (one
// already exists for the same seam) → confirm overwrite first; confirm →
// delete old, create new (overwrite, not stacked). The seam is extracted
// from the binding text (regex, no YAML parser run on the frontend). Logic lives here, presentation only renders.
//
// **Assembly is three steps, not one** (F-C-21): create the supplier → get
...
import type { SupplierListHook } from '@/lib/admin/use-supplier-list';
...
//   binding → fills a seam slot, the consuming block uses it;
...
const ASSEMBLE_FAILED = 'The supplier could not be assembled. Check the spec and try again.';
...
interface Pending { input: AssembleInput; seam: string }

export interface SupplierUploadHook {
  pending: Pending | null;
  // createdID —— the id of the supplier that was just assembled. Assembly
  ...
  // SupplierCard's responsibility. Without this id the owner would land on
  // a list row that **can't be connected to** (SupplierList's row only has
  // seam/status/delete, no Connect) — exactly the F-C-21 shape of "positive feedback the whole way, no exit at the end".
  createdID: string | null;
  // resetCreated —— clears "the one just assembled". **Reopening "add
  // supplier" must be a clean slate**: without this, the ingest form would
  // keep yielding to last time's card, and the spec input would never
  // reappear (no way to install a second supplier). createdID belongs to one modal session, not to this page.
  resetCreated: () => void;
...
// bindingSeam —— extracts the seam from the binding YAML text (`seam: calendar`).
function bindingSeam(binding: string): string {
  return binding.match(/^\s*seam:\s*["']?([\w.-]+)/m)?.[1] ?? '';
}
...
export function useSupplierUpload(list: SupplierListHook): SupplierUploadHook {
  ...
  const assemble = useCallback(async (input: AssembleInput) => {
    ...
    // use-supplier-card's saveCreds; that's the only battle-tested writer
    // on this path, and a shape mismatch would silently store credentials as empty).
    if (hasValue(input.credentials)) {
      await adminAPI.postVoid(`/suppliers/${id}/credentials`, {
  ...
  const upload = useCallback((input: AssembleInput) => {
    ...
    const seam = bindingSeam(input.binding);
    const exists = seam !== '' && list.suppliers.some((c) => c.seam === seam);
    exists
      ? setPending({ input, seam })
      : void assemble(input).catch(fail);
  }, [assemble, list.suppliers, fail]);

  const confirmOverwrite = useCallback(() => {
    const p = pending;
    const old = p === null ? undefined : list.suppliers.find((c) => c.seam === p.seam);
  ...
  // resetCreated —— clears the slate when "add supplier" is opened again: neither the previous id nor the previous failure should carry into the next round.
