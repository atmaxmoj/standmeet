...
// the route stays off the supplier implementation (go-arch-lint). Empty string when the
...
// firstTelegramToken — the token off the first im supplier that actually carries one.
// Disconnecting a supplier clears its credentials (ClearTokens), so an empty credential
// already means "not usable" — no need to also gate on Connected/Active.
func firstTelegramToken(log *slog.Logger, conns []credentials.Connection) string {
	for i := range conns {
