...
// port for the visitor-side resume-reading block. This block only needs
// "fetch this resume's JSON by access code", it shouldn't depend on the whole
// ApplicationRepo, and conversation / blockload definitely shouldn't know about
...
// real failure: block hides on any error unconditionally (fail-closed), so there's
...
// returns ErrApplicationNotFound, wrapped as an error and raised here (block hides
