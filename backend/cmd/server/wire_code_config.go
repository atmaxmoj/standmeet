// wire_code_config.go —— 各能力在**邀请码**上占的字段,和照声明执行的用量闸。
//
...
func codeFieldSurface(d *runtimeDeps) access.CodeExtras {
...
func capabilityQuotaHooks(d *runtimeDeps, hooks map[string]capload.CapHooks) {
	manifests := builtinManifests()
	for i := range manifests {
		m := &manifests[i]
...
func quotaCounterFor(d *runtimeDeps, m *mcpplugin.Manifest) *capquota.Counter {
	if !m.Quota.Usable() {
		return nil
	}
	store := capabilityStorage(d, m)
