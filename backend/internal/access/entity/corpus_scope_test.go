...
func TestAllowsCorpusScope_CodeNarrowsRole(t *testing.T) {
	...
	require.False(t, entity.AllowsCorpusScope(scope, uriCV),
		"the code took this one back — it must not be readable")
	require.True(t, entity.AllowsCorpusScope(scope, uriStandpoint),
		"the rest of the grant is untouched")
	require.True(t, entity.AllowsCorpusScope(scope, "wiki://math/logic"),
...
func TestAllowsCorpusScope_DenyCannotOpen(t *testing.T) {
	...
	require.False(t, entity.AllowsCorpusScope(scope, uriStandpoint),
...
func TestAllowsCorpusScope_OrderIndependent(t *testing.T) {
	...
	require.Equal(t, entity.AllowsCorpusScope(a, uri), entity.AllowsCorpusScope(b, uri),
		"reordering either list must not change the verdict — set intersection, not first-match")
	require.False(t, entity.AllowsCorpusScope(a, uri))
}
...
func TestAllowsCorpusScope_DenyGlobTakesSubtree(t *testing.T) {
	...
	require.False(t, entity.AllowsCorpusScope(scope, "wiki://private/salary"))
	require.False(t, entity.AllowsCorpusScope(scope, "wiki://private/deep/nested"))
	require.True(t, entity.AllowsCorpusScope(scope, "wiki://public/thing"))
}
...
func TestAllowsCorpusScope_EmptyDenyIsInheritance(t *testing.T) {
	...
	for _, denied := range [][]string{nil, {}} {
		...
		require.True(t, entity.AllowsCorpusScope(scope, uriCV),
...
func TestAllowsCorpusScope_RawStillHardDenied(t *testing.T) {
	...
	require.False(t, entity.AllowsCorpusScope(scope, "raw://anything"),
