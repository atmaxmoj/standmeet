// extras.go — config that **other blocks** self-manage on a subject (a code / a role).
//
...
// hands over the whole raw input and lets the block pick its own fields. The assembly
// root wires up every block that declares this kind of config.
//
// The same seam is used twice (once for codes, once for roles). The subject is a **parameter**,
// not two separate interfaces — this is the other half of the same point blockconfig makes about
// "what it's mounted on is a parameter".
//
// Both read and write are best-effort: that storage belongs to another block. Failing to
...
// SubjectExtras — the fields a block occupies on a subject (a code / a role).
...
// CodeExtras — the fields a block occupies on a code (an alias of SubjectExtras). Named
...
// RoleExtras — the fields a block occupies on a role (same as above, different mount
// point).
type RoleExtras = SubjectExtras

// KeyExtras — the fields a block occupies on an outward-facing API key (same as above,
// a third mount point).
//
...
// noExtras — the "nothing" for when no block has declared config on this kind of subject.
...
// withExtraFields — merge the fields a block declared into a schema's properties.
...
// mergedProperties — the original properties + the ones a block adds.
...
// withExtraValues — merge a block's field values into an already-serialized payload.
