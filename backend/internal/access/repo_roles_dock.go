...
func marshalDockButtons(buttons []DockButtonConfig) ([]byte, error) {
	if len(buttons) == 0 {
		return []byte("[]"), nil
	}
...
func decodeDockButtons(raw []byte) []DockButtonConfig {
	out := []DockButtonConfig{}
	if len(raw) == 0 {
		return out
	}
	if err := json.Unmarshal(raw, &out); err != nil {
		return []DockButtonConfig{}
	}
	return out
}
