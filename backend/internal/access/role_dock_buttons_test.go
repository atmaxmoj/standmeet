...
//   - access.RoleSnapshot 冻下 dock 按钮配置（session 起那刻拍死，owner 后改不影响在跑 session）
...
func TestValidateDockButtons_AcceptsUpToTwo(t *testing.T) {
	t.Parallel()
	err := access.ValidateDockButtons([]access.DockButtonConfig{
...
func TestValidateDockButtons_EmptyIsFine(t *testing.T) {
	t.Parallel()
	if err := access.ValidateDockButtons(nil); err != nil {
...
func TestRoleSnapshot_FreezesDockButtons(t *testing.T) {
	...
	if snap.DockButtons()[0].Trigger != "Summarize this" {
		t.Fatal("access.RoleSnapshot must defensively clone dock buttons")
	}
}
