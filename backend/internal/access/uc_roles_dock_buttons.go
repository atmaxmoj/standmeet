...
func checkWaypointsSubset(in *RoleWriteInput) error {
	if err := ValidateWaypoints(in.Waypoints); err != nil {
...
func checkDockButtonsSubset(in *RoleWriteInput) error {
	if err := ValidateDockButtons(in.DockButtons); err != nil {
		return fmt.Errorf("dock buttons: %w", err)
	}
	capErr := ValidateDockButtonCapabilities(in.DockButtons, in.ValidCapabilityIDs)
	if capErr != nil {
