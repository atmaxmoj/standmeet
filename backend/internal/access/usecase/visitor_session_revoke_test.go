...
func TestRevokeFindsLongLivedActiveSession(t *testing.T) {
	...
	t.Cleanup(mr.Close)
	store := usecase.NewVisitorSessionStore(redis.NewClient(&redis.Options{Addr: mr.Addr()}))
	ctx := context.Background()

	issued, err := store.Issue(ctx, &usecase.VisitorSessionData{
	...
	_, gerr := store.Get(ctx, issued.Token)
	require.ErrorIs(t, gerr, usecase.ErrVisitorSessionNotFound, "revoked session must be gone")
}
