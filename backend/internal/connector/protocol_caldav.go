...
// Apple/Nextcloud…）实现 calendar 品类契约（usecases.CalendarProxy）。跟 openapi 适配器、SMTP
...
// caldavConnector —— 实现 Connector 基面 + Verifier + usecases.CalendarProxy。
...
func (c *caldavConnector) FreeBusy(
	...
	if rerr != nil {
		return nil, domain.ErrCalendarUnavailable
	}
	if serr := caldavStatusErr(r.Status); serr != nil {
		return nil, serr
	}
	rows := parseFreeBusy(r.Body)
	out := make([]usecases.BusyInterval, 0, len(rows))
	for i := range rows {
		out = append(out, usecases.BusyInterval{Start: rows[i].Start, End: rows[i].End})
	}
	return out, nil
}
...
func caldavStatusErr(code int) error {
	...
	case caldavTransient(code):
		return domain.ErrCalendarUnavailable
	default:
		return domain.ErrCalendarBadRequest
	}
}
