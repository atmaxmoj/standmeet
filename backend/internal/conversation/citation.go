...
// uuid (持久层落 cited_wiki_ids / cited_output_ids / cited_writing_ids 列
// 时用)；Title 给 UI 渲。
//
// 单一 VO 涵盖 wiki + output + writing (writing 是 public/已发布 blog,一律
// 进 cited,无 show_as_source gate)；之前 backend 拿 `cited_wiki_ids[]` +
// `cited_output_ids[]` 分列存,frontend 也分数组维护 —— 现在抽出来。
//
// Genre 用 corpus.DocumentGenre (跟 corpus entry 复用同一类型)；引入独
...
type Citation struct {
	Genre Genre  // wiki / output / writing / subjectivity (raw 不引用)
