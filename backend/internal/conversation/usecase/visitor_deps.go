...
// (session lifecycle + block wiring raw materials). Split out of visitor.go to stay
...
// derivation / code intro). Carries no tool/block dependencies (those go through
// each block's narrow deps + VisitorSkillsDeps). Half of the split-out god-struct.
type VisitorSessionDeps struct {
	...
	// AgentSkills —— computes block states / tool specs (retrieval / booker /
	// ext-mcp / owner-skill) at session assembly time.
	AgentSkills *registry.Registry
	// CodeDenials —— the code layer of the ACL hierarchy (block-acl-hierarchy.md).
	...
	// RoleBlockConfig —— reads "each block's config on this role" when freezing the
	// role snapshot. Nil = no block has ever declared per-role config (a perfectly
	// normal instance).
	RoleBlockConfig RoleBlockConfigReader
...
// RoleBlockConfigReader —— reads each block's own config, keyed by role: **block
// id** → its config (a JSON object).
//
// This layer **never interprets any key inside it**, and shouldn't even know which
// blocks exist —— so the seam has exactly one method, returning opaque JSON. The
// implementation lives in the composition root (only it knows blockconfig and manifest
// declarations).
//
// The method is named ReadByBlock, not Read: the same implementation also has a
...
// symptom that every block reads a config that isn't its own.
type RoleBlockConfigReader interface {
	ReadByBlock(ctx context.Context, roleID string) map[string]json.RawMessage
}
...
// CodeDenialReader —— reads a code's deny set (block / skill id). Pure deny: a code
// can only cut further from what the chosen role already grants. access.CodeDenialRepo
// implements it.
type CodeDenialReader interface {
	ListBlocks(ctx context.Context, codeID string) ([]string, error)
...
// blocks (the block-wiring half). RegisterVisitorSkills builds each
// block's narrow deps from this. Both prod wireup + the eval facade construct it.
...
type VisitorSkillsDeps struct {
	...
	// DepConnected —— the named-seam dependency connectivity query (used by the
	// ext-mcp dep-grant gate: when a tool declares _meta.requires, it's allowed through
	// based on grant+connected). prod wires the seam DepRegistry; the eval facade
	// leaves it nil → ext-mcp dependent tools are always fail-closed hidden.
	DepConnected DepConnected
	// AgentToolSuppliers —— the source that exposes an openapi supplier's raw ops as
	// agent tools (§3). prod wires the composition root's adapter (credentials repo +
	// supplier table); nil → no agent tools exposed at all.
	AgentToolSuppliers AgentToolSupplierSource
	// Resumes —— the source for the visitor-side resume-reading block: fetches
	// this resume's JSON by the application's code. prod wires the composition root's
	// adapter (port.ResumesByCode); nil → this block stays hidden permanently.
