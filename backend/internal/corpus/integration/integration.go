...
//     Kind constant + a new struct implementing the Integration interface, without
...
func (i *Integrations) Find(kind Kind) (Integration, bool) {
...
func (i *Integrations) Has(kind Kind) bool {
	_, ok := i.Find(kind)
	return ok
}
...
// Kinds — returns every attached Kind (deduplicated, in appearance order). Used for
// admin UI rendering.
func (i *Integrations) Kinds() []Kind {
	seen := make(map[Kind]struct{}, len(i.items))
	out := make([]Kind, 0, len(i.items))
	for _, x := range i.items {
		k := x.Kind()
		if _, dup := seen[k]; dup {
			continue
