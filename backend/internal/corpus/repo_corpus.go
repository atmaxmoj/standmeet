...
func (r *RawRepo) Create(ctx context.Context, in *CreateRawInput) (Raw, error) {
	...
	if err != nil {
		return Raw{}, fmt.Errorf("create raw: %w", err)
	}
...
func (r *RawRepo) GetByID(ctx context.Context, ownerID, id string) (Raw, error) {
	ownerUUID, err := pgstore.ParseUUID(ownerID)
	if err != nil {
		return Raw{}, fmt.Errorf(pgstore.ErrParseOwnerIDPrefix, err)
	}
