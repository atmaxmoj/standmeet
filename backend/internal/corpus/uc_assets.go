...
func insertAssetRow(ctx context.Context, a *insertAssetArgs) (Asset, error) {
	asset, cerr := a.Deps.Repo.CreateTx(ctx, a.Tx, &CreateAssetInput{
	...
	if cerr != nil {
		return Asset{}, fmt.Errorf("create asset row: %w", cerr)
	}
	return asset, nil
}
