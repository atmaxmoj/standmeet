...
// MapEntry —— one visible node the map is built from: its derived path + title. The
...
func BuildCorpusMap(entries []MapEntry, under string, budget int) []MapNode {
...
func buildPathTree(entries []MapEntry) *mapTreeNode {
	root := newMapNode("")
