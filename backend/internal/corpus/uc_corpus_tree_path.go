...
func WikiTreePaths(ws []Wiki) map[string]string {
...
func OutputTreePaths(os []Output) map[string]string {
...
func RawTreePaths(rs []Raw) map[string]string {
...
func WikiMetaTreePaths(metas []WikiMeta) map[string]string {
	return treePathsFor(wikiMetaNodes(metas))
}

// OutputMetaTreePaths —— WikiMetaTreePaths 的 output 孪生。
func OutputMetaTreePaths(metas []OutputMeta) map[string]string {
...
func wikiMetaNodes(metas []WikiMeta) []pathNode {
	nodes := make([]pathNode, len(metas))
	for i := range metas {
		nodes[i] = metaNode(metas[i].ID, metas[i].Title, metas[i].ParentID)
	}
