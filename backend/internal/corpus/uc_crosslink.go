...
type ResolvedLink struct {
	Dst *Writing // nil = unresolved（render 留 literal）
	Ref CrossLinkRef
}
...
func ResolveCrossLinks(
	refs []CrossLinkRef, candidates []Writing,
) []ResolvedLink {
...
type writingIndex struct {
	bySlug, byTitle map[string]*Writing
}

func indexCandidates(candidates []Writing) writingIndex {
	idx := writingIndex{
		bySlug:  make(map[string]*Writing, len(candidates)),
		byTitle: make(map[string]*Writing, len(candidates)),
	}
...
func resolveAndDedupForOwner(body string, candidates []Writing) []string {
