...
func resolveNoteNodeID(
	ctx context.Context, repo *NoteRepo, ownerID, path string,
) (string, error) {
	var parentID *string
	id := ""
...
func findNoteChildBySegment(
	ctx context.Context, repo *NoteRepo, ownerID string, parentID *string, seg string,
) (string, error) {
...
func segInPageNote(kids []NoteMeta, seg string) (string, bool) {
	for i := range kids {
