...
func writeNote(
	ctx context.Context, repo *NoteRepo, in *WriteSubjectivityInput,
) (Note, error) {
	if in.ID == "" {
...
func createNote(
	...
	note, err := repo.Create(ctx, &CreateNoteInput{
	...
	if err != nil {
		return Note{}, fmt.Errorf("create subjectivity: %w", err)
	}
	return note, nil
}
...
func updateNote(
	...
	note, err := repo.UpdateBody(ctx, &UpdateNoteInput{
		OwnerID: in.OwnerID, ID: in.ID, ParentID: in.ParentID,
		Title: in.Title, Body: in.Body, Tags: in.Tags,
		CSSClasses: in.CSSClasses, ShowAsSource: in.ShowAsSource,
	})
	if err != nil {
		return Note{}, fmt.Errorf("update subjectivity: %w", err)
	}
	return note, nil
}
...
func validateNoteParent(
	ctx context.Context, repo *NoteRepo, ownerID string, parentID *string,
) error {
...
func validateNoteReparent(
	ctx context.Context, repo *NoteRepo, ownerID, nodeID string, parentID *string,
) error {
...
func checkNoteCycle(
	ctx context.Context, repo *NoteRepo, ownerID, nodeID, parentID string,
) error {
	cur := parentID
...
func deriveNotePath(
	ctx context.Context, repo *NoteRepo, ownerID, id string,
) (string, error) {
