...
func refreshCrossLinks(
	ctx context.Context, deps WritingsTxDeps, tx pgx.Tx, writing *Writing,
) error {
...
func buildShellCreateInput(in *SaveWritingInput) *CreateWritingInput {
	// path 不再存列:writing 折进 corpus_notes 后由 slug 派生 "writings/"+slug。
	return &CreateWritingInput{
...
func loadExistingWriting(
	...
	if err != nil {
		return Writing{}, fmt.Errorf("load existing writing: %w", err)
	}
	return p, nil
}
...
func writeWritingBody(ctx context.Context, a *writeBodyArgs) (Writing, error) {
	body := rewriteRefs(a.In.BodyMD, a.Rewrite)
	cover := rewriteCoverRef(a.In.CoverImageRef, a.Rewrite)
	p, err := a.Deps.Writings.UpdateTx(ctx, a.Tx, &UpdateWritingInput{
	...
	if err != nil {
		return Writing{}, fmt.Errorf("update writing body: %w", err)
	}
	return p, nil
}
