...
func (l *pgCorpusLister) visibleWikiNodes(
	...
	out := make([]MapEntry, 0, len(wikis))
	for i := range wikis {
		p := paths[wikis[i].ID()]
		if p == "" || !allowsCorpusEntry(scope, "wiki", p, wikis[i].Published()) {
			continue
		}
		out = append(out, MapEntry{Path: p, Title: wikis[i].Title()})
	}
	return out, nil
}
...
func ResolveByName(nodes []MapEntry, name string) []Meta {
	...
	for i := range nodes {
		if lastPathSegment(nodes[i].Path) == want || SlugifyTitle(nodes[i].Title) == want {
			out = append(out, Meta{Path: nodes[i].Path, Title: nodes[i].Title, Genre: "wiki"})
		}
	}
	return out
}
