...
//   - RoleSnapshot 冻下 dock 按钮配置（session 起那刻拍死，owner 后改不影响在跑 session）
...
func TestValidateDockButtons_AcceptsUpToTwo(t *testing.T) {
	t.Parallel()
	err := domain.ValidateDockButtons([]domain.DockButtonConfig{
...
func TestValidateDockButtons_EmptyIsFine(t *testing.T) {
	t.Parallel()
	if err := domain.ValidateDockButtons(nil); err != nil {
...
func TestRoleSnapshot_FreezesDockButtons(t *testing.T) {
	...
	if snap.DockButtons()[0].Trigger != "Summarize this" {
		t.Fatal("RoleSnapshot must defensively clone dock buttons")
	}
}
