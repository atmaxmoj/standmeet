// egress.go — the supplier's outbound SSRF guard. The owner self-hosts and can upload any
...
// Allow — an allow-list by hostname (bypasses the internal-network block). e2e injects
// external-mock; prod leaves it empty.
type Allow map[string]bool

// NewAllow — build an allow-list from a comma-separated hostname list.
func NewAllow(hosts []string) Allow {
	a := make(Allow, len(hosts))
	for _, h := range hosts {
...
func (a Allow) CheckEgressURL(rawURL string) error {
...
// jumps internal". Every supplier (built-in + uploaded) shares this, so it's uniform.
func (a Allow) GuardedHTTPClient() *http.Client {
	...
	// supplier layer manages its own retries with idempotency keys; a retrying transport
	// would double-send.
...
func (a Allow) safeDialAddr(ctx context.Context, addr string) (string, error) {
