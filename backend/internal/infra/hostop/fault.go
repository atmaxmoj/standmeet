...
// gets back `{"error":"<one sentence>"}`. **One sentence is not a class** — the sandbox
// side can only treat every failure as the same kind, so "owner hasn't configured mail" and
// "the mail supplier can't be reached right now" come out as the same sentence on the
// visitor's screen, and one of those sentences is false (F-C-42).
//
// The host already distinguishes them (the supplier layer has `ErrNoSupplier` →
// `ErrMailNotConfigured`); the place the class gets lost is **the boundary**. So the fix
...
// The socket layer is a strict leaf (`hostsocket: mayDependOn: []` in `.go-arch-lint.yml`) —
