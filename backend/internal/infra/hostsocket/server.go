// Package hostsocket —— host-side NARROW unix-socket APIs that sandboxed blocks
...
// only the op(s) that block needs — never general DB or LLM access. The
// builtin's MCP tool lives in its sandboxed plugin; this is just the compute/data
// plumbing the plugin calls, the same way it would call the database. So "the kernel
// names no concrete block" holds: the block is the plugin, this is a host service it
// talks to.
//
...
func ListenWith(
	...
	if err != nil {
		return nil, fmt.Errorf("hostsocket: listen %q: %w", path, err)
	}
...
func clearStale(path string) error {
	if rmErr := os.Remove(path); rmErr != nil && !os.IsNotExist(rmErr) {
		return fmt.Errorf("hostsocket: clear %q: %w", path, rmErr)
	}
	return nil
}
...
func chmodOrClose(ln net.Listener, path string, log *slog.Logger) error {
	if cherr := os.Chmod(path, socketMode); cherr != nil {
		if cerr := ln.Close(); cerr != nil {
			log.Warn("hostsocket: close after chmod fail", "err", cerr)
		}
		return fmt.Errorf("hostsocket: chmod %q: %w", path, cherr)
	}
	return nil
}
...
func (s *Server) Close() error {
	if rmErr := os.Remove(s.path); rmErr != nil && !os.IsNotExist(rmErr) {
		s.log.Warn("hostsocket: remove socket", "path", s.path, "err", rmErr)
	}
...
func (s *Server) handleConn(ctx context.Context, conn net.Conn) {
	defer func() {
		if cerr := conn.Close(); cerr != nil {
			s.log.Debug("hostsocket: conn close", "err", cerr)
		}
	}()
...
func (s *Server) dispatch(ctx context.Context, raw []byte) json.RawMessage {
	...
	if err != nil {
		...
		s.log.Error("hostsocket: op failed", "op", env.Op, "err", err)
...
// not by type**: this layer is a strict leaf (`hostsocket: mayDependOn: []`), so it
