...
// occasionally exceeds 8s, and a tighter timeout misdiagnoses the block as
...
// drag visitor chat down with it. Sized for fast supplier calls.
const callTimeout = 15 * time.Second

// LongCallTimeout —— budget for a tool that itself performs a full LLM round-trip (summarize's
// report generation): the generic 15s callTimeout is sized for quick supplier calls and times
