...
// (the vehicle an externalized block uses to carry a system-prompt fragment).
...
// externalized block uses to declare ReturnDirectly). clarify carries
...
// content. The real ui:// card comes from the externalized block server itself;
