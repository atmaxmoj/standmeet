// binding.go — supplier binding: maps a seam contract method (list_busy/create_event/
...
// the seam is known, and every required contract method is mapped.
...
// SeamContractOps — the method names each seam's contract "must map in full". Assembly
// time uses this to judge whether a binding is missing a mapping. calendar must be able to
// query busy times + create an event (booking); cancel_event is optional (a supplier may not
// support cancellation, and mapping it anyway is tolerated).
var SeamContractOps = map[string][]string{
...
// the supplier was externalized (#155) that switch had nowhere to live and was silently
...
// Binding — a complete binding: declares the seam + the mapping for each contract method.
type Binding struct {
...
// ValidateAgainst — checks the binding is self-consistent with the spec: the seam is known,
// every op exists in the spec, and every required contract method is mapped.
func (b *Binding) ValidateAgainst(spec *Spec) error {
	required, known := SeamContractOps[b.Seam]
	if !known {
		return fmt.Errorf("%w: %q", ErrBindingUnknownSeam, b.Seam)
	}
...
func (b *Binding) checkComplete(required []string) error {
	for _, req := range required {
		if _, ok := b.Operations[req]; !ok {
			return fmt.Errorf("%w: %q missing %q", ErrBindingIncomplete, b.Seam, req)
		}
	}
	return nil
}
