// runtime.go — the generic openapi supplier runtime (one runtime, handles every openapi
// supplier).
//
...
// Runtime — one spec + one binding + an outbound client = one callable supplier
// implementation. Auth does **not** live here: one supplier serves multiple owners and
...
// supplier call surface (op, argsJSON) → JSON, never a typed struct; same resolve→build→send.
...
func decodeInvokeArgs(op string, argsJSON json.RawMessage) (any, error) {
	...
	if err := json.Unmarshal(argsJSON, &input); err != nil {
		return nil, fmt.Errorf("supplier invoke %q args: %w", op, err)
	}
	return input, nil
}
...
func marshalInvokeOut(op string, out any) (json.RawMessage, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("supplier invoke %q marshal: %w", op, err)
	}
	return res, nil
}
...
func (r *Runtime) send(req *http.Request, ob *opBinding) (any, error) {
	...
	if derr != nil {
		return nil, fmt.Errorf("supplier http call: %w", derr)
	}
