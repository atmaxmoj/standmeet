// market_needs.go —— how the marketplace card's "still needs which seam" line gets
// computed (F-F-4).
//
// The truth is derived, in a three-stage chain: the skill declares which **tools** it uses
// (SKILL.md's `allowed-tools`) → the **blocks** that provide those tools need which
// seams (the manifest's `requires`) → has this owner connected them. None of the three
...
// SeamNeeds —— port: given a skill that declares it uses these tools, which seams
...
// actually did), the client took the seam list and computed the set difference itself,
...
type SeamNeeds interface {
	// DepsForTools —— the seam names behind these tools. Pure in-memory.
	DepsForTools(tools []string) []string
	// Unconnected —— of these seams, the ones this owner hasn't connected yet.
	Unconnected(ctx context.Context, ownerID string, deps []string) ([]string, error)
}

// fillNeeds —— fill in "which seams are still missing" for the results on this page
// whose **body has been read**.
//
// Asked in one shot: the page's seam names are unioned first, "which ones aren't
...
func fillNeeds(
	ctx context.Context, port SeamNeeds, ownerID string, page []entity.MarketSkill,
) {
	if port == nil || ownerID == "" {
		return
	}
	...
	if err != nil {
		slog.Default().Warn("marketplace: cannot resolve seam needs", "err", err)
		return
	}
...
// seam needs. Skips results whose body wasn't read — their answer is "unknown", and
// nil is that answer.
func assignNeeds(port SeamNeeds, missing []string, page []entity.MarketSkill) {
	for i := range page {
...
// unconnectedForPage —— of the seams this whole page needs, the ones this owner
...
// pageDeps —— the full set of seams this page (the results whose body was read)
// needs, deduplicated.
func pageDeps(port SeamNeeds, page []entity.MarketSkill) []string {
