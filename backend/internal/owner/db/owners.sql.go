...
const getSoleOwnerMonitoringEnabled = `-- name: GetSoleOwnerMonitoringEnabled :one
...
func (q *Queries) GetSoleOwnerMonitoringEnabled(ctx context.Context) (bool, error) {
	row := q.db.QueryRow(ctx, getSoleOwnerMonitoringEnabled)
	var monitoring_enabled bool
	err := row.Scan(&monitoring_enabled)
	return monitoring_enabled, err
}
...
const setOwnerMonitoringEnabled = `-- name: SetOwnerMonitoringEnabled :exec
UPDATE owners SET monitoring_enabled = $2 WHERE id = $1
`

type SetOwnerMonitoringEnabledParams struct {
	ID                pgtype.UUID
	MonitoringEnabled bool
}

// Flip the owner's traffic-collection master switch (monitor.md §8).
func (q *Queries) SetOwnerMonitoringEnabled(ctx context.Context, arg SetOwnerMonitoringEnabledParams) error {
	_, err := q.db.Exec(ctx, setOwnerMonitoringEnabled, arg.ID, arg.MonitoringEnabled)
	return err
}
