...
func parseWorkableConfig(raw []byte) (workableConfig, error) {
	...
	if cfg.Company == "" {
		return cfg, fmt.Errorf("workable missing company: %w", jobsmodel.ErrJobSourceConfigInvalid)
	}
	if cfg.APIToken == "" {
...
func workableToDomain(j *workableJob, accountName string) jobsmodel.FetchedJob {
