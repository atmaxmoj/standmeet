...
- `import { createClient, type CorpusCard, type CustomPageLink } from '@standmeet/sdk-core'`
  ...
  - `sm.fetchCorpusCards()` → published corpus cards; `sm.fetchWikiLanding(path)` → one note's body;
    `sm.fetchCustomPages()` → the owner's other published pages.
...
`custom_page.write_file` (path `App.tsx`) → `custom_page.build` → poll `custom_page.get_build` →
`custom_page.promote_to_staging` (owner-only preview) → `custom_page.promote_to_live`.
