// page_store_writable.go — the custom_pages.store_writable flag (model C): whether a page accepts
// visitor writes. Kept out of custom_pages.go to hold that file under max-lines.
...
func (r *CustomPageRepo) SetStoreWritable(
	...
	if err := db.New(r.pool).SetPageStoreWritable(ctx, db.SetPageStoreWritableParams{
