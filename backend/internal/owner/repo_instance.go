...
func (r *InstanceRepo) ClaimAndCreateOwner(
	...
	if err != nil {
		return Owner{}, fmt.Errorf("begin tx: %w", err)
	}
	ownerRow, txErr := claimTx(ctx, tx, tokenHash, input)
	if txErr != nil {
		if rerr := tx.Rollback(ctx); rerr != nil {
			return Owner{}, errors.Join(txErr, fmt.Errorf("rollback: %w", rerr))
		}
		return Owner{}, txErr
	}
	if cerr := tx.Commit(ctx); cerr != nil {
		return Owner{}, fmt.Errorf("commit claim: %w", cerr)
	}
	return ownerRow, nil
}
...
func claimTx(
	...
	if _, err := q.TryClaimInstance(ctx, &tokenHash); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return Owner{}, ErrInvalidSetupToken
		}
	...
	if err != nil {
		return Owner{}, translateCreateOwnerErr(err)
	}
