...
func (r *Repo) UpdateHandle(
	...
	if terr != nil {
		return Owner{}, fmt.Errorf("begin tx: %w", terr)
	}
...
func commitOrRollback(
	ctx context.Context, tx pgx.Tx, ownerRow *Owner, txErr error, commitTag string,
) (Owner, error) {
	if txErr != nil {
	...
	if cerr := tx.Commit(ctx); cerr != nil {
		return Owner{}, fmt.Errorf("%s: %w", commitTag, cerr)
	}
	return *ownerRow, nil
}
