...
func ResolveLiveBuild(
	...
	if herr != nil {
		return CustomPageBuild{}, fmt.Errorf("first owner handle: %w", herr)
	}
	if handle == "" {
		return CustomPageBuild{}, ErrOwnerNotFound
	}
	soleOwner, oerr := owners.GetByHandle(ctx, handle)
	if oerr != nil {
		if errors.Is(oerr, ErrOwnerNotFound) {
			return CustomPageBuild{}, ErrOwnerNotFound
		}
		return CustomPageBuild{}, fmt.Errorf("get sole owner: %w", oerr)
	}
...
func resolveByOwner(
	...
	if perr != nil {
		return CustomPageBuild{}, fmt.Errorf("get page: %w", perr)
	}
	if page.LiveBuildID == nil {
		return CustomPageBuild{}, ErrCustomPageNotFound
	}
	build, berr := deps.Builds.GetByID(ctx, *page.LiveBuildID)
	if berr != nil {
		return CustomPageBuild{}, fmt.Errorf("get build: %w", berr)
	}
	return build, nil
}
