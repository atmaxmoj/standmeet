// page_store.go — a custom page's own persistence namespace, on the isolation model the owner asked
...
//   - Dropped with the page: DeletePage runs DROP SCHEMA CASCADE (DropPageStore); no orphaned data.
...
//     public route.
//
// The owner domain depends only on the PageDocStore interface; the composition root backs it with
...
const (
	// PageStoreMaxDocs — per-page document cap (quota / leak guard).
	PageStoreMaxDocs = 500
	// PageStoreMaxDocBytes — per-document size cap.
	PageStoreMaxDocBytes = 8 * 1024
	// PageStoreMaxCollectionLen — collection name length cap.
	PageStoreMaxCollectionLen = 64
)

// ErrPageStoreInvalid — the write's collection or document violates a shape/size cap.
var ErrPageStoreInvalid = errors.New("invalid page store write")
...
// PageDocStore — a per-page document namespace (each page its own schema). Backed by capstore at
...
type PageDocStore interface {
	...
	AllRecords(ctx context.Context, pageID string) ([]entity.PageDocument, error)
...
func PublicInsertDoc(
	ctx context.Context, deps CustomPageDeps, owners SoleOwnerLookup, w DocWrite,
) (string, error) {
...
func PublicQueryDocs(
	ctx context.Context, deps CustomPageDeps, owners SoleOwnerLookup, q DocQuery,
) ([]json.RawMessage, error) {
	soleOwner, err := resolveSoleOwner(ctx, owners)
	if err != nil {
...
func VisitorInsert(
	ctx context.Context, deps CustomPageDeps, ownerID string, w DocWrite,
) (string, error) {
...
func insertablePage(
	...
	if !validWrite(w) {
		return entity.CustomPage{}, ErrPageStoreInvalid
	}
	...
	if !page.StoreWritable {
		return entity.CustomPage{}, entity.ErrPageStoreNotWritable
	}
	if cerr := ensureCapacity(ctx, deps, page.ID); cerr != nil {
		return entity.CustomPage{}, cerr
	}
	return page, nil
}

// ensureCapacity — the page's schema exists (provision is idempotent) and it is under the quota.
func ensureCapacity(ctx context.Context, deps CustomPageDeps, pageID string) error {
...
func checkQuota(ctx context.Context, deps CustomPageDeps, pageID string) error {
	n, err := deps.Docs.CountAll(ctx, pageID)
	if err != nil {
		return fmt.Errorf("count page store: %w", err)
	}
	if n >= PageStoreMaxDocs {
		return entity.ErrPageStoreQuota
	}
	return nil
}
...
func VisitorQuery(
	ctx context.Context, deps CustomPageDeps, ownerID string, q DocQuery,
) ([]json.RawMessage, error) {
	if !validCollection(q.Collection) {
		return []json.RawMessage{}, ErrPageStoreInvalid
	}
...
) ([]entity.PageDocument, error) {
	page, err := lookupPage(ctx, deps, ownerID, slug)
	if err != nil {
		return []entity.PageDocument{}, err
	}
	docs, lerr := deps.Docs.AllRecords(ctx, page.ID)
	if lerr != nil {
		return []entity.PageDocument{}, fmt.Errorf("list page docs: %w", lerr)
	}
	return docs, nil
}

// OwnerDeleteDoc — the owner removes one document by id from the management view.
func OwnerDeleteDoc(ctx context.Context, deps CustomPageDeps, ownerID string, ref DocRef) error {
...
func OwnerClear(ctx context.Context, deps CustomPageDeps, ownerID, slug string) error {
	page, err := lookupPage(ctx, deps, ownerID, slug)
	if err != nil {
		return err
	}
...
func OwnerSetWritable(
	ctx context.Context, deps CustomPageDeps, ownerID, slug string, writable bool,
) error {
...
// ProvisionPageStore — lifecycle hook: CreatePage provisions the page's schema.
func ProvisionPageStore(ctx context.Context, deps CustomPageDeps, pageID string) error {
	if err := deps.Docs.Provision(ctx, pageID); err != nil {
		return fmt.Errorf("provision page store: %w", err)
	}
	return nil
...
// DropPageStore — lifecycle hook: DeletePage drops the page's schema (DROP SCHEMA CASCADE = the
// page's data goes with it).
func DropPageStore(ctx context.Context, deps CustomPageDeps, pageID string) error {
...
func validCollection(collection string) bool {
	return collection != "" && len(collection) <= PageStoreMaxCollectionLen
}

func validDoc(doc json.RawMessage) bool {
	return len(doc) > 0 && len(doc) <= PageStoreMaxDocBytes && json.Valid(doc)
}
