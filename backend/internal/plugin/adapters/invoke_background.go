// invoke_background.go — background (fire-and-forget) supplier calls + retry on transient
// errors.
//
...
//
// Why this must live in the host rather than the block spawning its own goroutine: a
// sandboxed block's process lifetime is **only this one turn** — it may be reclaimed the
...
//
// The retry base is only allowed for use by the supplier layer per the architecture, so the
...
// this).
// The logger is injected once alongside the dispatcher, not passed on every call: that would
// blow up the parameter list, and the thin routes shell isn't allowed to import this package
// per the architecture, so it can't share an input struct.
func (s *Dispatcher) SetLogger(log *slog.Logger) { s.log = log }
...
func (s *Dispatcher) InvokeBackground(
	...
	go func() {
		...
		err := retry.Do(detached, notifyPolicy(), func() error {
			_, ierr := s.Invoke(detached, ownerID, seam, verb, args)
			return ierr
		})
		if err != nil && s.log != nil {
			// Nobody is waiting on a background failure — if it doesn't get logged loudly,
			// it's as if it never happened.
			s.log.Error("supplier background invoke failed",
				"seam", seam, "verb", verb, "owner", ownerID, "err", err)
		}
	}()
}
