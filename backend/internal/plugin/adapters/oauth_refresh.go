// oauth_refresh.go — silent token refresh for oauth2 suppliers. Before injecting an outbound
...
// oauthRefresher — does silent refresh for one oauth2 supplier. Built at assembly time
...
func (r *oauthRefresher) maybeRefresh(
	ctx context.Context, blockID, ownerID string, conn *credentials.Connection,
) error {
	if !tokenExpired(conn) || conn.RefreshToken == "" {
		return nil
	}
	return r.doRefresh(ctx, blockID, ownerID, conn)
}
...
func (r *oauthRefresher) doRefresh(
	...
	if rerr != nil {
		return r.handleRefreshErr(ctx, blockID, ownerID, rerr)
	}
	refresh := pickRefreshToken(conn.RefreshToken, tok.RefreshToken)
	if serr := r.store.SaveTokens(ctx, blockID, ownerID, &TokenRefresh{
...
func (r *oauthRefresher) handleRefreshErr(
	ctx context.Context, blockID, ownerID string, rerr error,
) error {
	if errors.Is(rerr, ErrInvalidGrant) {
		if derr := r.store.MarkDisconnected(ctx, blockID, ownerID); derr != nil {
...
// refresh).
func tokenExpired(conn *credentials.Connection) bool {
	return conn.TokenExpiresAt != nil && nowUTC().After(*conn.TokenExpiresAt)
}

// clientCred — the {client_id, client_secret} an oauth2 supplier stores.
