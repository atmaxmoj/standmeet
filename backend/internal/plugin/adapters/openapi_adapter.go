// openapi_adapter.go — seam contract adapter: wires the generic openapi execution core
// (openapi.Runtime) into contracts consumers use (CalendarProxy / MailProxy). Booker
// only knows CalendarProxy, unaware whether behind it is Google / Outlook / any SaaS with a
// spec+binding attached. One supplier serves multiple owners: runtime (spec+binding) is shared;
// auth + connection state resolves per (supplier, owner) via AuthManager (credentials/OAuth
// tokens stay inside, never leaving this layer). openapiCore holds shared pieces
...
// Name — Supplier base surface: the supplier's name.
...
// Connected — Supplier base surface: is this owner connected (reads connection state).
...
// ExposesAgentTools — whether this supplier exposes raw operations as agent tools (§3).
...
func (c *openapiCore) AgentOps() []AgentOp {
...
func (c *openapiCore) CallAgentOp(
	...
	if cerr != nil {
		return nil, fmt.Errorf("supplier %q agent call: %w", c.id, cerr)
	}
	return raw, nil
}
...
func (c *openapiCore) injector(ctx context.Context, ownerID string) (openapi.AuthInjector, error) {
	conn, err := c.store.Get(ctx, c.id, ownerID)
	if err != nil {
		return nil, fmt.Errorf("supplier %q load: %w", c.id, err)
	}
	if c.refresher != nil {
		if rerr := c.refresher.maybeRefresh(ctx, c.id, ownerID, &conn); rerr != nil {
			return nil, fmt.Errorf("supplier %q refresh: %w", c.id, rerr)
		}
	}
	inj, ierr := c.auth(&conn)
	if ierr != nil {
		return nil, fmt.Errorf("supplier %q auth: %w", c.id, ierr)
	}
	return inj, nil
}
...
// calendarAdapter — openapiCore implementing CalendarProxy.
...
func (a calendarAdapter) FreeBusy(
	...
	var out busyResult
	// A conversion, not a field-by-field literal: the wire shape and the seam's DTO are the
	...
	for i := range out.Busy {
		intervals = append(intervals, BusyInterval(out.Busy[i]))
	}
...
func mapCalendarSentinel(err error) error {
	...
	// a clean sentinel: it does not echo the internal URL back
	if errors.Is(err, egress.ErrBlockedEgress) {
		return ErrCalendarBlockedEgress
	}
	if errors.Is(err, openapi.ErrMissingRequired) {
		return fmt.Errorf("%w: %w", ErrCalendarBadRequest, err)
	}
	return nil
}
...
// mailAdapter — openapiCore implementing MailProxy.
...
func (a mailAdapter) Send(
	...
	return MailReceipt{ProviderID: out.ID}, nil
}
...
func classifyMailSendErr(err error) error {
	...
	if errors.As(err, &se) && !se.Transient {
		return fmt.Errorf("%w: %w", ErrMailRejected, err)
	}
	return fmt.Errorf("%w: %w", ErrMailUnavailable, err)
}
