...
// service boundary). Pins down the normalized "last mile": a supplier assembled from one
// spec+binding **actually** implements CalendarProxy, and maps execution-core errors
...
// Exercises the openapi adapter against the in-package seam types.
...
func (f fakeStore) Get(
	_ context.Context, blockID, _ string,
) (credentials.Connection, error) {
	return credentials.Connection{
		BlockID: blockID, Connected: f.connected,
		Credentials: []byte(`{"token":"x"}`),
	}, nil
}

func (fakeStore) SaveTokens(
	_ context.Context, _, _ string, _ *TokenRefresh,
) error {
	return nil
}
...
func assembleCal(t *testing.T, h http.Handler) (CalendarProxy, *httptest.Server) {
	t.Helper()
	srv := httptest.NewServer(h)
	m := &Manifest{
		ID: "google-calendar", Kind: "openapi", Seam: "calendar",
		Spec:    []byte(strings.ReplaceAll(calSpecTmpl, "%SERVER%", srv.URL)),
		Binding: []byte(calBindingYAML),
	}
	loopback := egress.NewAllow([]string{"127.0.0.1"}) // httptest server runs on loopback
	c, err := AssembleOpenAPI(m, http.DefaultClient, fakeStore{connected: true}, loopback)
	if err != nil {
		t.Fatalf("AssembleOpenAPI: %v", err)
	}
	cal, ok := c.(CalendarProxy)
	if !ok {
		t.Fatalf("assembled calendar supplier is not a CalendarProxy: %T", c)
	}
	return cal, srv
}
...
func TestAssembleOpenAPI_CalendarContract_FreeBusy(t *testing.T) {
	...
	busy, err := cal.FreeBusy(context.Background(), "owner-1", FreeBusyReq{
...
func TestAssembleOpenAPI_Calendar_TransientDegrades(t *testing.T) {
	...
	_, err := cal.FreeBusy(context.Background(), "owner-1", FreeBusyReq{})
