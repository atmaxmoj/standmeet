// owner_op.go — an owner-side operation a supplier **declares for itself**.
//
// This came out of suppliers.mail_test_send: sending a test email is **the mail supplier's**
// business, not the "supplier registry"'s. It used to live on the generic registry, forcing
// the registry to know about mail — a seam name leaking into the generic layer.
//
// This splits it apart into the same meta-structure as the block axis: the **declaration
// is data** (name / description / input schema / which seam-contract operation it wants),
// written into the supplier's own manifest; the **implementation** is wired up by the host per
// the seam contract. Adding a supplier-specific owner operation = adding a block to that
// supplier's manifest, without touching a single line of the generic layer.
...
// OwnerOp — one owner-side operation a supplier declares.
//
// Name is the externally-facing operation id (e.g. "suppliers.mail_test_send"); Op is the
// **seam-contract operation** it wants the host to execute (e.g. "mail.test_send"). Kept
