// seam_errors.go —— calendar-supplier sentinel errors (#190). These are SUPPLIER-side outcomes
// (owner hasn't connected / oauth revoked / upstream 5xx / bad request / SSRF-blocked egress), not
// booking-domain concepts — so they live with the seam contract, consumed by the supplier
// adapters + the block plugins that call the calendar proxy. Moved out of the kernel domain.
...
var ErrCalendarNotConnected = errors.New("calendar supplier not connected")
...
// ErrCalendarBlockedEgress —— the supplier's outbound target resolves inside the private
...
var ErrCalendarBlockedEgress = errors.New(
	"calendar supplier blocked: target resolves to an internal/private address",
)
