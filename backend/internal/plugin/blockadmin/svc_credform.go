// svc_credform.go — boundary types for the credential form and uploaded content. CredentialForm
// directly aliases the credform layer's type of the same name: the shape is already 1:1
...
// alias lets routes import only this package, while sparing the field copy. Un-alias it if the
// shapes ever genuinely diverge.

package blockadmin

// CredentialForm — the credential form a supplier asks the owner to fill in (auth type +
// field keys + oauth2 scopes + scheme list).

// UploadedSpec — the content of an uploaded/edited supplier (spec + JSONata binding + the
