...
//      owner actually typed into (`use-supplier-card.ts`'s `setField` only records changed
...
// SaveCredentials — save credentials. seam/kind are fixed by the built-in manifest;
...
func (s *Service) SaveCredentials(ctx context.Context, ownerID, id string, body []byte) error {
	...
	if err := s.d.Repo.SaveCredentials(ctx, &credentials.SaveCredentialsInput{
		OwnerID: ownerID, BlockID: id,
		Seam: m.Seam, Kind: m.Kind, Credentials: merged,
		ResetConnected: changed,
	}); err != nil {
		return fmt.Errorf("save block credentials: %w", err)
	}
	return nil
}

// storedCredentials — the plaintext credentials this block currently has stored; none /
...
//
// Uses RawMessage instead of decoding into a concrete type because every supplier's fields
