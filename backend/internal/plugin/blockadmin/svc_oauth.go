// svc_oauth.go — the OAuth dance internals: reading client_id, building redirect_uri,
...
// oauthCred — oauth2 supplier credentials (the owner's OAuth app) + the scope subset the owner
...
func (s *Service) loadOAuthCred(ctx context.Context, ownerID, id string) (oauthCred, error) {
	...
	if err != nil {
		return oauthCred{}, fmt.Errorf("load supplier: %w", err)
	}
	var cred oauthCred
...
func (s *Service) redirectURI(ctx context.Context, ownerID, id string) (string, error) {
	...
	return base + "/api/admin/suppliers/" + id + "/callback", nil
}
...
// pendingDance — what one dance has stored in Redis: who, which supplier, this round's PKCE
...
func (s *Service) persistState(ctx context.Context, state string, d *pendingDance) error {
	val := d.OwnerID + "|" + d.BlockID + "|" + d.Verifier
...
// danceFromState — decodes the state value "ownerID|blockID|verifier" back; block
...
func danceFromState(val, id string) pendingDance {
	...
	blockID, verifier, _ := strings.Cut(rest, "|")
	if blockID != id {
		return pendingDance{}
	}
	return pendingDance{OwnerID: ownerID, BlockID: blockID, Verifier: verifier}
}

func stateKey(state string) string { return "supplier:oauth:" + state }
...
func (s *Service) initDance(
	ctx context.Context, ownerID, id string, ep adapters.OAuthEndpoints,
) (ConnectResult, error) {
	...
	url := ep.BuildAuthorizeURL(&adapters.AuthorizeInput{
...
func (s *Service) openDance(ctx context.Context, ownerID, id string) (openedDance, error) {
	...
	if perr := s.persistState(ctx, state, &pendingDance{
		OwnerID: ownerID, BlockID: id, Verifier: verifier,
	}); perr != nil {
...
func (s *Service) danceContext(ctx context.Context, ownerID, id string) (danceCtx, error) {
	...
	ep, eerr := adapters.OAuthEndpointsFor(m, m.AuthScheme)
	if eerr != nil {
