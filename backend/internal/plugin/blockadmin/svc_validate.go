...
// blockadmin without importing the adapters package directly).
...
func (s *Service) ValidateSpec(
	...
	v := integration.ValidateIngestSpec(normalized)
...
func readSpecResponse(resp *http.Response) ([]byte, error) {
	raw, rerr := io.ReadAll(io.LimitReader(resp.Body, int64(integration.MaxSpecBytes)+1))
