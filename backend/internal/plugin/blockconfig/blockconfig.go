// Package blockconfig — a block's configurable fields: **generic**
// storage and retrieval where the host doesn't know the meaning of any field.
//
// A block declares which config fields it has in its own manifest
// (plugin.ConfigField: key, type, default), the owner fills them in on the
// panel, and the values are stored in **this block's own isolated
// storage** (a fixed collection in blockstore). Reads fall back to the
// declared default.
//
// The host side never contains a single business word — no "working_hours",
// no "booking". It only knows "this block declared a set of keys, and the
// owner overrode some of them."
//
// Why this package exists: before it, a block that wanted to be
...
// Store — a config read/write handle **bound to one block**.
//
// The collection name is fixed rather than each block picking its own:
// the host needs to be able to read/write it generically (see scope.go).
//
// (kind, blockID) is fixed at construction time — the same shape as blockstore's
// BoundStore: the handle a caller gets can only operate on this one
// block's config, and can't be filled in with someone else's id.
type Store struct {
...
// New — binds the underlying blockstore to one block's namespace.
func New(store *blockstore.Store, kind blockstore.Kind, blockID string) *Store {
	return &Store{store: store, kind: kind, blockID: blockID}
}
...
// Get — all of a block's current config fields under one owner
// (declaration ∪ owner overrides).
func (s *Store) Get(
	ctx context.Context, ownerID string, decl []plugin.ConfigField,
) ([]Field, error) {
	return s.GetScoped(ctx, OwnerScope(ownerID), decl)
}

// GetScoped — all of a block's current config fields at one attachment
// point (declaration ∪ overrides).
//
// The **declaration** is authoritative: a key not in the declaration is never
// returned even if a value was stored for it — after a block removes a
// config field, the old value shouldn't keep surfacing on the panel.
func (s *Store) GetScoped(
	ctx context.Context, scope Scope, decl []plugin.ConfigField,
) ([]Field, error) {
...
// Set — overrides an owner's config for a block.
func (s *Store) Set(
	ctx context.Context, ownerID string,
	decl []plugin.ConfigField, values map[string]json.RawMessage,
) error {
...
func (s *Store) SetScoped(
	...
	if _, err := s.store.Insert(ctx, s.kind, s.blockID, scope.collection, doc); err != nil {
		return fmt.Errorf("blockconfig insert: %w", err)
	}
	return nil
}
...
func writable(
	scope Scope, decl []plugin.ConfigField, values map[string]json.RawMessage,
) error {
	if !scope.ok() {
		return ErrNoScope
	}
...
func check(decl []plugin.ConfigField, values map[string]json.RawMessage) error {
...
func (s *Store) stored(
	...
	recs, err := s.store.Query(ctx, s.kind, s.blockID, scope.collection, filter)
	if err != nil {
		return nil, fmt.Errorf("blockconfig query: %w", err)
	}
...
func decodeStored(
	...
	if err := json.Unmarshal(recs[0], &doc); err != nil {
		return nil, fmt.Errorf("blockconfig decode: %w", err)
	}
	delete(doc, scopeKey)
	return doc, nil
}
...
func (s *Store) clear(ctx context.Context, scope Scope) error {
	...
	if _, err := s.store.Delete(ctx, s.kind, s.blockID, scope.collection, filter); err != nil {
		return fmt.Errorf("blockconfig clear: %w", err)
	}
	return nil
}
...
func scopeFilter(scope Scope) (json.RawMessage, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("blockconfig filter: %w", err)
	}
	return filter, nil
}
...
func buildDoc(scope Scope, values map[string]json.RawMessage) ([]byte, error) {
	...
	if ierr != nil {
		return nil, fmt.Errorf("blockconfig scope id: %w", ierr)
	}
	doc[scope.key] = id
	out, merr := json.Marshal(doc)
	if merr != nil {
		return nil, fmt.Errorf("blockconfig encode: %w", merr)
	}
	return out, nil
}
