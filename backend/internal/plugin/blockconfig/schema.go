...
func jsonTypeOf(t string) string {
	switch t {
	case plugin.ConfigTypeInt:
		return "integer"
	case plugin.ConfigTypeBool:
		return "boolean"
	case plugin.ConfigTypeStringList:
		return "array"
...
func schemaOf(f *plugin.ConfigField) json.RawMessage {
	parts := []string{
		`"type":["` + jsonTypeOf(f.Type) + `","null"]`,
		`"description":` + quote(f.Description),
	}
	if f.Type == plugin.ConfigTypeStringList {
