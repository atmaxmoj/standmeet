...
// block wanted a per-**code** setting (booker's "how many times this code
...
// block's owner config and code config never overwrite each other.
...
const (
	...
	roleCollection  = "blockconfig_role"
	keyCollection   = "blockconfig_key"
)

// OwnerScope — an owner's config for this block (what's filled in on the
// panel).
func OwnerScope(ownerID string) Scope {
	return Scope{key: "owner_id", collection: ownerCollection, id: ownerID}
}

// CodeScope — the fields this block occupies on one invitation code
...
// RoleScope — the fields this block occupies on one role (filled in
...
// with the session (see RoleSnapshot's block_config). blockconfig itself is
...
// KeyScope — the fields this block occupies on one **external API key**
...
// this surface is the key, so the limit attaches to the key.
...
// count usage (the block's own storage records the same id), so "who the
// limit is attached to" and "who usage is counted against" use the **same
