...
// block hand-writing its own — booker used to hand-write it: the host's
...
// them).
func validate(decl []plugin.ConfigField, values map[string]json.RawMessage) error {
...
type fieldValidator func(*plugin.ConfigField, string, json.RawMessage) error

var validators = map[string]fieldValidator{
	plugin.ConfigTypeInt: validateInt,
	plugin.ConfigTypeTime: func(_ *plugin.ConfigField, k string, r json.RawMessage) error {
...
func validateOne(f *plugin.ConfigField, key string, raw json.RawMessage) error {
...
func checkBounds(f *plugin.ConfigField, key string, n int) error {
