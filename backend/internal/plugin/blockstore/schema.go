// Package blockstore —— per-block isolated document storage, sitting on the **shared**
// Postgres (no separate database). Each block gets its own schema: mcp_<id>, created on
// install, DROPped on uninstall. Core data lives in `public`; block schemas always carry
// a **reserved prefix**, structurally separated from core.
//
...
// Kind —— which kind of thing the storage belongs to. It determines the prefix, a
// structural marker for "this is not a core schema".
type Kind string

const (
	// KindSupplier —— a supplier block's private storage, schema = supplier_<id>.
	KindSupplier Kind = "supplier"
	// KindMCP —— an MCP block's private storage, schema = mcp_<id>.
	KindMCP Kind = "mcp"
	// KindMicrosite —— a microsite's own persistence namespace, schema = microsite_<id>. Same
	// isolation model as a plugin: its own schema (not a shared table by id), dropped with it.
	KindMicrosite Kind = "microsite"
)

// kindPrefix —— kind → reserved prefix. A core schema never carries a prefix, so "has a
// prefix" ⟺ "is block storage, not core". The DROP guard uses this to keep core schemas out.
var kindPrefix = map[Kind]string{
	KindSupplier:  "supplier_",
	KindMCP:       "mcp_",
	KindMicrosite: "microsite_",
}
...
func schemaName(kind Kind, id string) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("blockstore: unknown kind %q", kind)
	}
...
func assertDroppable(name string) error {
	if coreSchemas[name] {
		return fmt.Errorf("blockstore: refuse to drop core schema %q", name)
	}
	if !droppableRe.MatchString(name) {
		return fmt.Errorf("blockstore: refuse non-block schema %q (no reserved prefix)", name)
	}
	return nil
}
