...
func TestSchemaName_Derivation(t *testing.T) {
	...
	cases := []struct {
		...
		{KindSupplier, "google-calendar", "supplier_google_calendar"}, // '-' sanitized
		{KindSupplier, "smtp", "supplier_smtp"},
...
func TestSchemaName_RejectsBadInput(t *testing.T) {
	...
	bad := []struct {
		...
		{KindSupplier, ""},    // empty id
		{KindSupplier, "---"}, // empty after sanitizing
...
func TestAssertDroppable_RefusesCoreAndUnprefixed(t *testing.T) {
	...
	refuse := []string{
		...
		"codes",              // no prefix
		"supplier",           // just the prefix word, no '_suffix'
		"mcp",                // same
...
func TestAssertDroppable_AllowsBlockSchemas(t *testing.T) {
	t.Parallel()
	ok := []string{"supplier_google_calendar", "mcp_calendar_book", "supplier_smtp"}
	for _, name := range ok {
		if err := assertDroppable(name); err != nil {
			t.Fatalf("assertDroppable(%q) must allow block schema, got %v", name, err)
		}
	}
}

// TestNoInputProducesCoreDrop —— a fallback: iterate a batch of malicious ids to prove
// schemaName either errors or produces a block name that passes the guard — it never
...
func TestNoInputProducesCoreDrop(t *testing.T) {
	...
	for _, id := range evil {
		assertNeverCoreDrop(t, KindSupplier, id)
