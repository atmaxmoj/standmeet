...
// stored is an opaque JSONB document — blockstore **knows nothing** about business concepts
...
func (s *Store) Provision(ctx context.Context, kind Kind, id string) error {
	...
	if _, eerr := s.pool.Exec(ctx, ddl); eerr != nil {
		return fmt.Errorf("blockstore provision %q: %w", schema, eerr)
	}
	return nil
}
...
func (s *Store) Query(
	...
	if qerr != nil {
		return nil, fmt.Errorf("blockstore query %q/%s: %w", schema, collection, qerr)
	}
	defer rows.Close()
...
func (s *Store) Count(
	...
	if cerr := s.pool.QueryRow(ctx, sql, collection, containment(filter)).Scan(&n); cerr != nil {
		return 0, fmt.Errorf("blockstore count %q/%s: %w", schema, collection, cerr)
	}
	return n, nil
}
...
func (s *Store) Delete(
	...
	if len(filter) == 0 {
		return 0, fmt.Errorf("blockstore delete %s/%s: empty filter refused", kind, collection)
	}
	schema, err := schemaName(kind, id)
	if err != nil {
		return 0, err
...
func (s *Store) AllRecords(ctx context.Context, kind Kind, id string) ([]CollectedRecord, error) {
	...
	if qerr != nil {
		return nil, fmt.Errorf("blockstore all-records %q: %w", schema, qerr)
	}
	defer rows.Close()
...
func scanCollected(rows pgx.Rows) ([]CollectedRecord, error) {
	...
	if rerr := rows.Err(); rerr != nil {
		return nil, fmt.Errorf("blockstore all-records rows: %w", rerr)
	}
	return out, nil
}
...
func scanDocs(rows pgx.Rows) ([]json.RawMessage, error) {
	...
	for rows.Next() {
		...
		if serr := rows.Scan(&doc); serr != nil {
			return nil, fmt.Errorf("blockstore scan doc: %w", serr)
		}
		out = append(out, doc)
	}
	if rerr := rows.Err(); rerr != nil {
		return nil, fmt.Errorf("blockstore rows: %w", rerr)
	}
	return out, nil
}
...
func (s *Store) QueryWithIDs(
	...
	rows, qerr := s.pool.Query(ctx, sql, collection, containment(filter))
	if qerr != nil {
		return nil, fmt.Errorf("blockstore query-with-ids %q/%s: %w", schema, collection, qerr)
	}
	defer rows.Close()
...
func scanRecords(rows pgx.Rows) ([]Record, error) {
	...
	for rows.Next() {
		...
		if serr := rows.Scan(&r.ID, &r.Doc); serr != nil {
			return nil, fmt.Errorf("blockstore scan record: %w", serr)
		}
		out = append(out, r)
	}
	if rerr := rows.Err(); rerr != nil {
