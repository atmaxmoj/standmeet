// allowance.go —— what this block tells the model when its allowance
// runs out.
//
// F-B-14 (actually happened in prod): once quota is spent, the host **hides this block
...
// spent it returns 429 `quota_exhausted` instead of "you never had this block"
...
//
// The host doesn't know what "booking" is: this sentence is phrased using the block's
// own Title, so it holds for any other block too.
...
// SessionNote —— registry.SessionNoter: the sentence this block must say in this session,
// no matter what.
//
...
func (c *mcpAppFiber) SessionNote(
...
// block's instructions. Empty = not spent, send the instructions as usual.
func (c *mcpAppFiber) spentAllowanceNote(
	ctx context.Context, in *registry.AssembleInput,
) string {
...
// quotaSpent —— whether the gate hid this block **because the allowance ran out**. No
...
func (c *mcpAppFiber) quotaSpent(ctx context.Context, in *registry.AssembleInput) bool {
	if c.gate == nil {
		return false
	}
	ok, err := c.gate(ctx, in)
	return !ok && errors.Is(err, registry.ErrQuotaExhausted)
}

// allowanceLabel —— what to call this block in the sentence. Title is the human-facing
// name; if there is none, fall back to id rather than leaving a hole ("used up their
// allowance for  on this access code").
func (c *mcpAppFiber) allowanceLabel() string {
	if c.m.Title != "" {
		return c.m.Title
	}
	return c.m.ID
