// owner_tools.go —— an externalized (sandboxed) block may also serve **owner-side**
// tools.
//
// Why this exists: a fully-externalized block owns its logic end-to-end. Before this, a
// sandboxed plugin could only face visitors, so any owner-facing surface of the SAME block had
...
// With owner tools declared on the manifest, the block keeps one implementation and the host
// keeps none.
//
...
func (c *mcpAppFiber) OwnerMCPBindings() []*registry.MCPBinding {
	out := make([]*registry.MCPBinding, 0, len(c.m.OwnerTools))
...
// owner's AI client rather than an empty success (the visitor path can hide a broken block to
...
func (c *mcpAppFiber) ownerToolHandler(t *plugin.OwnerTool) registry.MCPHandler {
	...
	return func(ctx context.Context, ownerID string, raw json.RawMessage) registry.MCPResult {
		...
		if out.IsError {
			return registry.MCPError(out.Text)
		}
		return registry.MCPSuccess(out.Text)
	}
}
...
func (c *mcpAppFiber) ownerToolErr(cause error, msg string) registry.MCPResult {
	if c.dialErrLog != nil {
