...
// ProgressLabel —— what the **visitor**-facing line shows while a block is
// running.
//
// This lives here rather than in the loader because it's the block's own
// property: "what I'm doing" should be answered by whatever declares the
// block, not decided by whichever code happened to load it in. (The
...
// ([[move-the-block-move-its-edges]]).
//
// So the fix isn't "add yet another progress_label field for the next block
// to fill in" — that field would just get forgotten the same way. Falling back to
// Title, which is already mandatory, gets every block a human-readable phrase
// for free.
