// binding_tool.go — the "one LLM tool" a Fiber exposes to the visitor agent loop.
//
...
var (
	errEmptySchema  = errors.New("registry: empty schema")
	errSchemaNoType = errors.New("registry: schema missing type")
)

// BindingTool — one LLM tool exposed by one Fiber.
//
...
// RunFn — the tool execution closure a block writes. args is the JSON
...
// NewTool — the standard constructor a block uses to register an LLM tool.
...
func paramsFromRaw(raw json.RawMessage) (*schema.ParamsOneOf, error) {
	...
	if err := json.Unmarshal(raw, &js); err != nil {
		return nil, fmt.Errorf("registry: schema decode: %w", err)
	}
	if js.Type == "" {
...
// uses reflection to auto-bind a typed arg function; our blocks take
...
type FlattenResult struct {
	...
	Tools          []tool.BaseTool
	// ClaimGates — among the blocks assembled this session, the ones that
