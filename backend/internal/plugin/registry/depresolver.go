// depresolver.go — the host's registry of named-dependency providers (supplier rework).
//
// Design (docs/design/supplier-deps-tests.md): a plugin manifest declares `Requires:[...]`,
// naming in-app dependencies ("calendar" / "smtp" …), each backed by a supplier. The host
// registers providers into this table; `enabledFibers` resolves each block's Requires: any one
// not-connected excludes that block from enabledFibers (one global choke point, D-2) — hidden
// from every session alike. Credentials never enter this layer: a provider only answers
// "connected or not" (Connected) and hands back a "call handle" (never a token).
...
// RequiresDeps — optional interface: a block declares which named in-app dependencies it
// needs (seam names, e.g. "calendar" / "smtp"). enabledFibers gates on this: any one
// not-connected excludes the block (global choke point, D-2). pluginFiber pulls it from
// manifest.Requires; a block not implementing this is never seam-gated.
type RequiresDeps interface {
	Requires() []string
}

// ProvidesVisitorTools — optional interface: which tool **names** this block provides on
...
// and the product must answer "that needs the calendar supplier" (F-F-4). Pairs with
// RequiresDeps: one says "which tools I provide", the other "which seams I need" — only
// together do you get "which supplier does this tool need". A block not implementing
...
// DepsForTools — behind these tool names, which named dependencies (seam names) are
// needed, altogether. Pure in-memory, touches neither network nor DB.
//
// Only counts blocks that **have declared their own tool names**; a miss only means this
// table doesn't recognize that tool — not that the tool needs no supplier.
...
func appendDepsIfProvides(
	out []string, seen map[string]struct{}, c Fiber, want map[string]struct{},
) []string {
	return appendNewDeps(out, seen, depsOfProvider(c, want))
}

// depsOfProvider — c's own tools include one in want → its needed seams; else empty.
func depsOfProvider(c Fiber, want map[string]struct{}) []string {
...
// depsConnected — whether all of this block's Requires dependencies are connected (half of the
...
func (r *Registry) depsConnected(ctx context.Context, c Fiber, in *AssembleInput) bool {
...
func (r *Registry) requiredDepsConnected(
	ctx context.Context, c Fiber, in *AssembleInput, names []string,
) bool {
	r.mu.RLock()
	dr := r.depReg
	r.mu.RUnlock()
	...
	if err != nil {
		slog.Default().Warn("dep resolve failed, hiding block",
			"block", c.ID(), "requires", names, "err", err)
		return false
	}
	return connected
}

// DepProvider — a named in-app dependency ("calendar"/"smtp"), backed by a supplier holding
...
type DepProvider interface {
	...
	// gate's connected half looks only at this; not-connected → the depending block is hidden.
...
// never comes true.
//
// A block names **which specific action** it needs via `Requires:
// ["calendar:events.insert"]`; no colon behaves as before. The kernel doesn't know about any
// concrete seam or scope — only that some dependencies answer action-level questions;
// computing the answer is the supplier axis's business.
...
// composition root wires in the answer the supplier axis already computed; the kernel still
// knows nothing about seams or scopes.
...
// DepRegistry — the named-dependency provider registry the host holds. enabledFibers queries it
...
func (r *DepRegistry) Register(p DepProvider) {
	...
	if _, dup := r.providers[name]; dup {
		panic("registry: duplicate dep provider " + name)
	}
	r.providers[name] = p
}
...
func (r *DepRegistry) Unknown(names []string) []string {
	...
	for _, n := range names {
		...
		// "not recognized". Caught at boot: at runtime this would show as a block
...
// (true, nil) (a block with no dependencies isn't gated).
...
// supplier" line on a marketplace card wants the latter.
//
...
// answer "can it be done" for, so the block shouldn't be exposed at all. Unknown() already
