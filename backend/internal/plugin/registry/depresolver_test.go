// depresolver_test.go — supplier rework unit tests for the dep provider registry. Covers
// supplier-deps-tests.md §1: dep-registry, requires-boot-reject, enabledfibers-multi-dep (AND),
// connected-errors (E1).
...
func TestRequiresBootReject_UnknownDepFlagged(t *testing.T) {
	...
	depReg.Register(registry.NamedProvider(calendarDep, okConnected))
	depReg.Register(registry.NamedProvider("smtp", okConnected))
	...
	res, err := plugin.ParseConfig(cfg)
	...
	res2, err := plugin.ParseConfig(ok)
	require.NoError(t, err)
...
// ownerID through. The composition root uses it to register a supplier proxy's Connected
// method as a named dependency.
func TestNamedProvider_Delegates(t *testing.T) {
	t.Parallel()
	var gotOwner string
	p := registry.NamedProvider(calendarDep, func(_ context.Context, owner string) (bool, error) {
		gotOwner = owner
		return true, nil
	})
...
func TestDepRegistry_DuplicatePanics(t *testing.T) {
	...
	require.PanicsWithValue(t, "registry: duplicate dep provider calendar", func() {
		r.Register(fakeProvider{name: calendarDep})
	})
}
...
// enabledfibers-multi-dep — AllConnected is AND: A connected, B not → false;
// both connected → true.
...
// enabledfibers-multi-dep — an unregistered dependency name → false (defensive,
...
// fakeVisitorFiber — a minimal visitor-facing fiber: VisitorBinding returns a real binding (not
// hidden). depBlock embeds it + adds Requires (implementing RequiresDeps); noReqBlock is it alone.
type fakeVisitorFiber struct{ id string }

func (c fakeVisitorFiber) ID() string          { return c.id }
func (fakeVisitorFiber) Shape() registry.Shape { return registry.ShapeVisitorOnly }

func (c fakeVisitorFiber) VisitorBinding(
	context.Context, *registry.AssembleInput,
) (*registry.Binding, error) {
	return &registry.Binding{State: registry.FiberState{ID: c.id, Enabled: true}}, nil
}

func (fakeVisitorFiber) OwnerMCPBindings() []*registry.MCPBinding { return nil }

func (fakeVisitorFiber) SystemPromptFragment(context.Context, *registry.AssembleInput) string {
	return ""
}

func (fakeVisitorFiber) SystemPromptFragmentID(context.Context, *registry.AssembleInput) string {
	return ""
}

// depBlock — declares Requires → subject to seam-gate.
...
func (c depBlock) Requires() []string { return c.requires }

// noReqBlock — does not declare Requires → never seam-gated.
type noReqBlock struct{ fakeVisitorFiber }

func stateIDs(states []registry.FiberState) []string {
...
func regWithCalendar(c registry.Fiber, p fakeProvider) *registry.Registry {
	reg := registry.NewRegistry()
	reg.MustRegister(c)
	dr := registry.NewDepRegistry()
	dr.Register(p)
	reg.SetDepRegistry(dr)
	return reg
}

func calendarBookBlock() depBlock {
	return depBlock{
		fakeVisitorFiber: fakeVisitorFiber{id: "calendar.book"},
		requires:         []string{calendarDep},
	}
}

// enabledfibers-seam — `Requires:[calendar]` not connected → excluded from enabledFibers
// (VisitorStates omits it); connected → included. D-2: gating is one global choke point.
func TestEnabledFibers_SeamGate(t *testing.T) {
	t.Parallel()
	in := &registry.AssembleInput{OwnerID: "o1"}

	down := fakeProvider{name: calendarDep, connected: false}
	hidden := regWithCalendar(calendarBookBlock(), down).
		VisitorStates(context.Background(), in)
	require.Empty(t, stateIDs(hidden), "calendar not connected -> block hidden")

	shown := regWithCalendar(calendarBookBlock(), fakeProvider{name: calendarDep, connected: true}).
		VisitorStates(context.Background(), in)
...
func TestEnabledFibers_ArbitraryDepName_GatesIdentically(t *testing.T) {
	t.Parallel()
	const weatherDep = "weather"
	in := &registry.AssembleInput{OwnerID: "o1"}
	weatherBlock := depBlock{
		fakeVisitorFiber: fakeVisitorFiber{id: "weather.report"},
		requires:         []string{weatherDep},
	}

	hidden := regWithCalendar(weatherBlock, fakeProvider{name: weatherDep, connected: false}).
		VisitorStates(context.Background(), in)
	require.Empty(t, stateIDs(hidden), "arbitrary dep not connected -> cap hidden")

	shown := regWithCalendar(weatherBlock, fakeProvider{name: weatherDep, connected: true}).
		VisitorStates(context.Background(), in)
	require.Equal(t, []string{"weather.report"}, stateIDs(shown),
		"arbitrary dep connected -> exposed (same mechanism as calendar)")
}

// errBindBlock — VisitorBinding returns a controllable error, simulating an injected handle /
// injector construction failure (E3). Implements the interface directly instead of embedding
// fakeVisitorFiber (avoids a fieldalignment × embeddedstructfieldcheck conflict).
type errBindBlock struct {
	err error
	id  string
}

func (c errBindBlock) ID() string          { return c.id }
func (errBindBlock) Shape() registry.Shape { return registry.ShapeVisitorOnly }

func (c errBindBlock) VisitorBinding(
	context.Context, *registry.AssembleInput,
) (*registry.Binding, error) {
	return nil, c.err
}

func (errBindBlock) OwnerMCPBindings() []*registry.MCPBinding { return nil }

func (errBindBlock) SystemPromptFragment(context.Context, *registry.AssembleInput) string {
	return ""
}

func (errBindBlock) SystemPromptFragmentID(context.Context, *registry.AssembleInput) string {
	return ""
}

// supplier-errors (E3) — injected handle construction fails: VisitorBinding
...
func TestVisitorState_BindBuildFailure_DegradesNotCrash(t *testing.T) {
	...
	reg := registry.NewRegistry()
	reg.MustRegister(errBindBlock{id: "flaky.block", err: errors.New("handle build failed")})
	states := reg.VisitorStates(context.Background(), in)
	require.Len(t, states, 1, "build failure → still surfaced (degraded), not dropped")
	require.Equal(t, "flaky.block", states[0].ID)
	...
	reg2.MustRegister(errBindBlock{id: "hidden.block", err: registry.ErrHidden})
	require.Empty(t, reg2.VisitorStates(context.Background(), in), "ErrHidden → fully hidden")
}

// connected-errors (E1) at the enabledFibers layer — Connected returns an
// error → hidden (fail-closed).
func TestEnabledFibers_SeamError_Hides(t *testing.T) {
	t.Parallel()
	in := &registry.AssembleInput{OwnerID: "o1"}
	down := fakeProvider{name: calendarDep, err: errors.New("db down")}
	states := regWithCalendar(calendarBookBlock(), down).
		VisitorStates(context.Background(), in)
	require.Empty(t, stateIDs(states), "Connected errored -> hide when uncertain")
}

// enabledcaps-global-override's "not gated" boundary —
//   - a block that doesn't declare Requires → never seam-gated (even if the
//     provider isn't connected).
//   - no owner context (OwnerID empty) → can't resolve → not gated (present).
func TestEnabledFibers_SeamGate_NotGatedCases(t *testing.T) {
	t.Parallel()
	// A cap that doesn't declare Requires: stays present even if the provider
	// is down.
	always := noReqBlock{fakeVisitorFiber{id: "always"}}
	noReq := regWithCalendar(always, fakeProvider{name: calendarDep, connected: false}).
		VisitorStates(context.Background(), &registry.AssembleInput{OwnerID: "o1"})
	require.Equal(t, []string{"always"}, stateIDs(noReq), "no Requires -> not seam-gated")

	// Has Requires but no owner context: can't resolve, so not gated.
	disc := fakeProvider{name: calendarDep, connected: false}
	noOwner := regWithCalendar(calendarBookBlock(), disc).
		VisitorStates(context.Background(), &registry.AssembleInput{OwnerID: ""})
