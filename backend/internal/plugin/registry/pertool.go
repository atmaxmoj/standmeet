// pertool.go — the **action-level** gate: a block clearing the gate
// doesn't mean every one of its actions can be performed.
//
// Why this layer is needed (F-B-8 ⭐⭐): `Requires` is block-level, it
...
//
// Why the requirement can't just be raised to block level: that would
...
// block each need some extra dependency.
//
...
func (r *Registry) dropUnperformableTools(
	ctx context.Context, c Fiber, in *AssembleInput, b *Binding,
) {
...
// perToolRequires — whether this block has an action-level declaration,
// and whether this session has owner context to judge by. Either not holding
// → empty table → nothing gets filtered.
func perToolRequires(c Fiber, in *AssembleInput) map[string][]string {
...
// (fail-closed, the same discipline as the block-level layer): when it's
