...
//     never had this block" and "your quota ran out" call for opposite
//     actions (go ask the owner for access / wait or top up), yet before this both
//     returned the same `block_not_enabled`
...
// HiddenReasonForTool —— for the block that declares this tool, why it
// didn't show up in this session.
//
// Returns nil to mean "no reason to give": either no block declares this
...
//
// Only asks blocks that **can state their own tool names** (ToolNameKnower).
...
// bindingReason —— whether this block can actually assemble in this
// session. If it can → close it, return nil (the caller wants "why is it
// missing", and it plainly isn't).
func bindingReason(ctx context.Context, c Fiber, in *AssembleInput) error {
	b, err := c.VisitorBinding(ctx, in)
	if err != nil {
		// Wrap to attribute which block, but **keep the sentinel**: the caller
		// identifies the reason via errors.Is.
		return fmt.Errorf("block %q hidden: %w", c.ID(), err)
	}
	closeBinding(b)
	return nil
}
