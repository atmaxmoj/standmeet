// registry_visitor_bundle_test.go —— when a session opens, block
// instantiation must happen **concurrently**.
//
// `/api/v1/sessions` needs every block's tool spec, so there's no avoiding
// "dial each one" — but there's no reason to dial them one at a time. Each
// externalized block instantiation spawns a bwrap sandbox (~1s cold start),
...
// inFlight —— tracks "how many blocks are inside VisitorBinding right
// now" and its peak.
//
...
func (*barrierCap) Shape() registry.Shape { return registry.ShapeVisitorOnly }
...
func (c *barrierCap) VisitorBinding(
	...
	return &registry.Binding{
		Tools: []registry.BindingTool{registry.NewTool(c.id+"_tool", c.id, "", nil, nil)},
		State: registry.FiberState{ID: c.id, Enabled: true},
	}, nil
}

func (*barrierCap) OwnerMCPBindings() []*registry.MCPBinding { return []*registry.MCPBinding{} }
func (*barrierCap) SystemPromptFragment(_ context.Context, _ *registry.AssembleInput) string {
	return ""
}

func (*barrierCap) SystemPromptFragmentID(_ context.Context, _ *registry.AssembleInput) string {
	return ""
}

const barrierWait = 300 * time.Millisecond

// The four blocks must be in flight **at the same time**. The test is
// peak in-flight: serial is always 1, only concurrent reaches 4.
func TestAssembleVisitorBundle_DialsBlocksConcurrently(t *testing.T) {
	t.Parallel()
	ids := []string{"cap.a", "cap.b", "cap.c", "cap.d"}
	reg, flight := barrierRegistry(ids)

	bundle := reg.AssembleVisitorBundle(context.Background(), &registry.AssembleInput{
		OwnerID: "owner-1", Mode: "code",
	})

	require.Len(t, bundle.States, len(ids))
	require.Equal(t, len(ids), flight.max(),
		"blocks were instantiated one at a time (peak in-flight 1) — "+
...
// order matches Register order", and both the frontend's block list and
