...
// supplier dropped, a grant got narrowed — **has no path into that prompt**.
//
// That's how F-B-14 happened: when quota ran out the host hid the booking
// block, the agent had no tool by the third turn, yet the prompt still said
...
// every call.
//
// This file provides the missing channel itself: a block can say one thing
// for **this session**, and the host splices it into the instruction every turn.
// The block decides what to say (the host doesn't know about booking), and
// decides when to say it too (only the block knows its own gate).

package registry

import "context"

// SessionNoter —— optional interface: does this block have something it
...
// SessionNotes —— what each block wants to say in this session, in
