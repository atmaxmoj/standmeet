...
type rawManifest struct {
	...
	// VisitorTools — optional here, unlike in a block directory: a deploy-time
	// declaration usually names no tools and lets the dial report them.
	VisitorTools     []rawVisitorTool `json:"visitor_tools"`
	ID               string           `json:"id"`
...
// LoadOwnerSource —— reads + parses from a config file path. Empty path / file doesn't
// exist → empty Result (a deployment with no plugins by default is valid); a read
// failure (permissions, etc.) → error.
func LoadOwnerSource(path string) (Result, error) {
	if path == "" {
		return Result{}, nil
	}
...
// toVisitorTools — the config spelling of a tool declaration → the manifest's.
//
// An empty declaration comes back as an empty slice, not nil. "This declaration names no
// tools" is read downstream as `len(...) == 0` in every case (VisitorToolDrift,
// VisitorToolNames, providesAny) — no reader distinguishes nil from empty, so returning
// nil only buys a second spelling of the same fact. The unknown-vs-none distinction that
// does matter is carried elsewhere, by whether a block implements ProvidesVisitorTools at
// all.
func toVisitorTools(rs []rawVisitorTool) []VisitorTool {
	out := make([]VisitorTool, 0, len(rs))
	for i := range rs {
		out = append(out, VisitorTool{Name: rs[i].Name, Requires: rs[i].Requires})
	}
	return out
}
...
func toManifest(r *rawManifest) Manifest {
	m := Manifest{
		...
		Requires:         r.Requires,
		VisitorTools:     toVisitorTools(r.VisitorTools),
		Transport: Transport{
