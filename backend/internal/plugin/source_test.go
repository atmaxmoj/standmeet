...
// wrap —— wraps a single block's JSON into a full config document.
func wrap(one string) []byte {
	return []byte(`{"plugins":[` + one + `]}`)
}
...
func TestParseConfig_StdioAndHttp(t *testing.T) {
	...
	require.Equal(t, plugin.ShapeVisitorOnly, b.Shape)
	...
	require.Equal(t, "blocks/booking", b.PromptFragmentID)

	w := res.Manifests[1]
	require.Equal(t, "weather", w.ID)
	require.Equal(t, plugin.ShapeBoth, w.Shape)
...
func TestParseConfig_MalformedJSON(t *testing.T) {
	t.Parallel()
	_, err := plugin.ParseConfig([]byte(`{"plugins":[ this is not json`))
	require.Error(t, err)
}
...
func TestParseConfig_EmptyPlugins(t *testing.T) {
	t.Parallel()
	res, err := plugin.ParseConfig([]byte(`{"plugins":[]}`))
	require.NoError(t, err)
	require.Empty(t, res.Manifests)
	require.Empty(t, res.Skipped)

	res2, err2 := plugin.ParseConfig([]byte(`{}`))
...
func TestParseConfig_PerManifestRejections(t *testing.T) {
	...
	for _, c := range cases {
		res, err := plugin.ParseConfig(wrap(c.plugin))
...
func TestLoad_MissingOrEmpty(t *testing.T) {
	t.Parallel()
	res, err := plugin.LoadOwnerSource("")
	require.NoError(t, err)
	require.Empty(t, res.Manifests)

	res2, err2 := plugin.LoadOwnerSource(filepath.Join(t.TempDir(), "nope.json"))
	require.NoError(t, err2)
	require.Empty(t, res2.Manifests)
}
