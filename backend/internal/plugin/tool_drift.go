...
// nothing to flag — either this block never declared at all (the default for
...
func VisitorToolDrift(m *Manifest, actual []string) ToolDrift {
	...
	declared := VisitorToolNames(m.VisitorTools)
	missing := notIn(declared, actual)
	extra := notIn(actual, declared)
	return ToolDrift{
