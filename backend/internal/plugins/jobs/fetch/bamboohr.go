...
func (f *bambooHRFetcher) Fetch(
	...
	out := make([]jobsmodel.FetchedJob, 0, len(payload.Result))
...
func parseBambooHRConfig(raw []byte) (bambooHRConfig, error) {
	...
	if cfg.Company == "" {
		return cfg, fmt.Errorf("bamboohr missing company: %w", jobsmodel.ErrJobSourceConfigInvalid)
	}
	return cfg, nil
}
...
func bambooHRToDomain(j *bambooHRJob, slug string) jobsmodel.FetchedJob {
