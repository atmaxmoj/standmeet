...
// present (null → clear to instance default), matching credentialdomain.UpdateAPIKeyInput.
...
func (c *apiKeysCapability) handleUpdate(
	...
	row, err := c.deps.Keys.Update(ctx, &credentialdomain.UpdateAPIKeyInput{
		ID: args.ID, OwnerID: ownerID, Label: args.Label,
		RateLimitRPM: args.RateLimitRPM, SetRate: args.SetRate,
	})
	if err != nil {
		if errors.Is(err, credentialdomain.ErrAPIKeyNotFound) {
