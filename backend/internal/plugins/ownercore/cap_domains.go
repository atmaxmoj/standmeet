...
type domainsCapability struct {
	deps owner.AllowedDomainsDeps
	log  *slog.Logger
}

func newDomainsCapability(
	deps owner.AllowedDomainsDeps, log *slog.Logger,
) *domainsCapability {
...
func (c *domainsCapability) handleList(
	...
	list, err := owner.ListAllowedDomains(ctx, c.deps)
	if err != nil {
...
func (c *domainsCapability) handleAdd(
	...
	if err := owner.AddAllowedDomain(ctx, c.deps, args.Domain); err != nil {
...
func (c *domainsCapability) handleRemove(
	...
	if err := owner.RemoveAllowedDomain(ctx, c.deps, args.Domain); err != nil {
