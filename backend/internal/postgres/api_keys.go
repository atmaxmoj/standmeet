...
func (r *APIKeyRepo) Create(
	...
	if err != nil {
		return accessdomain.APIKey{}, fmt.Errorf(errParseOwnerIDPrefix, err)
	}
	roleUUID, rerr := parseUUID(in.AssumedRoleID)
	if rerr != nil {
		return accessdomain.APIKey{}, fmt.Errorf("parse role id: %w", rerr)
	}
	...
	if qerr != nil {
		return accessdomain.APIKey{}, fmt.Errorf("create api key: %w", qerr)
	}
...
func (r *APIKeyRepo) GetBySecretHash(
	ctx context.Context, hash []byte) (accessdomain.APIKey, error,
) {
	...
	if err != nil {
		return accessdomain.APIKey{}, fmt.Errorf("get api key by secret: %w", err)
	}
	return decodeAPIKey(&row), nil
}
...
func (r *APIKeyRepo) ListByOwner(
	ctx context.Context, ownerID string) ([]accessdomain.APIKey, error,
) {
	...
	out := make([]accessdomain.APIKey, 0, len(rows))
	for i := range rows {
...
func (r *APIKeyRepo) GetByID(
	ctx context.Context, id, ownerID string) (accessdomain.APIKey, error,
) {
	idUUID, err := parseUUID(id)
	if err != nil {
		return accessdomain.APIKey{}, fmt.Errorf(errParseKeyIDPrefix, err)
	}
	ownerUUID, oerr := parseUUID(ownerID)
	if oerr != nil {
		return accessdomain.APIKey{}, fmt.Errorf(errParseOwnerIDPrefix, oerr)
	}
	...
	if qerr != nil {
		return accessdomain.APIKey{}, fmt.Errorf("get api key: %w", qerr)
	}
	return decodeAPIKey(&row), nil
}
...
func (r *APIKeyRepo) Update(
	...
	if qerr != nil {
		return accessdomain.APIKey{}, fmt.Errorf("update api key: %w", qerr)
	}
	return decodeAPIKey(&row), nil
}
...
func decodeAPIKey(row *dbq.ApiKey) accessdomain.APIKey {
	return accessdomain.APIKey{
