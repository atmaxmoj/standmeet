...
// 这些 method 跟 OwnerRepo 同包，但 ownerdomain.Owner 不含 password_hash，
...
// ownerdomain.ErrOwnerNotFound（usecase 层翻译成 401，避免暴露"用户存在与否"）。
func (r *OwnerRepo) GetCredentialsByEmail(ctx context.Context, email string) (Credentials, error) {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return Credentials{}, ownerdomain.ErrOwnerNotFound
		}
...
func (r *OwnerRepo) GetByHandle(ctx context.Context, handle string) (ownerdomain.Owner, error) {
	q := dbq.New(r.pool)
	...
	if !errors.Is(err, pgx.ErrNoRows) {
		return ownerdomain.Owner{}, fmt.Errorf("get owner by handle: %w", err)
	}
	return r.getByAlias(ctx, handle)
}

func (r *OwnerRepo) getByAlias(ctx context.Context, handle string) (ownerdomain.Owner, error) {
	q := dbq.New(r.pool)
	...
	if err != nil {
		...
		return ownerdomain.Owner{}, fmt.Errorf("get owner by alias: %w", err)
	}
	return aliasRowToDomainOwner(&row), nil
}

// aliasRowToDomainOwner —— alias JOIN 用的 owner 子集（无 password_hash /
// custom_domain），映射到 ownerdomain.Owner identity。settings 不在这条路径上
// 提取——/<handle> 公开页用不到 BYOAI / AI settings。
func aliasRowToDomainOwner(o *dbq.GetOwnerByHandleAliasRow) ownerdomain.Owner {
...
func (r *OwnerRepo) GetByID(ctx context.Context, id string) (ownerdomain.Owner, error) {
	q := dbq.New(r.pool)
	pgID, perr := parseUUID(id)
	if perr != nil {
		return ownerdomain.Owner{}, fmt.Errorf("parse owner id: %w", perr)
	}
	...
	if err != nil {
		...
		return ownerdomain.Owner{}, fmt.Errorf("get owner by id: %w", err)
	}
	return toDomainOwner(&row), nil
}
