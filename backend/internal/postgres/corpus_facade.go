...
// getWiki —— pre-launch 只支持 wiki://<uuid> 形态；path-string 寻址留 A.3-IAM
// 重构 retriever 内核时一起加 query。
func (c *Corpus) getWiki(
	ctx context.Context, ownerID, idOrPath string,
) (domain.Document, error) {
	w, err := c.wiki.GetByID(ctx, ownerID, idOrPath)
	if err != nil {
...
func (c *Corpus) getOutput(
	...
	o, err := c.output.GetByID(ctx, ownerID, idOrPath)
	if err != nil {
