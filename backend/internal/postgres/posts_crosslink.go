// posts_crosslink.go —— public /blog 路径渲 `[[crosslink]]` 用的 slim
// resolution index 查询，从 posts.go 拆出来守 350-line cap。
...
// ListPublishedSlugAndTitle —— owner 全部 published post 的 (slug, title)；
// public 路径渲 [[crosslink]] 用的 resolution index。
func (r *PostRepo) ListPublishedSlugAndTitle(
	...
	rows, err := dbq.New(r.pool).ListPublishedSlugAndTitle(ctx, ownerUUID)
	if err != nil {
