...
func (r *ResumeDraftRepo) Create(
	ctx context.Context, in *domain.CreateResumeDraftInput,
) (domain.ResumeDraft, error) {
	ownerUUID, err := parseUUID(in.OwnerID)
	if err != nil {
		return domain.ResumeDraft{}, fmt.Errorf(errParseOwnerIDPrefix, err)
	}
	snapshotJSON, err := json.Marshal(in.JobSnapshot)
	if err != nil {
		return domain.ResumeDraft{}, fmt.Errorf("marshal job snapshot: %w", err)
	}
	contentJSON, err := json.Marshal(in.ResumeContent)
	if err != nil {
		return domain.ResumeDraft{}, fmt.Errorf("marshal resume content: %w", err)
	}
	q := dbq.New(r.pool)
	...
	if err != nil {
		return domain.ResumeDraft{}, fmt.Errorf("create resume draft: %w", err)
	}
...
func (r *ResumeDraftRepo) GetByID(
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return domain.ResumeDraft{}, domain.ErrResumeDraftNotFound
		}
		return domain.ResumeDraft{}, fmt.Errorf("get resume draft: %w", err)
	}
	return toDomainResumeDraft(&row)
}
...
func (r *ResumeDraftRepo) UpdateContent(
	ctx context.Context, ownerID, id string, content *domain.ResumeContent,
) (domain.ResumeDraft, error) {
	key, err := parseDraftKey(ownerID, id)
	if err != nil {
		return domain.ResumeDraft{}, err
	...
	if err != nil {
		...
		return domain.ResumeDraft{}, fmt.Errorf("update resume draft: %w", err)
	}
	return toDomainResumeDraft(&row)
}
...
// toDomainResumeDraft —— sqlc Row → domain.ResumeDraft（含 jsonb unmarshal）。
func toDomainResumeDraft(row *dbq.ResumeDraft) (domain.ResumeDraft, error) {
	var snapshot domain.FetchedJob
	if err := json.Unmarshal(row.JobSnapshot, &snapshot); err != nil {
		return domain.ResumeDraft{}, fmt.Errorf("unmarshal job snapshot: %w", err)
	}
	var content domain.ResumeContent
	if err := json.Unmarshal(row.ResumeContent, &content); err != nil {
		return domain.ResumeDraft{}, fmt.Errorf("unmarshal resume content: %w", err)
	}
