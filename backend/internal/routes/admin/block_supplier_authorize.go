// block_conn_authorize.go — the return leg of the oauth2 dance (the callback). The dance
...
// always 302s back to the suppliers area (a constant target, to avoid feeding the
...
// supplierOAuthCallback — the provider returns with code+state → exchange for a token.
// A provider rejection (?error=) / a state mismatch / a token exchange failure all →
// 302 back to the suppliers area with connect_error=1.
func (h *Handlers) supplierOAuthCallback() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Query().Get("error") != "" {
			http.Redirect(w, r, "/admin/suppliers?connect_error=1", http.StatusFound)
			return
		}
		err := h.BlocksAdmin.Svc.Callback(
		...
		if err != nil {
			...
			h.Log.Warn("block oauth callback", logErrKey, err)
			http.Redirect(w, r, "/admin/suppliers?connect_error=1", http.StatusFound)
			return
		}
		http.Redirect(w, r, "/admin/suppliers", http.StatusFound)
	}
}
