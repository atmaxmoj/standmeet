// posts_multipart.go —— admin POST/PATCH /posts 接的 multipart 解析。
// 拆出来守 350-line cap。
//
// 形态：
//   field "data"            —— JSON post fields (postSaveRequest)
...
const maxPostMultipartSize = 50 << 20 // 50MB

// parsedMultipart —— parsePostMultipart 多返回打包（避开 funcresult-limit 2）。
...
func parsePostMultipart(w http.ResponseWriter, r *http.Request) (parsedMultipart, error) {
	...
	r.Body = http.MaxBytesReader(w, r.Body, maxPostMultipartSize)
	// #nosec G120 -- 上游已有 MaxBytesReader bound。
	if err := r.ParseMultipartForm(maxPostMultipartSize); err != nil {
		return parsedMultipart{}, parseMultipartErr(err)
	}
	return decodePostFromForm(r)
}

func decodePostFromForm(r *http.Request) (parsedMultipart, error) {
	req, perr := decodePostJSON(r)
	if perr != nil {
...
func decodePostJSON(r *http.Request) (postSaveRequest, error) {
	raw := r.FormValue("data")
	if raw == "" {
		return postSaveRequest{}, errors.New("missing 'data' field")
	}
	var req postSaveRequest
	if err := json.Unmarshal([]byte(raw), &req); err != nil {
		return postSaveRequest{}, errors.New("data: invalid JSON")
	}
	return req, nil
}
