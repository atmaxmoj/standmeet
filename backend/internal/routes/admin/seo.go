...
func (h *Handlers) putSEOSettings() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		saved, err := h.SEOAdmin.SEO.UpsertSettings(r.Context(), &corpus.SEOSettings{
			OwnerID:       ownerID,
...
func writeSEOSettings(log *slog.Logger, w http.ResponseWriter, settings *corpus.SEOSettings) {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
...
func writeWikiSEOResp(
	log *slog.Logger, w http.ResponseWriter, wiki *corpus.Wiki, unpinned []string,
) {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	resp := wikiSEOResp{
...
func writeOutputSEOResp(log *slog.Logger, w http.ResponseWriter, out *corpus.Output) {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	resp := wikiSEOResp{
		ID:        out.ID(),
