// config_socket.go — the socket inbound controller: a block inside the sandbox asks the
// host for **its own config**.
//
// Why a dedicated op instead of letting the sandbox call blockstore.query on that doc itself:
...
//
// Like blockstore, it's bound to one cap's namespace at construction time — the sandbox can't
...
// ConfigOps — blockconfig.get. Read-only: the owner changes config through the panel, not
// through the sandbox.
//
...
//
// cfg is nil (this block declared no config) → don't expose the op. A source with
// nothing to give says so itself.
func ConfigOps(cfg BoundConfig) []hostop.Op {
	if cfg == nil {
		return []hostop.Op{}
	}
	return []hostop.Op{{
		Name:        "blockconfig.get",
...
func getHandler(cfg BoundConfig) hostop.Invoke {
	return func(ctx context.Context, raw json.RawMessage) (json.RawMessage, error) {
		...
		if err := json.Unmarshal(raw, &req); err != nil {
			return nil, fmt.Errorf("blockconfig.get: decode: %w", err)
		}
		values, err := cfg.Values(ctx, req.OwnerID)
		if err != nil {
			return nil, fmt.Errorf("blockconfig.get: %w", err)
		}
		out, merr := json.Marshal(values)
		if merr != nil {
			return nil, fmt.Errorf("blockconfig.get: marshal: %w", merr)
		}
		return out, nil
	}
}
