// Package blockstore — the socket inbound controller. blockstore's four host ops
// (insert/query/count/delete): an offline-sandboxed cap reads and writes its own isolated
// store over the socket. Classified by domain, it lives with blockstore rather than in the
...
// four onto whichever blocks need storage.
...
// StoreOps — one block's **own** storage: insert / query / count / delete. store is bound at
// construction time to that block's namespace, so the sandbox side can't fill in someone
// else's table — the isolation is built in, not checked on every request.
//
// store is nil (this block didn't ask for storage) -> no ops are exposed. That check
...
func StoreOps(store BoundStore) []hostop.Op {
	if store == nil {
		return []hostop.Op{}
	}
	return []hostop.Op{
		{
			Name: "blockstore.insert", Description: "Insert a document into your own collection.",
			Invoke: insertHandler(store),
		},
		{
			Name: "blockstore.query", Description: "Query your own collection by JSONB filter.",
			Invoke: queryHandler(store),
		},
		{
			Name: "blockstore.count", Description: "Count documents matching a filter.",
			Invoke: countHandler(store),
		},
		{
			Name: "blockstore.delete", Description: "Delete documents matching a filter.",
			Invoke: deleteHandler(store),
		},
		{
			Name: "blockstore.query_records", Description: "Query, returning records with ids.",
			Invoke: queryRecordsHandler(store),
		},
		{
			Name: "blockstore.delete_by_id", Description: "Delete one record by its id.",
			Invoke: deleteByIDHandler(store),
		},
		{
			Name: "blockstore.claim",
...
func insertHandler(store BoundStore) hostop.Invoke {
	return func(ctx context.Context, raw json.RawMessage) (json.RawMessage, error) {
		...
		if merr != nil {
			return nil, fmt.Errorf("blockstore.insert: marshal: %w", merr)
		}
		return out, nil
	}
}
...
func queryHandler(store BoundStore) hostop.Invoke {
	return func(ctx context.Context, raw json.RawMessage) (json.RawMessage, error) {
		...
		if merr != nil {
			return nil, fmt.Errorf("blockstore.query: marshal: %w", merr)
		}
		return out, nil
	}
}
...
func queryRecordsHandler(store BoundStore) hostop.Invoke {
	return func(ctx context.Context, raw json.RawMessage) (json.RawMessage, error) {
		...
		if merr != nil {
			return nil, fmt.Errorf("blockstore.query_records: marshal: %w", merr)
		}
		return out, nil
	}
}
...
func deleteByIDHandler(store BoundStore) hostop.Invoke {
	return func(ctx context.Context, raw json.RawMessage) (json.RawMessage, error) {
		...
		if merr != nil {
			return nil, fmt.Errorf("blockstore.delete_by_id: marshal: %w", merr)
		}
		return out, nil
	}
}
...
func countHandler(store BoundStore) hostop.Invoke {
	return func(ctx context.Context, raw json.RawMessage) (json.RawMessage, error) {
		...
		if merr != nil {
			return nil, fmt.Errorf("blockstore.count: marshal: %w", merr)
		}
		return out, nil
	}
}
...
func deleteHandler(store BoundStore) hostop.Invoke {
	return func(ctx context.Context, raw json.RawMessage) (json.RawMessage, error) {
		...
		out, merr := json.Marshal(map[string]int64{"deleted": n})
		if merr != nil {
			return nil, fmt.Errorf("blockstore.delete: marshal: %w", merr)
		}
		return out, nil
	}
}
