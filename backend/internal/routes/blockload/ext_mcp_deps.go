// ext_mcp_deps.go —— the seam-dep gate for ext-mcp tools (§1: ext-mcp does not
// wire deps by default).
//
// ext-mcp is lowest trust: someone else's process, where the owner only registered a URL.
// Even when its tool declares a dependency on some seam (like calendar) via
// `_meta.requires` and that seam is already supplied, the handle is **not** injected
// by default — a supplier handle carries owner privileges, and auto-granting it to any
...
// logic is collected here, in the same module as ext-mcp tool exposure (ext_mcp.go's
...
// DepConnected —— whether a named seam dependency is supplied (satisfied by
// registry.DepRegistry). The grant lives in server config; connected status is checked here;
...
// extToolDepsAllowed —— whether this ext-mcp tool's seam-dep is allowed through
// (lowest trust, deny by default): every dep in requires must be explicitly granted by the
// owner AND its seam already supplied. A tool with no requires is unaffected by this
// gate.
