// resume_read_test.go —— the résumé-reading block: which one, and isolation.
//
...
func resCodeSubject(id string) registry.Subject {
	return registry.Subject{Kind: registry.SubjectCode, ID: id}
}

// runResumeTool —— assemble for the subject, invoke resume_read, return its result text.
func runResumeTool(
	t *testing.T, src resumeSource, owner string, subj registry.Subject, argsJSON string,
) string {
	t.Helper()
	c := newResumeReadFiber(src)
	b, err := c.VisitorBinding(context.Background(), &registry.AssembleInput{
		OwnerID: owner, Subject: subj,
	})
	require.NoError(t, err)
	require.NotNil(t, b, "the résumé tool must be exposed for an application session")
	var bt *registry.BindingTool
	for i := range b.Tools {
...
func TestResumeRead_hidden_unless_the_code_is_an_application(t *testing.T) {
	t.Parallel()
	c := newResumeReadFiber(twoResumes())
	cases := []struct {
		in   *registry.AssembleInput
		name string
	}{
		{name: "ordinary code (no application)", in: &registry.AssembleInput{
			OwnerID: resOwnerA, Subject: resCodeSubject("PLAIN-CODE"),
		}},
		{name: "api-key subject", in: &registry.AssembleInput{
			OwnerID: resOwnerA,
			Subject: registry.Subject{Kind: registry.SubjectAPIKey, ID: resCodeA},
		}},
		{name: "anonymous (public/byoai)", in: &registry.AssembleInput{
			OwnerID: resOwnerA, Subject: registry.Subject{Kind: registry.SubjectCode, ID: ""},
		}},
	}
	for _, tc := range cases {
		b, err := c.VisitorBinding(context.Background(), tc.in)
		require.ErrorIs(t, err, registry.ErrHidden, tc.name)
