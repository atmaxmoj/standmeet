// skill_runner.go —— Phase C: skillRunnerFiber (faithful Agent
// Skills + progressive disclosure).
//
// A skill that's role-granted and enabled only puts name+description into the system
// prompt (L1, see visitor_role_snapshot.collectRoleSkillBundle). This block exposes
...
//
// One block, two tools. Role contains ≥1 enabled skill → enabled; otherwise
...
type skillRunnerFiber struct {
	deps skillRunnerDeps
}

func newSkillRunnerFiber(deps skillRunnerDeps) *skillRunnerFiber {
	return &skillRunnerFiber{deps: deps}
}

func (*skillRunnerFiber) ID() string { return capSkillRunner }
func (*skillRunnerFiber) Shape() registry.Shape {
	return registry.ShapeVisitorOnly
}

func (*skillRunnerFiber) OwnerMCPBindings() []*registry.MCPBinding {
	return []*registry.MCPBinding{}
}

func (*skillRunnerFiber) SystemPromptFragment(
	_ context.Context, _ *registry.AssembleInput,
) string {
	// L1 (name+description) is injected through ComposeBasePersona's SkillPrompts
	// channel, not here; this block's fragment stays empty to avoid duplication.
	return ""
}

func (*skillRunnerFiber) SystemPromptFragmentID(
	_ context.Context, _ *registry.AssembleInput,
) string {
	return ""
}
...
// skill_use + skill_run_script; otherwise ErrHidden (the block is hidden).
func (c *skillRunnerFiber) VisitorBinding(
	...
	return &registry.Binding{
		Tools: tools,
		State: registry.FiberState{ID: capSkillRunner, Enabled: true},
	}, nil
}
...
func loadSkillsForBinding(
	ctx context.Context, skills conversation.SkillGetter, in *registry.AssembleInput,
) []marketplace.Skill {
...
func makeSkillUse(skills []marketplace.Skill) registry.RunFn {
...
func makeSkillRunScript(runner sandbox.Runner, skills []marketplace.Skill) registry.RunFn {
	byName := skillsByName(skills)
