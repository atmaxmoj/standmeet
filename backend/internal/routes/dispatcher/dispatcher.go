// Package dispatcher -- the outbound convergence point: every block this instance
// exposes gets declared here once, then dispatched out to each face.
//
// # Why this extra layer
//
// Before this layer existed: the same block was hand-written once on admin HTTP,
...
//
//   - **It leaks.** Add a block, admin gets it and MCP forgets it (or the reverse),
...
//     define their own payload shape, so the same block slowly grows two different
...
//
// The convergence point fixes all three at once: a block is declared exactly once,
...
// block a face gets has already passed through the same chain.
//
...
// second block registry (that's the declaration registry for the block axis, tracking "what
...
//     facade.
//  2. supplier operations -- seam+verb on the supplier axis.
//  3. registry blocks -- the ones an agent loads, the outward-facing slice of them.
//
...
//
// The convergence point itself **implements no block**: it imports each domain's
...
//
// Why it lives here: every face can only get a block through the convergence point
// (the HTTP face still hand-writes its REST shape, but the block behind a handler
...
//
//	a block is declared once  → the two faces can't grow into two different shapes
...
// **all** operations: every block a face gets has already passed through this chain;
...
// pulling a block is the same act as registering its projection. It's exported here
...
// **Unexported**: a face can only get a block through Face.Op, so "which op did this
