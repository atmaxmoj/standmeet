// microsite_preview.go —— lets the owner see what this page looks like from the panel.
//
//	GET /api/v1/microsites/{slug}/preview/{token}
//	GET /api/v1/microsites/{slug}/preview/{token}/*
//
...
// MicrositePreviewHandlers —— what preview needs. Both functions are injected by the
// composition root.
type MicrositePreviewHandlers struct {
	Log *slog.Logger
...
func (h *MicrositePreviewHandlers) Mount(r chi.Router) {
	r.Get("/microsites/{slug}/preview/{token}", h.previewAsset())
	r.Get("/microsites/{slug}/preview/{token}/*", h.previewAsset())
}

func (h *MicrositePreviewHandlers) previewAsset() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		ServeBuildAsset(w, r, &BuildAssetReq{
			...
			BaseHref:   "/api/v1/microsites/" + slug + "/preview/" + token + "/",
		})
	}
}
...
func (h *MicrositePreviewHandlers) resolve(
