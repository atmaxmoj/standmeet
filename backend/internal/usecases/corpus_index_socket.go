...
type corpusRunner func(context.Context, CorpusLister, *corpusIndexReq) (string, error)

// RegisterCorpusIndexSocket —— prod 接线：从 postgres 的 CorpusIndexDeps 建 pgCorpusLister，
// 注册三个 corpus op。
func RegisterCorpusIndexSocket(srv *capsocket.Server, deps *CorpusIndexDeps) {
...
// CorpusLister。prod 经 RegisterCorpusIndexSocket 注入 pgCorpusLister；agentcore 的 eval
// mini-host 注入一个 Driver-backed 内存 lister，让消费方不碰 postgres 也能装配。
func RegisterCorpusIndexLister(srv *capsocket.Server, lister CorpusLister) {
...
func runCorpusSearch(ctx context.Context, l CorpusLister, req *corpusIndexReq) (string, error) {
	var args struct {
...
func runCorpusRead(ctx context.Context, l CorpusLister, req *corpusIndexReq) (string, error) {
	var args struct {
...
func runCorpusList(ctx context.Context, l CorpusLister, req *corpusIndexReq) (string, error) {
	var args struct {
		Path string `json:"path"`
		Page int    `json:"page"`
	}
...
func runCorpusLinks(ctx context.Context, l CorpusLister, req *corpusIndexReq) (string, error) {
	var args struct {
		Path string `json:"path"`
	}
	if uerr := json.Unmarshal(req.Args, &args); uerr != nil {
