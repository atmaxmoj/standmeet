...
//
// 失败一律返 ownerdomain.ErrUnauthorized（不告诉 token 错 vs 过期 vs 用过）。
...
// 任何步骤失败统一返 ownerdomain.ErrUnauthorized；ErrPasswordTooShort 单独保留
...
func ConsumePasswordResetToken(
	...
	owner, err := deps.Owners.GetActiveResetToken(ctx)
	if err != nil {
		return fmt.Errorf("load reset token: %w", err)
	}
	if !matchesAndFresh(tokenPlaintext, owner.Hash, owner.IssuedAt) {
		return ownerdomain.ErrUnauthorized
	}
	return applyNewPassword(ctx, deps, owner.OwnerID, newPassword)
}
