// report_store_socket.go —— report.store host op：断网沙箱 cap 把生成的原始 HTML 交给 host,
...
func RegisterReportStoreOp(srv *capsocket.Server, reports ReportStore) {
	srv.Handle("report.store", func(
		...
		styled := ReportStyledDocument(SanitizeReportHTML(req.HTML))
