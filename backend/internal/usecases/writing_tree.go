...
func WritingTreeChildren(writings []corpus.Writing, parentID string) []WritingTreeNode {
...
func writingNode(w *corpus.Writing, hasKids map[string]bool) WritingTreeNode {
...
func writingIDSet(writings []corpus.Writing) map[string]bool {
...
func writingEffectiveParent(w *corpus.Writing, present map[string]bool) string {
	pid, ok := w.ParentID()
	if ok && present[pid] {
		return pid
	}
...
func writingParentsWithChild(
	writings []corpus.Writing, present map[string]bool,
) map[string]bool {
