...
> **状态：** 设计定稿（2026-06-20，2026-06-23 对齐纯 AND·deny 模型）。配 [`block-acl-hierarchy.md`](block-acl-hierarchy.md)（业务 + 代码架构）。本文件只管**测什么、怎么测、跟现有怎么结合**。
...
`chat-book-success` · `chat-book-not-connected` · `mcp-skill-grant-booking` · `external-mcp-tools` · `retrieval-block-state` · `session-block-bundle` · `block-disable-while-attached`（global 活闸）。这些一条不能红。
...
| spec | 场景 | 期望 | 锁的 |
|---|---|---|---|
| `acl-code-deny-drops-prompt-fragment` | code deny retrieval（贡献 fragment 的 cap） | 该 fragment **不在** `system_prompt_part_ids`；hash 随之变 | feature-floor：cap 贡献 fragment+part_ids+hash（对齐 `session-block-bundle`） |
...
| `acl-global-live-mid-session` | global | issue → 关 global → 同 session | **立刻消失** | global 活（= 现有 `block-disable-while-attached` 的同族，引用即可） |
