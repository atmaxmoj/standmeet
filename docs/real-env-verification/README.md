...
- **round directory** — `make verify-round` opens `e2e/manual-runs/<UTC timestamp>/` (gitignored): the generated **run sheet** you work down, and one self-contained **trajectory** folder per module (`trajectory/<module>/{<module>.md, shots/}`, a screenshot per operation). The trajectory is also what the **designer pass** (§1c, `critique` / `audit` skills) reviews — each module carries two verdicts, `Result` (works) and `Design` (uiux designer verified). See `sop.md` §0.
- **`items/`** — one doc per **module** (a functional unit + the surface it owns), each decomposing its checks into runnable steps + expected + the backing e2e + a fresh-eyes LOOK. Each module notes the historical `§`-findings it inherits, and states in `Exclusive:` the one real-world thing it must own alone — which is what schedules the round (`sop.md` §0·5).
- **running it in parallel** — `make stack-init` gives a checkout its own dev **and** prod stack, so N worktrees are N instances; `make stack-ready` says whether this one can open a round. 43 of the 54 modules own nothing outside their stack and run at once; the other 11 sit in lanes behind the one Gmail account, the one calendar, the GitHub rate limit, a vendor account and a camera.
