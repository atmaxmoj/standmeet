...
- **round directory** — `make verify-round` opens `e2e/manual-runs/<UTC timestamp>/` (gitignored): the generated **run sheet** you work down, and one self-contained **trajectory** folder per module (`trajectory/<module>/{<module>.md, shots/}`, a screenshot per operation). See `sop.md` §0.
