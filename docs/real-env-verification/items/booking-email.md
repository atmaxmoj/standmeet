...
- **Real dep:** A real calendar connector (see [[calendar-connect]]), a real mail connector (see [[mail-supplier]]), and an inbox you can read.
- **Exclusive:** gmail-inbox
- **Backing e2e:** `booking-confirmation-email` · `supplier-send-confirmation-tool`.
...
- **Backing test:** `booking-confirmation-email.spec.ts` · `supplier-send-confirmation-tool.spec.ts`
...
- **Backing test:** `supplier-send-confirmation-tool.spec.ts`
...
- **Backing test:** `supplier-send-confirmation-tool.spec.ts` (visibility only) · perception over time → `gap`
