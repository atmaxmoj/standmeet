...
- **Backing e2e:** `supplier-happy-matrix` (mock dance) · `chat-book-token-refresh` · `security-oauth-callback-state` · `supplier-err-refresh-network` · `supplier-retry-invalid-grant-no-retry` · `connector-gcal-rotate-creds-reverify`.
...
- **Backing test:** `supplier-happy-matrix.spec.ts` (mock dance) · the real dance → `gap`
...
- **Backing test:** `supplier-retry-invalid-grant-no-retry.spec.ts` · `supplier-err-refresh-network.spec.ts`
