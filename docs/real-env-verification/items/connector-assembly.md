...
- **Status:** 🟡 (2026-07-15 live) — validate-spec mechanism ✅ (real server-side fetch + parse + **friendly** `ok:false`, no crash); real-vendor ingest 🔴 **F-H-1** (3.0.x-only ceiling rejects the fetched Cal.com spec); real proxied call (H3) not reached (gated on a supported spec)
...
- **Result:** 🔴 **F-H-1** (2026-07-15 live). Drove the real "fetch from URL" path with `https://api.cal.com/v2/docs-json`: the backend really fetched + parsed it and `POST /api/admin/connectors/validate-spec` returned `{"ok":false,"error":"only OpenAPI 3.0.x is supported (not 2.0 / 3.1)"}`. So the **ingest is capped at 3.0.x** and a real modern spec is rejected. **Two honest halves:** (a) the mechanism + friendly-error is GOOD — real fetch, clear message, no crash; (b) but a growing share of real vendor specs are **3.1** (released 2021), so the "real vendor OpenAPI" path this module is meant to prove can fail on a live vendor. **Caveat (not faked):** `docs-json` 307-redirects in a plain UA, so I did NOT confirm the *canonical* Cal.com v2 spec version — only that the fetched doc was non-3.0.x. To close: obtain Cal.com's canonical spec URL + version; if 3.1, this blocks the whole module's designated vendor. RED to add: feed a 3.1 spec, assert either support or a first-class "unsupported version, here's what to do" path (not just a dead-end reject).
- **Result:** ⬜ (real proxied call H3 — gated on a supported spec + `CALCOM_API_KEY`)
