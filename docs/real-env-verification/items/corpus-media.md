...
- **Mock gap:** prod compose ships `STORAGE_USE_SSL=false` even in the prod profile — verify a real store fronted by TLS (or a real S3/R2 endpoint) works with SSL on, since CI never exercises the `https://` object path or a real `STORAGE_PUBLIC_URL`.
...
- **Mock gap:** `sync-g-hidden` uses a synthetic 1×1 PNG; real vaults carry heavier/varied image types and real `cover_image` refs.
...
- **Mock gap:** CI now renders the download row and asserts filename + `size_bytes` + the `download` attribute (`genre-assets-reader`). What it can't do is **download**: whether the click lands a file on disk or opens the PDF as a page, and whether a real 4 MB PDF reads as `4.1 MB` rather than `NaN MB` / `4194304 B` — every CI fixture is tens of bytes, so the formatting branches never run.
...
- **Note:** `corpus.update` replaces every non-hero field wholesale, so a cover must be submitted together with the body. A cover-only update wipes the body **and the save succeeds**, with a success toast. Same family as [[corpus-acl-editing]]'s "an edit form must not zero the fields it does not show", one field over. The damage is silent and a real corpus has no undo, which is why a human drives this one.
...
`upload_media` appears in older prose and design notes but was never implemented. The real tool
takes a genre, an entry id and an **https URL** that the server fetches itself; plain http is
refused by design, and no bytes are posted through MCP. The panel's own path is different: the
browser hands over a local file, so no image host is needed to drive it.
