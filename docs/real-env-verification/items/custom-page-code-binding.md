# custom-page-code-binding — A page is a rendering of a code
...
- **Steps:** From `/admin/custom-pages`, paste source into the page, build it, watch the build reach a terminal state, promote it. Then open `/p/<slug>` as a visitor.
...
- **Backing test:** `custom-page-admin-authoring.spec.ts`
...
- **Backing test:** `custom-page-code-binding.spec.ts`
