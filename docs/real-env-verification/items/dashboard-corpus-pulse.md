...
- **Status:** 🔴 found (2026-07-17). The "corpus pulse · 14d" sparkline on `/admin/dashboard` is
  **hardcoded** (`MOCK_14D = [4,7,2,6,11,3,8,5,9,12,6,14,9,17]`), rendered beside the real total and a
  green "active" badge so it reads as a genuine 14-day activity trend. The real per-day series already
  exists — `/api/admin/stats/growth` returns `series:[{day,count}]` (a real `date_trunc GROUP BY` over
  corpus writes) — and the dashboard's own schema drops it, drawing the fake curve instead. A chart
  the owner reads as "is my corpus alive / growing" that never once reflects the corpus. (Rot-sweep A1.)
...
- **Expected:** it reflects real shortlist state. (Currently a hardcoded `0` styled like the real
  ...
  real source.)
- **Result:** ⬜ (deferred — needs the shortlist data source)
