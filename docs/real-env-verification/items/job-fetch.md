...
- **Status:** 🟡 blocked-by-setup — source '1 active' (gitlabgreenhouse); 1d pool aged out (0 listings); MCP jobs.fetch_new needed to repopulate; e2e-covered
...
- **Result:** 🟡 blocked-by-setup this round (outside self-serve scope §0) — 1d job pool aged out (0 listings); needs MCP jobs.fetch_new to repopulate. Backing e2e green; not manually driven (no live disproof, no manual proof).
