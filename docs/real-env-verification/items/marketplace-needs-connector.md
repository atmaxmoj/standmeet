...
- **Mock gap:** No real search result carries a `needs` value yet, so the backing spec pins the search response to a skill that declares one. A live search cannot exercise this until the backend wire carries the field.
