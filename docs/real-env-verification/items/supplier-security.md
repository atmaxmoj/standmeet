...
- **Exclusive:** none
- **Backing e2e:** `supplier-secret-no-leak` · `connector-security` · Go unit `egress_test.go`. Rotation → `gap`.
...
- **Backing test:** `supplier-secret-no-leak.spec.ts` · `supplier-security.spec.ts`
...
- **Backing test:** `supplier-security.spec.ts` (hits the whitelisted mock) · `egress_test.go` (unit) · a real rebinding host → `gap`
