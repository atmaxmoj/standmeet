...
### 1a · 同步侧(喂图) 🟡
- ✅ `writings` 一层的 export(zip)/ import(整 vault) 手动批量,`publish: true` 闸,web-edits-win。
...
> **块一小结**:数据(树+边)全现成,决策(非 vector / portable / per-host 但可选同步 owner CSS)全 settle,**风险局部,不牵一发动全身**。
...
admin 接真后端 + 基建杂活,智力上不是大块:`#101`(/admin/system 真 system-info)、`#102`(/admin/seo 真后端)、`#103`(role 卡编辑 prompt)、`#104`(per-code prompt)、`#105`(MCP key 下载凭证+README)、`#106`(inference 计费)、`#117`(URL env prod fallback)、`#118`(MCP vs HTTP admin parity)、`#132`(通用重试 HTTP infra)、`#152`(mock 正名)。
