...
// Asset URLs are now served through the backend forwarder (/api/v1/assets/<id>), not presigned
// minio URLs — so there is no longer a browser-visible minio host to match. Tests assert the
// forwarder path directly (`toContain('/api/v1/assets/')`).
