// supplier-agent-rig.ts —— the rig for "install an openapi supplier → connect →
// grant → start a visitor session".
//
// Split out of supplier-agent-tools.spec.ts: that file hit the 350-line gate,
// and **the same rig** now has a second user (the tool-names one, see
// supplier-agent-tool-names.spec.ts). What's split out is the rig, not the
...
export const AGENT_OWNER = {
  email: 'supplier-agent-tools@example.com',
 ...
 * connectSupplier runs this dance; if a sample spec used a different auth,
...
// CreateBody —— the input to POST /api/admin/suppliers (openapi supplier).
export interface CreateBody {
  spec: unknown;
  binding?: unknown;               // category binding (optional; agent-only suppliers have none)
...
// connectSupplier —— store the oauth2 credentials (spec-derived) + run the mock
// dance to connect the supplier.
...
export async function disconnectSupplier(
  request: APIRequestContext, csrf: string, id: string,
): Promise<void> {
...
// createAndConnect —— create an openapi supplier + connect it, returning the
// supplier id (asserts 201 + connected).
export async function createAndConnect(
  request: APIRequestContext, csrf: string, body: CreateBody,
): Promise<string> {
  const r = await createSupplier(request, csrf, body);
  expect(r.status, r.error ?? '').toBe(201);
  await connectSupplier(request, csrf, r.id!);
  return r.id!;
}
...
export async function diagAgentCall(
  ...
  const res = await request.post(
    `${BACKEND}/api/admin/diag/supplier/${encodeURIComponent(id)}/agent-call`,
