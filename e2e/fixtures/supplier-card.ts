// supplier-card.ts —— the generic driver for a card on /admin/suppliers:
// locate it, fill oauth2 credentials, tick scopes, assert connection status, and
// read the mock OAuth provider's records.
//
// These helpers originally lived in `supplier-connect-flow.spec.ts`. When a
...
/**
 * activateSupplier —— set a supplier as the active one for its category slot
 * (one active per category; connecting doesn't auto-preempt).
 *
 * Waits until activation is confirmed in `GET /suppliers` before returning ——
...
export async function activateSupplier(
  ...
  await expect.poll(async () => {
    ...
    const rows = (await list.json() as { suppliers?: { id: string; active?: boolean }[] })
      .suppliers ?? [];
    return rows.find((c) => c.id === id)?.active === true;
  }, { timeout: 10_000 }).toBe(true);
}

/** openSupplierCard —— click into the suppliers section from the sidebar and
 *  locate a supplier's card (without page.goto). */
export async function openSupplierCard(page: Page, id: string): Promise<Locator> {
...
export async function ensureDisconnected(card: Locator): Promise<void> {
  const btn = card.getByTestId('supplier-disconnect-button');
...
export async function fillOAuth2Creds(
  ...
  await card.getByTestId('supplier-field-client_id').fill(clientId);
  await card.getByTestId('supplier-field-client_secret').fill(clientSecret);
}

/** selectScope —— check/uncheck one scope in the supplier's derived form. */
export async function selectScope(
  card: Locator, scope: string, checked: boolean,
): Promise<void> {
  const box = card.getByTestId(`supplier-scope-${scope}`);
...
export async function expectConnected(card: Locator): Promise<void> {
  await expect(card.getByTestId('supplier-status')).toHaveText(/^(connected|已连接)$/i);
}
