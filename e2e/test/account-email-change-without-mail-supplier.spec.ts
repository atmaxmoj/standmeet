// account-email-change-without-mail-supplier.spec.ts —— what backstops an email change when there's no SMTP.
//
// The confirmation-email path (see account-email-change-needs-confirmation.spec.ts) has a precondition:
// a verified mail supplier must exist. When it doesn't, you can't just remove the gate —— that's the
...
test.describe('account · no mail supplier → double entry, and the consequence is stated', () => {
  test.beforeAll(async ({ playwright }) => {
    ...
    // Deliberately **do not** configure a mail supplier —— this spec tests exactly the no-SMTP half.
