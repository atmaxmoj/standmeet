// acl-block-matrix.spec.ts — §B.1 (block target) of block-acl-hierarchy-tests.md.
//
// e2e version of the pure AND-with-code-deny truth table, target = calendar.book (a
// role-granted block; GCal is connected to isolate the supplier gate). Exhausts
...
// calendar_book?).
//
// Red until: the code_block_denials table + the admin deny sub-route + applyCodeDenials
// wired into buildRoleSnapshotForCode. Until then setCodeBlockDenial gets a 404 -> A2/A4 red.
...
test.describe('ACL §B.1 · block deny matrix (calendar.book, GCal connected)', () => {
  let seed: BaseSeed;
