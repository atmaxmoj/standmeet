// acl-frozen-product.spec.ts —— §B.3 of block-acl-hierarchy-tests.md.
//
...
//   2. block_state — deny a cap a role granted → it is **completely absent** from
//      blocks[] (**not** enabled=false). "ACL = undiscoverable" is a different lock
//      from the supplier/quota/empty-corpus case of "visible but enabled=false degraded"
//      (see the empty-corpus row in retrieval-block-state).
//
...
function capIDs(v: VisitorSession): string[] {
  if (!v.blocks) throw new Error('response missing blocks[]');
  return v.blocks.map((c) => c.id);
}
...
test.describe('ACL §B.3 · code-deny removes the whole frozen product (not just tool name)', () => {
  ...
  test('acl-code-deny-drops-prompt-fragment · deny corpus.retrieval → its fragment leaves part_ids', async () => {
    ...
    expect(await setCodeBlockDenial(seed.request, seed.csrf, denied.id, 'corpus.retrieval')).toBe(201);
  ...
  test('acl-code-deny-cap-absent-from-states · denied cap is ABSENT, not enabled=false', async () => {
    ...
    expect(await setCodeBlockDenial(seed.request, seed.csrf, code.id, 'calendar.book')).toBe(201);
    ...
    // supplier/quota/empty-corpus which appear with enabled=false).
