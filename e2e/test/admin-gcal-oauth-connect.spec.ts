...
test.describe('admin · GCal OAuth · paste credentials → Authorize → Connected', () => {
  ...
  test('init → mock OAuth dance → status flips to connected', async () => {
    ...
    const status = await getGCalStatus(seed.request);
    // #162 externalized gcal as a generic supplier: the status is the uniform
    // supplier shape (connected/has_credentials), no gcal-specific calendar_id/
    // scopes. The meaningful outcome — the oauth dance completes and the supplier
