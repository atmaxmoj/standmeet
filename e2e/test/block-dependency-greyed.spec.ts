// block-dependency-greyed.spec.ts —— Phase H / P.6: the availability "supplier
// dependency" gate. Each row of the block panel surfaces the status of the supplier
...
test.describe('Phase H · block supplier-dependency status (P.6)', () => {
  ...
  test('calendar.book row: dependency Google Calendar, not connected → connected after OAuth',
    async () => {
      ...
      const before = await findBlock(request, csrf, BOOKING_ID);
      ...
      const after = await findBlock(request, csrf, BOOKING_ID);
