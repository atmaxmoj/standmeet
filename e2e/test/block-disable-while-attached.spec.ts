// block-disable-while-attached.spec.ts —— Phase H / P.6 corner: the owner_enabled gate
// **takes priority over** the role_acl gate. When the owner turns off a block that is
...
test.describe('Phase H · owner-disable beats ACL grant (P.6)', () => {
  ...
  test.beforeAll(async ({ playwright }) => {
    // role grants calendar.book AND GCal is connected → both ACL + supplier
  ...
  test('disable calendar.book while a role still grants it → gone from the session',
    async () => {
      ...
      // baseline: ACL granted + supplier connected → calendar_book exposed.
      expect(await sessionToolNames(request, sess.session_token))
        .toContain(BOOKING_TOOL);

      // owner disables the block globally (even though the role still grants it).
      expect(await setBlockEnabled(request, seed.csrf, BOOKING_ID, false)).toBe(200);
      ...
      expect(await setBlockEnabled(request, seed.csrf, BOOKING_ID, true)).toBe(200);
      expect(await sessionToolNames(request, sess.session_token))
        .toContain(BOOKING_TOOL);
