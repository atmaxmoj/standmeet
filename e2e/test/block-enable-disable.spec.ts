// block-enable-disable.spec.ts — Phase H / P.6+P.7: availability is the owner-plane
// gate. Owner disables a block in the "blocks" panel -> its tool immediately
// disappears from visitor sessions; **builtin blocks can be disabled too**
// (P.7: builtin can be disabled but not deleted). Re-enable -> it comes back.
//
// exposed = exists(origin) ∧ owner_enabled ∧ supplier_deps_met ∧ role_acl ∧ quota
// This spec locks down the owner_enabled gate, treating builtin the same as any other
// block (corpus.retrieval is builtin, and any visitor with corpus grant can see
// corpus_search).
...
import {
  findBlock, setBlockEnabled, sessionToolNames,
...
// corpus.retrieval is a builtin block, exposing corpus_search / corpus_read to any
...
test.describe('Phase H · block enable/disable (owner-plane availability gate)', () => {
  ...
  test('builtin corpus.retrieval: listed as builtin + enabled + NOT deletable',
    async () => {
      const request = admin;
      const cap = await findBlock(request, csrf, RETRIEVAL_ID);
  ...
  test('disable a builtin → its tool disappears from a visitor session; re-enable → back',
    async () => {
      ...
      expect(await setBlockEnabled(request, csrf, RETRIEVAL_ID, false)).toBe(200);
      expect(await sessionToolNames(request, sess.session_token), 'disabled → gone')
        .not.toContain(RETRIEVAL_TOOL);

      // re-enable → comes back.
      expect(await setBlockEnabled(request, csrf, RETRIEVAL_ID, true)).toBe(200);
