// block-list-by-origin.spec.ts -- Phase H / P.5: ListByOrigin (migration count).
// Built-in blocks all carry origin=builtin; the panel can filter by origin. This test
...
// Built-in blocks currently shipped with the product (this set shrinks as they migrate out).
...
test.describe('Phase H · ListByOrigin (P.5 migration counter)', () => {
  ...
  test('known builtin blocks all report origin=builtin',
    async () => {
      const request = admin;
