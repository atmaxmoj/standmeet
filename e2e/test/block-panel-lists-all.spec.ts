// block-panel-lists-all.spec.ts -- Phase H / P.5: the block panel lists **all**
// blocks + suppliers + skills, each row surfacing an origin badge (builtin/managed/
...
test.describe('Phase H · block panel lists all kinds with origin badge (P.5)', () => {
  ...
  test('panel includes a builtin block, a supplier, and an owner skill — each with origin',
    async () => {
      const request = admin;
      const rows = await listBlocks(request, csrf);

      // builtin block (corpus.retrieval ships with the product).
      const builtin = rows.find((c) => c.id === 'corpus.retrieval');
      expect(builtin, 'builtin block listed').toBeDefined();
      expect(builtin?.kind).toBe('block');
      expect(builtin?.origin).toBe('builtin');

      // a supplier row (google calendar) — kind supplier.
      const supplier = rows.find((c) => c.kind === 'supplier');
      expect(supplier, 'a supplier listed').toBeDefined();
