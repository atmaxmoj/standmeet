// block-panel-names-skills.spec.ts —— every row in the blocks panel must state
// **what it is**.
//
// For an owner-authored skill, the block id is a UUID. The panel renders `row.id`, so
// built-in blocks look fine (their id is already human words like `mail.send`), but an
...
// up.
//
// The existing block-panel-lists-all test goes through HTTP (listBlocks) and
...
test.describe('blocks · every row says what it is', () => {
  ...
  test('an owner skill shows its name, and no row wears a bare UUID', async ({ adminPage }) => {
    ...
    const labels = await panel.locator('[data-testid^="block-row-"]').allInnerTexts();
    expect(labels.length, 'the panel actually rendered rows').toBeGreaterThan(0);
    expect(
      labels.filter((text) => UUID_LABEL.test(text)),
      'no block row may be labelled with a bare UUID',
    ).toHaveLength(0);
  });
});
