...
// 依赖 smtp 的能力(can_email_codes)隐藏 → 重跑连接测试才恢复。
//
...
// clears connected_at) → the smtp-dependent capability (can_email_codes on
...
test.describe('connector · mail rotate SMTP identity → re-verify (§三 D-5)', () => {
  test.beforeAll(async ({ playwright }) => {
    ...
    // save creds → connect(test) → activate → connected = can_email_codes true.
    await configureMailConnector(request, OWNER.email, OWNER.password);
    await request.dispose();
  });

  test('connected → change from_address → connected resets, can_email_codes false; re-connect restores',
    async ({ playwright }) => {
      const request = await playwright.request.newContext();
