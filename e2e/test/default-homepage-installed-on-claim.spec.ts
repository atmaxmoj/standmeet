...
// homepage as the reserved `home` microsite (a draft, pre-loaded with the design-system
...
// real, editable microsite — the owner publishes it (build + promote) and it serves at `/`
...
test.describe('claiming an instance installs the default homepage as a draft', () => {
  test('the reserved `home` microsite is pre-loaded, ready to publish', async ({ request }) => {
    resetInstance();
    ...
    expect(res.ok(), `list microsites should be 200, got ${res.status()}`).toBeTruthy();
    const text = await res.text();
    expect(text, 'a `home` microsite must be installed at claim').toMatch(/"slug"\s*:\s*"home"/);
  });
});
