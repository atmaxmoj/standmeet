// embed-widget-carries-code-blocks.spec.ts -- a widget session gets **the whole
...
// dispatchIssueSession as redeeming a code at /gate -- so the corpus ACL + blocks
// determined by the code's role should freeze into the widget session unchanged. But
// "should" isn't evidence ([[test-covers-block-not-face]]: an API-driven e2e can
// pass green even when the block was never wired up at all). This test proves it.
//
...
//     entry that only this code's role can reach, and its block list is non-empty --
//     both corpus reach and blocks followed the code.
...
test.describe('embed · a widget session carries the code\'s corpus reach and blocks', () => {
  ...
  // -- Positive control: the widget session genuinely reaches the CV, and blocks
  // followed the code --
  test('a widget session from the allowed origin reaches the code\'s corpus',
    async ({ request }) => {
      ...
      // be extracted ("reach" = both retrieval block + corpus ACL followed the code
  ...
  test('the widget session is handed the code\'s blocks, not a stripped-down set',
    async ({ request }) => {
      const sess = await issueWidgetSession(request, WIDGET_CODE, ALLOWED);
      // A non-empty block list = the block set the code's role grants got
      // frozen into the session. The widget only adds one extra origin gate; it must not
      // strip out the blocks the code already carries
      // ([[retrieval-vs-corpus-acl]]: corpus.retrieval is a block).
      expect(sess.blocks?.length ?? 0,
        'widget 会话的 block 列表不该是空的 —— 码带什么，widget 就带什么').toBeGreaterThan(0);
    });
