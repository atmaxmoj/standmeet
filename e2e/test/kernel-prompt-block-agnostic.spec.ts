// kernel-prompt-block-agnostic.spec.ts — a block the visitor was not granted
...
// based on it, for a block that doesn't exist at all.
//
...
// show both — go back to whichever block actually handles scheduling, said in its
...
// Words specific to a concrete block. This visitor has no scheduling tool at all,
// so none of these should appear in their system prompt.
const BLOCK_WORDS = /calendar|schedul|book a meeting|appointment/i;

test.describe('the always-on part of the system prompt names no block', () => {
  ...
  test('a visitor granted only corpus carries no scheduling instruction', async ({ request }) => {
    ...
    ).not.toMatch(BLOCK_WORDS);
  });
});

// grantCorpusOnlyCode — a role + code that grants corpus access only, no blocks.
async function grantCorpusOnlyCode(request: APIRequestContext): Promise<void> {
  const { csrf } = await login(request, OWNER.email, OWNER.password);
  const role = await createRole(request, csrf, {
    name: 'reader-only',
    description: 'corpus only, no blocks',
    corpus_uris: ['wiki://**'],
  });
