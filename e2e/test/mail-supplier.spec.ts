// mail-supplier.spec.ts —— the access-code email loop, end to end, real SMTP.
//
// User story: owner configures + verifies the mail supplier (Mailpit) → a visitor
...
import { configureMailSupplier, clearMailpit, waitForMailTo } from '@/fixtures/mail';
...
test.describe('mail supplier access-code loop', () => {
  ...
  // Runs last: disconnect removes the supplier, so can_deliver_codes flips back
  ...
  test('disconnect → can_deliver_codes false → gate hides request-access',
    async ({ playwright, page }) => {
      const request = await playwright.request.newContext();
      const { csrf } = await login(request, OWNER.email, OWNER.password);
      // eslint-disable-next-line e2e-local/no-direct-mutating-api -- action under test: disconnect flips can_deliver_codes; needs helper: suppliers
      const dis = await request.post(`${BACKEND}/api/admin/suppliers/smtp/disconnect`, {
        headers: { 'X-Csrftoken': csrf }, data: {},
      });
