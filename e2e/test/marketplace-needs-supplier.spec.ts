// marketplace-needs-supplier.spec.ts —— a marketplace card's "needs X supplier" hint must track
// **real** supplier state, not a hardcoded fake connection status, nor a field the server never sends.
//
// The truth of this hint is **derived**, through a three-hop chain, all of it on the server:
//   the skill's `allowed-tools` (SKILL.md) → the block that provides those tools (manifest's `visitor_tools`)
//   → the supplier that block needs (manifest's `requires`) → whether the owner has connected it.
// The middle hop used to have no home at all (a sandboxed block's visitor tool
...
// ([[test-covers-block-not-face]]). Now it goes through the real pipeline: the
// mock marketplace catalog's `tz-booking` entry declares `allowed-tools: [calendar_book]`,
// while the other skills declare `corpus_search` (no supplier dependency).
//
// Two criteria, both required:
//   ① On an instance with zero suppliers, the tz-booking card says `needs calendar`;
...
// NEEDS_CAL —— the one skill in the mock marketplace catalog that needs a supplier (mock-stack/job-board/marketplace.go).
const NEEDS_CAL = 'tz-booking';
// NEEDS_NOTHING —— an entry from the real fetched catalog. Its allowed-tools is
// corpus_search, no supplier dependency, so its needs is **empty** (fully answerable,
...
test.describe('marketplace · the "needs X supplier" hint tracks real supplier state', () => {
  let seed: BaseSeed | undefined;

  test.beforeAll(async ({ playwright }) => { seed = await seedOwnerLoggedIn(playwright); });
  test.afterAll(async () => { await teardownSeed(seed); });

  test('no suppliers ⇒ the calendar-needing skill warns, its neighbour does not; connect ⇒ it clears',
    async ({ adminPage }) => {
      await openMarketplace(adminPage);

      // ① Zero suppliers: the calendar-needing card says so.
