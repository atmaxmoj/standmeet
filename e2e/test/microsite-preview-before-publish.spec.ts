// microsite-preview-before-publish.spec.ts — the owner can SEE the page before it
...
test.describe('custom pages · preview before publish', () => {
  ...
  test('the editor is fillable and both actions gate on the slug', async ({ adminPage: page }) => {
    ...
    await expect(page.getByTestId('microsite-source').locator('.cm-content')).toContainText(MARKER);
  });
  ...
  test('build preview renders inline but does not go live; publish does', async ({ adminPage: page }) => {
    ...
    await expect(
      page.frameLocator('[data-testid="microsite-staging-frame"]').getByRole('heading', { name: MARKER }),
    ...
    const beforePublish = await page.request.get(`/api/v1/microsites/${SLUG}`);
    ...
    await expect.poll(async () => {
      const after = await page.request.get(`/api/v1/microsites/${SLUG}`);
      return after.status();
