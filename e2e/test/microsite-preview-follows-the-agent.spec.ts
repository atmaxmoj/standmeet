// microsite-preview-follows-the-agent.spec.ts — the owner can see it while directing
...
//
// Today `/admin/microsites` is **a table** — slug, which codes are bound, whether
...
test.describe('custom pages · the panel shows what the agent just built, without a reload', () => {
  ...
  test.beforeAll(async ({ playwright }) => {
    ...
    await callTool(request, token, agent.sid, 'microsite.create', {
