// page-store-isolation.spec.ts — the per-microsite persistence store's security invariants.
//
...
async function openStore(
  ...
  await callTool(request, token, sid, 'microsite.set_store_writable', {
...
test.describe('per-page store · isolation + write gate + no-leak', () => {
  ...
  test.beforeAll(async ({ playwright }) => {
    ...
    await callTool(request, token, sid, 'microsite.create', { slug: 'store-a', title: 'A' });
    await callTool(request, token, sid, 'microsite.create', { slug: 'store-b', title: 'B' });
  ...
  test('deleting a page drops its data with the schema (its store becomes gone)',
    async ({ request }) => {
      await callTool(request, token, sid, 'microsite.create', { slug: 'store-gone', title: 'G' });
      ...
      await callTool(request, token, sid, 'microsite.delete', { slug: 'store-gone' });
