// norm-inward-blocks.spec.ts —— the 【inward】 block golden snapshot
// (normalization safety net).
//
// Inward blocks = blocks loaded into the agent for **visitor AI** use, i.e.
// **visitor-facing**: shape is visitor_only or both. The criterion matches the backend's one
// and only shape gate (capreg.VisitorBlockIDs: Shape() != owner_only) — a block
...
// "block normalization" migrates: externalizing these builtins from in-process Go into
...
const GOLDEN_INWARD: readonly Cap[] = [
  // skill.runner + ext.mcp are loaders/mechanisms (not leaf blocks), left in capreg's
  // builtin registration slot.
  { id: 'skill.runner', shape: 'visitor_only', origin: 'builtin' },
  { id: 'ext.mcp', shape: 'visitor_only', origin: 'builtin' },
  // supplier.agent_tools — once an openapi supplier turns on expose_as_agent_tools, its
  ...
  // SessionGate). Landed as part of the supplier refactor #155 §3, registered after
  // ext.mcp and before the leaf blocks.
  { id: 'supplier.agent_tools', shape: 'visitor_only', origin: 'builtin' },
  ...
  // blocks are externalized into sandboxed plugins (mcp-servers/*), loaded through
  // registerBuiltins via the unified sandbox_stdio path with origin=builtin (after the
  // capreg loader, before any managed third party). No block-specific MCP code remains
  // in the main app. id/shape/origin unchanged, only the loading mechanism changed.
  // calendar.book additionally layers a SessionGate (supplier+quota) for per-session hiding.
  //
  // **These five are ordered by id**, because once the declaration moved into
  // backend/blocks/<id>/manifest.yaml, load order became directory order, and
  ...
  // mail.send — the send operation exposed by the built-in SMTP supplier's
...
test.describe('能力归一化 · 【对内】agent 能力黄金快照', () => {
  ...
  test('多次拉取顺序稳定(prompt hash 依赖注册顺序)',
    async ({ playwright }) => {
      const request = await playwright.request.newContext();
      const a = (await fetchRegistry(request)).blocks.map((c) => c.id);
      const b = (await fetchRegistry(request)).blocks.map((c) => c.id);
      expect(b).toEqual(a);
      await request.dispose();
    });
});
