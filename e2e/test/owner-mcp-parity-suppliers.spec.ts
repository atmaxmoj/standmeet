// owner-mcp-parity-suppliers.spec.ts -- [outward] functional guard for the
// owner-MCP **supplier + external-dependency** tools added once
// facade-parity debt was paid off. Supplier CRUD does a real roundtrip with
// SAMPLE_SPEC; mail_test_send returns {ok:false} by design when no mail
// supplier is configured; access_requests seeds a real request then does an
...
// gracefully.
//
// Coverage: suppliers.{validate_spec,create,status,update,activate,disconnect,delete,
...
async function checkSupplierCRUD(r: APIRequestContext): Promise<void> {
  const verdict = await callTool<{ ok: boolean; title: string }>(
    r, token, sid, 'suppliers.validate_spec', { spec: specStr });
  expect(verdict.ok, 'valid spec validates ok').toBe(true);

  const created = await callTool<{ id: string }>(r, token, sid, 'suppliers.create', {
  ...
  const status = await callTool<{ id: string; kind: string }>(
    r, token, sid, 'suppliers.status', { id: created.id });
  expect(status.id, 'status returns the supplier').toBe(created.id);

  const updated = await callTool<{ id: string; ok: boolean }>(r, token, sid, 'suppliers.update', {
    id: created.id, spec: specStr, binding: bindingStr,
  });
  expect(updated.ok, 'update ok').toBe(true);

  const disc = await callTool<{ ok: boolean }>(
    r, token, sid, 'suppliers.disconnect', { id: created.id });
  expect(disc.ok, 'disconnect ok').toBe(true);

  const del = await callTool<{ ok: boolean }>(
    r, token, sid, 'suppliers.delete', { id: created.id });
  expect(del.ok, 'delete ok').toBe(true);
}

async function checkSupplierActivateAndMail(r: APIRequestContext): Promise<void> {
  const created = await callTool<{ id: string }>(r, token, sid, 'suppliers.create', {
    kind: 'openapi', spec: specStr, binding: bindingStr,
  });
  // activate an un-credentialed supplier: dispatches, then either flips or reports a
  // friendly failure (no oauth creds). Either way is a valid binding outcome, never a crash.
  const act = await callOrErr(r, 'suppliers.activate', { id: created.id });
  if (!act.ok) assertFriendly(act.err, 'suppliers.activate');
  await callTool(r, token, sid, 'suppliers.delete', { id: created.id });

  // no mail supplier is configured → the tool reports {ok:false} by design (not an error).
  const mail = await callTool<{ ok: boolean }>(r, token, sid, 'suppliers.mail_test_send', {
    to: 'nobody@example.com', subject: 'ping', text: 'hi',
  });
  expect(mail.ok, 'mail_test_send reports ok:false without a mail supplier').toBe(false);
}
...
async function checkAccessRequests(r: APIRequestContext): Promise<void> {
  ...
  // approve issues a code AND mails it; with no mail supplier configured it must refuse with a
  ...
  expect(approve.ok, 'approve blocked without a mail supplier').toBe(false);
  if (!approve.ok) {
    assertFriendly(approve.err, 'access_requests.approve');
    expect(approve.err.toLowerCase(), 'friendly reason names the mail supplier')
      .toContain('mail supplier');
  }
}
...
test.describe('facade-parity · 新增 owner-MCP 连接器/外部工具守护', () => {
  ...
  test('suppliers validate→create→status→update→disconnect→delete roundtrip',
    ({ playwright }) => run(playwright, checkSupplierCRUD));
  test('suppliers.activate dispatches + mail_test_send reports ok:false',
    ({ playwright }) => run(playwright, checkSupplierActivateAndMail));
