// retrieval-block-state.spec.ts -- Phase B-2 contract: the
// corpus.retrieval block appears on the visitor-blocks endpoint,
...
//   /visitor-blocks must return:
//     - blocks[?].id=='corpus.retrieval' enabled=true
...
//     - blocks[?].id=='corpus.retrieval' enabled=false
...
// this spec only verifies the block-map contract introduced by B-2.
...
test.describe('Phase B-2 RetrievalBlock state contract', () => {
  ...
  test('role with corpus URIs → corpus.retrieval enabled + 3 tool specs',
    async ({ playwright }) => {
      ...
      const corpusCap = body.blocks.find((c) => c.id === 'corpus.retrieval');
  ...
  test('system_prompt_hash differs between full vs empty corpus roles',
    async ({ playwright }) => {
      ...
      // differing block state should still make the hashes diverge.
      const request = await playwright.request.newContext();
      ...
      const full = await fetchVisitorBlocks(request, sessFull.session_token);
      const empty = await fetchVisitorBlocks(request, sessEmpty.session_token);
