// session-block-bundle.spec.ts —— POST /api/v1/sessions responds in one shot with
// (a) the list of blocks enabled for this session + (b) the array of fragment ids needed to
// assemble the system prompt. The frontend GETs /api/v1/prompts/{id} per fragment id and composes the
// text locally.
//
// Invariants:
//   - blocks is the same shape as the blocks field of /internal/diag/session
//     (both computed in one place via Registry.VisitorStates)
//   - system_prompt_part_ids[0] is always 'visitor-header'
//   - when a block's fragment enters the actual system prompt, its fragment id
//     must appear in system_prompt_part_ids (and vice versa)
//
// Drift detection: if anyone later adds a new block with a fragment but does not expose its id, the
...
function requireCaps(sess: VisitorSession): SessionBlock[] {
  if (!sess.blocks) throw new Error('response missing blocks[]');
  return sess.blocks;
}
...
test.describe('session block bundle · POST /sessions response shape', () => {
  test.beforeAll(async ({ playwright }) => {
    await setupBundleOwner(playwright);
  });

  test('full corpus role → blocks includes corpus.retrieval enabled + part_ids contains its fragment id',
    async ({ playwright }) => {
      const request = await playwright.request.newContext();
      ...
      expect(partIDs).toContain('blocks/corpus.retrieval');
      await request.dispose();
    });
  ...
  test('empty corpus role → corpus.retrieval visible disabled + part_ids omits its fragment id',
    async ({ playwright }) => {
      ...
      expect(partIDs[0]).toBe('visitor-header');
      expect(partIDs).not.toContain('blocks/corpus.retrieval');
      await request.dispose();
    });

  test('public mode session → returns part_ids (visitor-header first) + blocks array',
    async ({ playwright }) => {
      const request = await playwright.request.newContext();
      const sess = await issueSession(
  ...
  test('session.blocks map matches /internal/diag/session (same Registry source)',
    async ({ playwright }) => {
      const request = await playwright.request.newContext();
      const sess = await issueSession(
        request, { handle: OWNER.handle, code: CODE_FULL, visitor_name: 'V' },
      ...
      expect(sortCaps(requireCaps(sess))).toEqual(sortCaps(dev.blocks));
      await request.dispose();
    });
...
function sortCaps(caps: SessionBlock[]): SessionBlock[] {
