...
// grant out a supplier's operation.
//
...
// **owner-MCP's skill_create**. The GUI had none -- and for a supplier the
...
//   1. `GET /suppliers/agent-ops` -- the owner needs to be able to **know
...
import {
  AGENT_OWNER, MOCK_OAUTH2_SCHEME, createAndConnect, disconnectSupplier, initOwner,
...
async function agentOps(request: APIRequestContext): Promise<AgentOpsRow[]> {
  const res = await request.get(`${BACKEND}/api/admin/suppliers/agent-ops`);
  expect(res.status(), 'the owner can ask which operations are grantable').toBe(200);
  const body = await res.json() as { suppliers?: AgentOpsRow[] };
  return body.suppliers ?? [];
}

test.describe('skills · a supplier operation can be granted from the owner face (F-C-57)', () => {
  ...
  test('a connected supplier publishes the operation names a skill must carry', async () => {
    const id = await createAndConnect(
      request, csrf, { spec: VENDOR_SPEC, expose_as_agent_tools: true },
    );

    const rows = await agentOps(request);
    const mine = rows.find((r) => r.block_id === id);
    expect(mine, 'the connected supplier is listed').toBeDefined();
    // What's asserted is **this doc's own operation, after normalization** --
    // an empty list wouldn't pass, and neither would another supplier's name.
    ...
    expect(after.find((r) => r.block_id === id), 'a disconnected supplier is not offered')
      .toBeUndefined();
  });
