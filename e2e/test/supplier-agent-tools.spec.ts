// supplier-agent-tools.spec.ts — #155 §3's target contract (RED) for the second consumer
...
//
// Two consumer paths (supplier.md §3):
//   - Code consumer (booker/mailer/job-loop) -> normalized via a **seam contract**:
//     the supplier declares a seam + maps operations onto the contract
//     (CalendarContract/MailContract). Existing contract coverage lives in
//     supplier-binding-jsonata.spec.ts (section C).
...
//
// This file pins down the agent path: an openapi supplier (**without** a seam
...
//       on-by-default or opt-in. This file assumes a supplier carries an explicit
//       `expose_as_agent_tools` flag at creation time — which also answers "does a
//       category-bound-only supplier leak raw ops" (see [A5]). If the implementation
//       chose "expose by default for every openapi supplier", drop the flag from the
...
//       different prefix (e.g. `<supplier>__events_insert`) or keeps the dot, update
//       TOOL_NAME_FOR().
//
...
//       is "per-session grant/ACL is unified" (supplier.md §6's DepRegistry single
//       global gate + memory: retrieval-vs-corpus-acl). Assumes an op can only be exposed
//       if (a) the supplier is connected and (b) that session's role/skill grants this
//       agent-tool name. An ungranted op does not enter the tool set. If the
//       implementation chose supplier-level granularity instead of per-op, downgrade the
//       PARTIAL_GRANT case to "the whole supplier is granted or not".
//
//  [A5] **A category-only-bound supplier does not leak raw ops**. Design §3 lists the
//       two paths side by side without saying whether installing a category binding on a
//       supplier also auto-exposes its raw ops to the agent. This file assumes **no
//       auto-leak** — a category-bound supplier only exposes its normalized category cap
...
//       matching test case).
//
//  [A6] **The runtime entry point is diag `/api/admin/diag/supplier/{id}/agent-call`**,
...
//       supplier-binding's diag naming convention.
// ─────────────────────────────────────────────────────────────────────────────
...
import {
  ...
  disconnectSupplier, initOwner, sessionToolNames, sessionToolSpecs, startSession,
} from '@/fixtures/supplier-agent-rig';
...
// this is the agent path, the supplier declares no category, and the LLM picks by summary.
const AGENT_SPEC = {
  openapi: '3.0.3',
...
// supplier must not leak raw ops) ───
// A calendar spec + binding isomorphic to supplier-binding-jsonata.spec.ts's, but
...
// raw op names the calendar supplier would expose IF it leaked them (must NOT appear).
...
// fixtures/supplier-agent-rig.ts. It was pulled out because this file hit the 350-line
// gate, and the same rig now has a second user (supplier-agent-tool-names.spec.ts).

test.describe('supplier · agent-tool exposure (§3 second consumer path: agent = semantic read ops)', () => {
  ...
  // happy: a connected openapi supplier ([A1] expose_as_agent_tools) -> its operations
  // enter the session tool set, with names [A2] / descriptions [A3] coming from the
  // spec's operation summaries.
  test('connected openapi supplier exposes its operations as agent tools (names + descriptions from spec summaries)',
    async () => {
  ...
  test('agent tools are distinct from category-contract caps (raw ops, not a normalized category cap)',
    async () => {
      ...
      expect(names, 'no normalized category cap leaked from an agent-only supplier')
...
test.describe('supplier · agent-tool exposure · ACL/gating (§3 + §6 single global gate)', () => {
  // The same grant/ACL gate governs agent tools: per-op grant, a category-only supplier
  ...
  // [A5] ⚠️ most needs confirming: a category-only-bound supplier (no
  ...
  test('a category-only supplier does NOT leak raw ops as agent tools — only the category cap [ASSUMPTION A5]',
    async () => {
      // No expose_as_agent_tools — a category-only-bound supplier.
      ...
      for (const raw of CALENDAR_RAW_OP_TOOLS) {
        expect(names, `raw op ${raw} NOT leaked from a category-only supplier`).not.toContain(raw);
      }
    });

  // ── err: supplier disconnects -> its agent tools disappear (gated the same as any
  // other cap) ──
  // connected -> op_* exposed; disconnect -> all of them disappear via the single global
  // gate (dependency.connected=false).
  test('a disconnected supplier — its agent tools disappear (gated like everything else)',
    async () => {
      const id = await createAndConnect(request, csrf, { spec: AGENT_SPEC, expose_as_agent_tools: true });
      ...
      await disconnectSupplier(request, csrf, id);
