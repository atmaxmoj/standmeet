// supplier-binding-jsonata.spec.ts -- #155 §8 area C (declarative JSONata bindings). The
...
import {
  ...
  EXTRA_OP_BINDING, BROKEN_JSONATA_BINDING, GHOST_OP_BINDING, UNKNOWN_SEAM_BINDING,
  INCOMPLETE_BINDING,
} from '@/fixtures/supplier-jsonata';
...
// POST /api/admin/suppliers -- builds a supplier from spec+binding. 201 -> {id}; 4xx ->
// {error}.
...
async function createSupplier(
  ...
  // eslint-disable-next-line e2e-local/no-direct-mutating-api -- action under test: bespoke supplier build the test inspects (status/id/error, both 201 and 4xx cases)
  const res = await request.post(`${BACKEND}/api/admin/suppliers`, { headers: { 'X-Csrftoken': csrf }, data: body });
...
// diag: runs the list_busy contract op for this supplier, returns the normalized
...
// createOK -- POSTs a binding, asserts 201, returns the supplier id. Shared by the
...
async function createOK(request: APIRequestContext, csrf: string, binding: unknown): Promise<string> {
  const r = await createSupplier(request, csrf, { spec: SAMPLE_SPEC, binding });
...
// dance to connect the supplier, then issues a calendar.book code + starts a visitor
...
// no supplier was created. Shared by the four assemble-time rejection reasons (invalid
...
test.describe('supplier binding · JSONata binding (§8 area C)', () => {
  // #155 §8-C has landed: the declarative JSONata binding subsystem (POST
  // /api/admin/suppliers accepts spec+binding, assemble-time validation, runtime
  ...
  // -- happy: category "calendar" -> supplier fills the calendar dep slot ->
  ...
  // -- err: an invalid JSONata expression -> rejected at assemble time, supplier not
  // created --
  test('invalid JSONata expression is rejected at assemble time (supplier not created)',
    async () => {
  ...
  test('binding with an unknown category is rejected',
    async () => {
      await expectRejected(request, csrf, UNKNOWN_SEAM_BINDING, /seam|telepathy|unknown/i);
    });
  ...
  test('binding maps an extra op the consumer does not need → tolerated (ignored), supplier still assembles',
    async () => {
      await assertExtraOpTolerated(request, csrf);
    });
});

// diagInvoke -- hits the owner-authed supplier diag endpoint. **This is a backdoor that
// bypasses the real chain** (the real path is visitor chat -> agent -> booker sandbox ->
// supplier.invoke), so it's **deliberately** kept inline here instead of extracted into
