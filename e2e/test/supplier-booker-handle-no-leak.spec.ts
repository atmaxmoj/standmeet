// supplier-booker-handle-no-leak.spec.ts —— the new leak surface opened up once
// booking was externalized (supplier-deps-tests.md §1 booker-handle-no-leak). What the
...
// `supplier-secret-no-leak` watches the visitor-side wire; `supplier-arbitrary-dep`
// watches synthetic supplier X. This spec watches the **real booker plugin** only:
...
// (the token never leaves the supplier layer).
...
test.describe('supplier · booker plugin gets a handle, never the owner credentials', () => {
  let seed: CodedSeed;
