// supplier-calendar-cancel-tool.spec.ts —— §1 (visitor calendar_cancel as a tool)
//
// today cancellation goes through REST (`/api/v1/booking-cancellation`, postBookingCancellation). After the refactor
// it becomes a supplier-backed **tool**: the cancel button on the booked sandbox card posts
// `{type:'mcp-ui:tool', name:'calendar_cancel', args}` → host dispatches the `calendar_cancel` tool with session context
// → the tool deletes the event via the calendar supplier proxy → returns
// `mcp-ui:tool-result` → the card enters the cancelled state. This spec guards the **TOOL path** (not the old REST):
//   - the deletion really happens (that event is gone from getMockEvents = the tool deleted it via the supplier proxy)
...
test.describe('supplier · visitor calendar_cancel as a tool (§1)', () => {
  let seed: CodedSeed;
  ...
  test('cancel inside the iframe → calendar_cancel tool removes the event + card shows cancelled',
    async ({ browser }) => {
      ...
      // observable side effect of the tool path: the event is deleted via the supplier proxy. Dispatch is async (card → host →
