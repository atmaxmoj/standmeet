// supplier-connect-before-form.spec.ts — **Connect must not be pressable while the
// card still doesn't know which kind of connection it is.**
//
// ①🔴 Full-suite finding #436 goes red right here (supplier-happy-matrix, openapi
...
// path** (the fallback copy for `runNonDanceConnect`). An oauth2 supplier had gone
// through the bearer/apiKey branch instead.
//
// ②🎯 `useSupplierCard.connect()` branches on `authType`, and `authType` only gets a
...
import { openSupplierCard, fillOAuth2Creds, expectConnected } from '@/fixtures/supplier-card';
...
// The built-in oauth2 supplier (calendar kind, runs the dance). oauth2 is chosen
// because it's **the only one** where taking the wrong branch turns into a visible
// error outcome: the non-dance branch can never connect an oauth2 supplier.
const OAUTH2_SUPPLIER_ID = 'google-calendar';

test.use({ ownerCredentials: { email: OWNER.email, password: OWNER.password } });

test.describe('supplier card · the frame before its form arrives', () => {
  ...
  test('Connect is dead while credential-form is in flight; oauth2 then dances',
    async ({ adminPage: page }) => {
      ...
      const connect = card.getByTestId('supplier-connect-button');
