// supplier-connect-flow.spec.ts — the RED contract for #155 §8 area D (the connect flow).
//
// Target behavior (docs/design/supplier.md §4 post-submit + §5.2 assembly flow +
// §8 area D): the owner connects, in the admin UI, a supplier that **already has a
...
//                      the token → supplier-status = Connected.
//   non-dance (key/basic/bearer):  fill the secret → click Connect → immediately Connected,
//                      no redirect.
//
// Covers the spec-driven supplier connect flow (§8 area D). Implemented, really compiles,
...
// /api/admin/suppliers/google-calendar/{init,...} into
// /api/admin/suppliers/{id}/{connect,status,disconnect}. Area D's red tests are written
...
import {
  ...
  openSupplierCard, resetMockOAuthRecord, selectScope,
} from '@/fixtures/supplier-card';
...
// oauth2 supplier: the hand-rolled gcal implementation is now just "a built-in openapi
// binding", with its id still google-calendar (calendar category, oauth2 securityScheme,
// runs the dance).
const OAUTH2_SUPPLIER_ID = 'google-calendar';
// non-dance supplier: a bearer/apiKey-authed supplier (no OAuth dance) — storing the
// secret is all it takes to connect.
const NONDANCE_SUPPLIER_ID = 'bearer-api';
...
test.describe('supplier · connect flow happy (§8 area D)', () => {
  test.beforeAll(async ({ playwright }) => { await initOwner(playwright); });

  test('oauth2: fill client_id/secret → Connect → mock authorize → callback → token stored → Connected',
    async ({ adminPage: page }) => {
      const card = await openSupplierCard(page, OAUTH2_SUPPLIER_ID);
      ...
      // callback returns to the suppliers section.
      await card.getByTestId('supplier-connect-button').click();
      await page.waitForURL('**/admin/suppliers**');

      await expectConnected(card);
      const status = await getSupplierStatus(page, OAUTH2_SUPPLIER_ID);
      expect(status.connected).toBe(true);
      expect(status.has_credentials).toBe(true);
    });

  test('non-dance: bearer supplier fills token → Connect → immediately Connected, no redirect',
    async ({ adminPage: page }) => {
      const card = await openSupplierCard(page, NONDANCE_SUPPLIER_ID);
      // bearer: a single token field, a secret. No redirect_uri, no dance.
      await expect(card.getByTestId('supplier-redirect-uri')).toHaveCount(0);
      await card.getByTestId('supplier-field-token').fill('static-bearer-token');
      await expectNotConnected(card);
      ...
      const status = await getSupplierStatus(page, NONDANCE_SUPPLIER_ID);
      expect(status.connected).toBe(true);
    });

  test('oauth2: per-supplier redirect_uri shown readonly before connecting',
    async ({ adminPage: page }) => {
      const card = await openSupplierCard(page, OAUTH2_SUPPLIER_ID);
      // The owner needs this URI to register an OAuth client with the SaaS; it must be
      // visible and read-only before connecting.
      const redirect = card.getByTestId('supplier-redirect-uri');
      ...
      expect(new URL(value).pathname,
        'and it still points at this supplier\'s callback')
        .toBe(`/api/admin/suppliers/${OAUTH2_SUPPLIER_ID}/callback`);
    });
  ...
  test('oauth2: owner-selected scope subset carried verbatim into the authorize dance',
    async ({ adminPage: page }) => {
      ...
      const card = await openSupplierCard(page, OAUTH2_SUPPLIER_ID);
      // An earlier happy-path test may already have connected this supplier; the
...
test.describe('supplier · connect flow oauth2 errors (§8 area D)', () => {
  test.beforeAll(async ({ playwright }) => { await initOwner(playwright); });
  ...
  test('user denies on the consent page → not connected + friendly message',
    async ({ adminPage: page }) => {
      ...
      expect((await getSupplierStatus(page, OAUTH2_SUPPLIER_ID)).connected).toBe(false);
    });
  ...
  test('callback state/CSRF mismatch → rejected, not connected',
    async ({ adminPage: page }) => {
      ...
      await expectNotConnected(card);
      await expect(card.getByTestId('supplier-error')).toBeVisible();
      expect((await getSupplierStatus(page, OAUTH2_SUPPLIER_ID)).connected).toBe(false);
    });
...
test.describe('supplier · reconnect / rotate / disconnect (§8 area D)', () => {
  test.beforeAll(async ({ playwright }) => { await initOwner(playwright); });
  ...
  test('rotate client_id/secret after connecting → reconnects after re-verify',
    async ({ adminPage: page }) => {
      ...
      await expectConnected(card);
      expect((await getSupplierStatus(page, OAUTH2_SUPPLIER_ID)).connected).toBe(true);
    });
  ...
  test('click Disconnect after connecting → status flips to not-connected',
    async ({ adminPage: page }) => {
      ...
      await card.getByTestId('supplier-disconnect-button').click();
      await expectNotConnected(card);
      expect((await getSupplierStatus(page, OAUTH2_SUPPLIER_ID)).connected).toBe(false);
    });
  ...
  test('Disconnect keeps client_id/secret → one-click reconnect without re-entering credentials',
    async ({ adminPage: page }) => {
      ...
      const afterDisconnect = await getSupplierStatus(page, OAUTH2_SUPPLIER_ID);
...
// ════════ generic openapi oauth2 supplier: silent token refresh (§8 area D) ═══
// Generalizes chat-book-token-refresh's "expired access token → silent refresh →
// consume succeeds" to a **owner-uploaded** openapi oauth2 supplier (not the gcal
// built-in one). Connecting goes through the UI dance; consuming goes through §8's
// interface-sketch runtime direct-verify diag.
test.describe('supplier · generic oauth2 token silent refresh (§8 area D)', () => {
  test.beforeAll(async ({ playwright }) => { await initOwner(playwright); });

  test('uploaded oauth2 supplier: access token expires → backend silently refreshes → runtime call succeeds',
    async ({ adminPage: page, playwright }) => {
      ...
      // The owner connects a generic openapi oauth2 supplier (and gets its id) via
      ...
      // Directly edit the database to mark this supplier's access token as expired →
...
// return to the suppliers section. Returns the card Locator for the caller to assert
...
async function expectNotConnected(card: Locator): Promise<void> {
  await expect(card.getByTestId('supplier-status')).toHaveText(/not connected|未连接/i);
}
...
async function expectFriendlyError(card: Locator, forbidden: RegExp): Promise<void> {
  const err = card.getByTestId('supplier-error');
  await expect(err).toBeVisible();
  await expect(err).not.toContainText(forbidden);
}

// ─── supplier status (read-only GET; eslint permits GET) ──────────
...
async function getSupplierStatus(page: Page, id: string): Promise<SupplierStatus> {
  const res = await page.request.get(`${BACKEND}/api/admin/suppliers/${id}/status`);
  if (res.status() !== 200) throw new Error(`supplier status ${id}: ${res.status()}`);
  return await res.json() as SupplierStatus;
}
...
// ─── generic uploaded oauth2 supplier: upload + dance + runtime-consume helpers ──
// runUploadedOAuth2Dance — upload a spec via the UI (supplier-spec-input/submit),
// deriving an oauth2 form → fill credentials → Connect → dance → back to the
// suppliers section. Returns the card Locator.
// assembleUploadedOAuth2 — uploads a generic openapi oauth2 calendar supplier through
...
// navigates away and back on a full page; after returning, polls GET /suppliers to
// get the id of the now-connected supplier.
async function assembleUploadedOAuth2(page: Page, request: APIRequestContext): Promise<string> {
  ...
  await page.getByTestId('supplier-card-calendar').click();
  ...
  await page.getByTestId('supplier-spec-input').fill(JSON.stringify(UPLOADED_OAUTH2_SPEC));
  await page.getByTestId('supplier-binding-input').fill(JSON.stringify(UPLOADED_OAUTH2_BINDING));
  ...
  // SupplierCard after** assembly (which renders even for a single scheme); now the
  // scheme is an input to creating the supplier, and with only one option it's
  // already determined — nothing left to pick.
  await page.getByTestId('supplier-field-client_id').fill('mock-client-id');
  await page.getByTestId('supplier-field-client_secret').fill('mock-client-secret');
  // assemble = create the supplier + store the credentials just filled in; the
  // ingestion form then steps aside for this supplier's card, and Connect happens on
  // the card.
...
// pollConnectedCalendarId — after the dance returns, polls GET /suppliers until the
// calendar category has a connected supplier, returning its id (waitForURL can fire
// early because it's "already in the suppliers section", so poll until it's actually
// in the database).
...
// expireUploadedAccessToken — marks this supplier's access token as expired (the same
// direct-DB-edit trick as chat-book-token-refresh), matched by supplier id, forcing
// the next runtime consumption onto the refresh path. Table block_connections, column
// token_expires_at.
function expireUploadedAccessToken(id: string): void {
  const sql = `UPDATE block_connections
...
// (see fixtures/supplier-diag.ts); this only keeps the "return an HTTP status"
// signature.
...
// supplier: servers/token points at MOCK_API (hit by the backend), authorize points
...
// diagInvoke — hits the owner-authed supplier diag endpoint. **This is a backdoor
// that bypasses the real chain** (the real path is visitor chat → agent → booker
// sandbox → supplier.invoke), so it is **deliberately** kept inline here instead of
