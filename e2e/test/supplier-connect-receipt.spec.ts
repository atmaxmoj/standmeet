// supplier-connect-receipt.spec.ts —— when Connect says "connected", it must really have written that record.
//
// when `POST /suppliers/{id}/connect` takes the non-dance path (bearer/apikey/basic), the backend calls
// `MarkSupplierConnected`, which is a bare UPDATE:
//
//     UPDATE block_connections SET connected_at = ... WHERE owner_id = $1 AND block_id = $2
//
...
// a non-dance built-in supplier: bearer auth, a single token field, store-is-connect —— it takes exactly that bare UPDATE.
const SUPPLIER_ID = 'bearer-api';
const CREDS_URL = `**/suppliers/${SUPPLIER_ID}/credentials`;

test.use({ ownerCredentials: { email: OWNER.email, password: OWNER.password } });

test.describe.configure({ mode: 'serial' });
test.describe('supplier · connect writes a receipt, not a claim', () => {
  ...
  test('Connect with nothing filled in never reports connected', async ({ adminPage: page }) => {
    const card = await openSupplierCard(page, SUPPLIER_ID);
    ...
    await expect.poll(
      async () => (await getSupplierStatus(page, SUPPLIER_ID)).connected,
      { message: 'connect must not mark a supplier connected when it stored nothing' },
    ).toBe(false);
    await expectNotConnected(card);
  ...
  test('Connect clicked while the credential save is still in flight still connects',
    async ({ adminPage: page }) => {
      ...
      await expect(card.getByTestId('supplier-status')).not.toHaveText(/^not connected$/i);
      gate.open();
  ...
  //
  // the backend **never returns** the credential values (supplier-security verified: the credential-form returns field names,
  ...
  test('a card with stored credentials says so, instead of showing empty boxes',
    async ({ adminPage: page }) => {
      ...
      const card = await openSupplierCard(page, SUPPLIER_ID);
      await expect(
        card.getByTestId('supplier-creds-stored'),
        'credentials are saved and the card must say so — empty boxes read as "nothing configured"',
      ).toBeVisible();

      // the reverse direction: on another supplier that was never configured, this line must be absent.
      const untouched = await openSupplierCard(page, 'smtp');
      await expect(
        untouched.getByTestId('supplier-creds-stored'),
        'a supplier with no credentials must not claim to have any',
      ).toHaveCount(0);
    });
});
...
async function openSupplierCard(page: Page, id: string): Promise<Locator> {
  await page.getByTestId('admin-nav-suppliers').click();
  await page.waitForURL('**/admin/suppliers**');
  const card = page.getByTestId(`supplier-row-${id}`);
...
async function expectConnected(card: Locator): Promise<void> {
  await expect(card.getByTestId('supplier-status')).toHaveText(/^connected$/i);
}

async function expectNotConnected(card: Locator): Promise<void> {
  await expect(card.getByTestId('supplier-status')).toHaveText(/^not connected$/i);
}
