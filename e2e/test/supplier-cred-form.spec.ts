// supplier-cred-form.spec.ts —— #155 area B (credential form derivation) RED contract.
//
// Business story: the owner pastes an OpenAPI 3.0 spec into /admin/suppliers → the backend
...
// supplier). This area only pins down "how the form derives from the spec and renders
// correctly"; the connect action itself (the OAuth dance / storing secrets) belongs to area D.
//
// Aligned with docs/design/supplier.md §4 (auth type → field table) + §7 decision #3
// (owner picks among multiple schemes in the UI) + §8 area B testids
// (supplier-spec-input / supplier-scheme-select / supplier-field-{key} /
// supplier-needs-dance / supplier-status). supplier-connect-button belongs to area D:
// connecting needs a supplier id, and this form runs **before** the supplier is assembled (F-C-24).
//
...
// NOT a field (Connect runs the dance); scope is a multi-select; per-supplier
...
test.describe('supplier · credential form derived from spec · happy (area B)', () => {
  // Covers spec-driven credential form derivation (docs/design/supplier.md §4/§8 area B). Implemented, green.
  ...
  test('oauth2 spec → client_id + client_secret + scope picker + readonly redirect_uri + Connect',
    async ({ adminPage: page }) => {
      ...
      await expect(page.getByTestId('supplier-field-client_id')).toBeVisible();
      await expect(page.getByTestId('supplier-field-client_secret')).toBeVisible();
      await expect(page.getByTestId('supplier-field-token')).toHaveCount(0);

      // scope is a multi-select drawn from the flow's scopes.
      const scope = page.getByTestId('supplier-field-scope');
      await expect(scope).toBeVisible();
      await expect(scope).toContainText(/cal\.read/);
      await expect(scope).toContainText(/cal\.write/);

      // per-supplier redirect_uri shown read-only (owner registers it upstream).
      const redirect = page.getByTestId('supplier-field-redirect_uri');
      ...
      // the supplier's id, which does not exist until it is assembled, so the button that used to
      // sit here had no handler and could never have one (F-C-24).
      await expect(page.getByTestId('supplier-needs-dance')).toBeVisible();
    });
  ...
  test('apiKey spec → single key field + shows where it goes (header + name)',
    async ({ adminPage: page }) => {
      ...
      const key = page.getByTestId('supplier-field-key');
      await expect(key).toBeVisible();
      // no oauth client creds for a bare apiKey scheme.
      await expect(page.getByTestId('supplier-field-client_id')).toHaveCount(0);

      // UI tells owner where the key is injected: header X-Api-Key.
      const form = page.getByTestId('supplier-cred-form');
      ...
      await expect(page.getByTestId('supplier-needs-dance')).toHaveCount(0);
    });
  ...
  test('http basic spec → username + password fields', async ({ adminPage: page }) => {
    ...
    await expect(page.getByTestId('supplier-field-username')).toBeVisible();
    const pass = page.getByTestId('supplier-field-password');
  ...
  test('http bearer spec → single token field', async ({ adminPage: page }) => {
    ...
    const token = page.getByTestId('supplier-field-token');
    await expect(token).toBeVisible();
    await expect(token).toHaveAttribute('type', 'password');
    // bearer ≠ basic: no username/password split.
    await expect(page.getByTestId('supplier-field-username')).toHaveCount(0);
    await expect(page.getByTestId('supplier-field-password')).toHaveCount(0);
  });
  ...
  test('multi-scheme spec → scheme picker; picking each renders that scheme fields',
    async ({ adminPage: page }) => {
      ...
      await expect(page.getByTestId('supplier-field-client_secret')).toBeVisible();
      await expect(page.getByTestId('supplier-needs-dance')).toBeVisible();
      await expect(page.getByTestId('supplier-field-key')).toHaveCount(0);

      // pick apiKey → the form swaps to the key field, oauth fields gone.
      await select.selectOption('apiKey');
      await expect(page.getByTestId('supplier-field-key')).toBeVisible();
      await expect(page.getByTestId('supplier-field-client_id')).toHaveCount(0);
      await expect(page.getByTestId('supplier-needs-dance')).toHaveCount(0);
      // apiKey here is in query param api_key — surfaced to owner.
      await expect(page.getByTestId('supplier-cred-form')).toContainText(/query/i);
      await expect(page.getByTestId('supplier-cred-form')).toContainText(/api_key/);
    });
});
...
test.describe('supplier · credential form derived from spec · corner (area B)', () => {
...
test.describe('supplier · credential form derived from spec · err (area B)', () => {
  ...
  // still wants `Authorization: Bearer …`, so refusing outright made those suppliers
  ...
  test('spec with no securityScheme → manual-auth fallback offers pickable schemes',
    async ({ adminPage: page }) => {
      ...
      const status = page.getByTestId('supplier-status');
...
// pasteSpec —— navigate to the suppliers area by clicking (no page.goto),
...
async function pasteSpec(page: Page, spec: string): Promise<void> {
  ...
  await page.getByTestId('supplier-spec-input').blur();
}
