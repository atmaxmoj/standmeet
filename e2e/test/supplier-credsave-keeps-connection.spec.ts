// supplier-credsave-keeps-connection.spec.ts -- **the same root cause** as F-C-30 / F-C-34.
//
// Saving credentials once wipes out "connected": the upsert in `block_connections.sql.go`
...
//   - F-C-34: in the same state, clicking test-send gets "no mail supplier is set up
//     yet -- connect one first". Because "no active supplier" is exactly the condition
//     mapped to "never configured" (`supplier/slots.go:260`). The owner is told to do
...
import {
  configureMailSupplier, connectMailOutcome, mailSupplierStatus,
...
test.describe('F-C-30/34 · saving credentials does not un-connect a working connection', () => {
  ...
  test('a connected mail supplier stays connected when its credentials are saved again',
    async ({ playwright }) => {
      const request = await playwright.request.newContext();
      await configureMailSupplier(request, OWNER.email, OWNER.password);
      const before = await mailSupplierStatus(request);
      expect(before.connected, 'setup: the supplier is connected to begin with').toBe(true);
      ...
      const after = await mailSupplierStatus(request);
