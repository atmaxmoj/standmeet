// supplier-dep-disconnect-mid-session.spec.ts -- fills in the state-transition matrix row
// "connected -> disconnected (owner disconnect) - between two turns - next turn hides the
// tool (single gate recomputes)".
//
// Now that supplier gating routes through the global single gate `enabledCaps`, supplier
...
test.describe('supplier dep · owner disconnect between turns hides booking tool', () => {
  let seed: CodedSeed;
  ...
  test('connected → booking exposed; owner disconnects → next session has no booking tool',
    async () => {
      ...
      // Next turn = a fresh session assembled anew. The single gate recomputes supplier
