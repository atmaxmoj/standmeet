// supplier-dep-drop-mid-turn.spec.ts -- fills in the state-transition matrix cell
...
//
// Shaped just like supplier-revoked-degrades: still connected at assembly time (the tool
...
//
// RED: if the supplier-backed tool's error-mapping/degradation path isn't unified before
...
function expireAccessToken(): void {
  // The supplier refactor generalised the per-category tables into one `block_connections`
  // (block_id / category / token_expires_at); `owner_calendar_suppliers.provider` is gone.
  ...
  if (out.includes('UPDATE 0')) {
    throw new Error(`expireAccessToken matched no supplier row: ${out.trim()}`);
  }
}
...
test.describe('supplier dep · connection drops mid-turn → friendly degrade', () => {
  let seed: CodedSeed;
