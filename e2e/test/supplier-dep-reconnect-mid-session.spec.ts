// supplier-dep-reconnect-mid-session.spec.ts —— state-transition matrix:
...
test.describe('supplier dep · owner reconnect between turns reveals booking tool', () => {
  let seed: BaseSeed;
  test.beforeAll(async ({ playwright }) => {
    // Credentials are already saved, but OAuth hasn't run → supplier not connected (refresh_token IS NULL).
  ...
  test('disconnected → booking absent; owner completes OAuth → next session exposes booking',
    async () => {
      ...
      // Owner completes OAuth between the two turns → supplier connects.
