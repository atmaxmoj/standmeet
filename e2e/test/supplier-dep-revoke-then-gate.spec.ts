// supplier-dep-revoke-then-gate.spec.ts — state matrix "revoke → persisted
// disconnected → next session gated" (supplier-deps-tests.md §3).
// `supplier-dep-drop-mid-turn` verifies the "hit invalid_grant mid-call → graceful
// degrade" half; this covers the **linkage** half: once a refresh hitting
// invalid_grant is recognized as "the connection is now invalid", the supplier's
...
// service every turn).
//
// RED until: invalid_grant marks the owner's calendar supplier disconnected
...
test.describe('supplier dep · revoke detected → supplier marked disconnected → next session gated', () => {
  let seed: CodedSeed;
  ...
  test('a book hitting invalid_grant → connection persisted disconnected → new session no longer exposes booking',
    async () => {
      ...
      // this moment is verified by supplier-dep-drop-mid-turn; here we only care
      ...
      // The linkage: invalid_grant is recognized → supplier is persisted disconnected.
...
function expireAccessToken(): void {
  const sql = `UPDATE block_connections
              ...
              WHERE block_id = 'google-calendar'`;
