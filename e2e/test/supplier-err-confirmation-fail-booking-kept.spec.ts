// supplier-err-confirmation-fail-booking-kept.spec.ts -- Section 4, error
...
// Same failure class as E8 (supplier-err-smtp-fail) but focused on the
...
import {
  configureMailSupplier, clearMailpit, armSMTPFault, resetSMTPFault,
} from '@/fixtures/mail';
...
test.describe('supplier error stream · confirmation send fails but booking is kept (E11)', () => {
  let seed: CodedSeed;
  ...
  test('booking commits → confirmation send fails → event kept, card shows error, no rollback, no crash',
    async ({ browser }) => {
      ...
      // (`ownerCanDeliver`). Being visible = **this owner's mail supplier is
