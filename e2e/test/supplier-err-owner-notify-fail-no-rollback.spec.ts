// supplier-err-owner-notify-fail-no-rollback.spec.ts —— §4 E10
...
import {
  configureMailSupplier, clearMailpit, countMailpitMessages, armSMTPFault, resetSMTPFault,
} from '@/fixtures/mail';
...
test.describe('supplier error stream · owner-notify fails, booking not rolled back (E10)', () => {
  let seed: CodedSeed;
...
async function prep(playwright: Playwright): Promise<CodedSeed> {
  ...
  // configureMailSupplier re-logs-in internally, rotating the CSRF token — refresh
