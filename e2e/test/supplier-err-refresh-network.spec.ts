// supplier-err-refresh-network.spec.ts -- §4 error-flow matrix E7
// token refresh hits a **network error / 500 (not invalid_grant)** -> friendly degrade.
// Distinct from supplier-revoked-degrades: that one is invalid_grant (revoked -> stored
...
// supplier-revoked-degrades (invalid_grant): a transient refresh fault must not
// be treated as a revoked supplier.
//
...
// book call down the refresh path (same technique as supplier-revoked-degrades).
...
test.describe('supplier error stream · token refresh network/500 (not invalid_grant) degrades (E7)', () => {
  let seed: CodedSeed;
