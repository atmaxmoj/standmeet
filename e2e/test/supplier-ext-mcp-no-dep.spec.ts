// supplier-ext-mcp-no-dep.spec.ts -- §1 ext-mcp does not wire dep by default
//
...
// be silently injected a supplier dependency handle, even when it declares
// `Requires:["calendar"]` and calendar IS connected. Supplier handles carry
...
// mcp_server_create) but the registered server's tool declares a supplier
...
interface VisitorBlocksResp { tool_specs: readonly { name: string }[] }
...
// and grants a supplier dep to an ext-mcp server. Admin endpoint:
...
test.describe('supplier dep · ext-mcp plugin is NOT auto-injected a dep handle (lowest trust)', () => {
