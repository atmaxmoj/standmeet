// supplier-gcal-rotate-creds-reverify.spec.ts -- §III row D-5:
...
//
// RED / TDD: depends on the supplier dependency-resolution refactor + the "identity
...
test.describe('supplier · GCal rotate credentials → re-verify (§3 D-5)', () => {
  let seed: BaseSeed;
  ...
  test('connected → save NEW client_id/secret → connected flips false, booking hidden',
    async () => {
      ...
      // fresh session (single-point gate recomputes supplier state).
