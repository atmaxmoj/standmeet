// supplier-happy-matrix.spec.ts —— the "full closed-loop combination matrix" contract from
// #155 §8 (already implemented, green).
//
// This is the **end-to-end happy combination** grid for supplier assembly: for every
// diagonal cell of kind × seam × auth, the whole chain runs from the admin UI —
...
//
// The 5 diagonal combinations covered (docs/design/supplier.md §1 matrix + §4 auth + §5.2
// assembly flow):
...
//      really sends.
//
// The spec-driven assembly UI + derived forms + each auth's connect flow + seam-unified
...
//
// Interface (§8's target interface, real testids): admin-nav-suppliers / supplier-add-open /
// supplier-card-{seam} / supplier-spec-input / supplier-spec-submit /
// supplier-scheme-select / supplier-field-{key} / supplier-connect-button /
// supplier-status; REST GET /api/admin/suppliers (to get {id}); diag/consumer go through
// the API.
//
// Convention: the UI drives assembly (adminPage, no page.goto, entered by clicking through
// admin-nav-suppliers), the API asserts the consumer outcome. Helpers are inlined (a thin
...
interface ConnRef { id: string; seam: string; kind: string; connected: boolean }
interface CalEvent { summary: string; start: string; attendees?: string[] }
interface MailMsg { to: string[]; subject: string }

// ─── UI assemble drivers (no page.goto; click admin-nav-suppliers → add) ───

// openByCategory — enters the suppliers area, clicks "add", picks a card by seam
...
async function openAddCard(page: Page, seam: string) {
  await page.getByTestId('admin-nav-suppliers').click();
  await page.getByTestId('supplier-add-open').click();
  await page.getByTestId(`supplier-card-${seam}`).click();
}

// assembleOpenAPI — submits spec+binding (supplier-spec-input/submit) → (if multiple
// schemes) picks a scheme → fills in the derived fields → connects. Returns that supplier
// (fetched via GET /api/admin/suppliers for the newest {id}).
async function assembleOpenAPI(
  page: Page, request: APIRequestContext, opts: {
    seam: string; spec: unknown; binding: unknown;
  ...
  await page.getByTestId('supplier-spec-input').fill(JSON.stringify(opts.spec));
  await page.getByTestId('supplier-binding-input').fill(JSON.stringify(opts.binding));
  ...
  // supplier's card (credentials + Connect have always been that card's job). Closing it
  // would leave the owner on a list row they can't connect from — a SupplierList row only
  // has seam/status/delete.
  ...
  // (waitForURL might match early just because we're already on /admin/suppliers — the
  // polling in newSupplier absorbs the delay before the callback lands in storage).
  // Non-dance: connects in place, status flips straight to connected.
  if (opts.needsDance) await page.waitForURL('**/admin/suppliers**');
  else await expect(page.getByTestId('supplier-status')).toHaveText(/connected|已连接/i);
...
async function pickSchemeIfOffered(page: Page, scheme?: string): Promise<void> {
  if (!scheme) return;
  const select = page.getByTestId('supplier-scheme-select');
  await expect(page.getByTestId('supplier-cred-form')).toBeVisible();
...
// newSupplier — polls until a connected supplier of this seam shows up that wasn't in
// the before snapshot (= the one just assembled). Multiple combos share one owner: taking
// "the first connected" by seam would grab an earlier combo's supplier (causing the
// wrong supplier to be activated, the booker to hit the wrong provider); pinpointing by
// "newly appeared id" is the correct approach.
async function newSupplier(
  request: APIRequestContext, before: Set<string>, seam: string,
): Promise<ConnRef> {
  ...
  await expect.poll(async () => {
    ...
    const rows = (await res.json() as { suppliers?: ConnRef[] }).suppliers ?? [];
    hit = rows.find((c) => c.seam === seam && c.connected && !before.has(c.id));
    return Boolean(hit);
  }, { timeout: 15_000 }).toBe(true);
  if (!hit) throw new Error(`no new connected ${seam} supplier found`);
  return hit;
}
...
// bookAndAssert — the shared tail for calendar combos: asserts the seam slot is
...
async function bookAndAssert(
  ...
  expect(cap?.dependency?.connected, 'calendar seam slot connected').toBe(true);
...
// mailViaContract — triggers MailContract.Send through the mail seam slot's test-send,
...
async function expectMailSent(
  ...
  expect(cap?.dependency?.connected, 'mail seam slot connected').toBe(true);
  // The address is derived from the supplier's own declaration (smtp's manifest.yaml →
  // suppliers.mail_test_send), no longer hardcoded as `/suppliers/mail/test-send` on the
  // generic registry.
  ...
  const res = await request.post(`${BACKEND}/api/admin/suppliers/ops/mail_test_send`, {
...
test.describe('supplier · happy combination matrix (kind × seam × auth full loop)', () => {
  // Covers the spec-driven assembly UI + derived forms + each auth's connect flow +
  // seam-unified consumption (docs/design/supplier.md §8 phase-one A+B+D+F).
  ...
  test('openapi calendar + oauth2: assemble → dance → booker books → event on provider',
    async ({ adminPage: page }) => {
      ...
      await assembleOpenAPI(page, request, {
        seam: 'calendar', spec: OAUTH2_CAL_SPEC, binding: CAL_BINDING,
        scheme: 'oauth2',
  ...
  test('openapi calendar + apiKey: assemble → apiKey form (no dance) → booker books',
    async ({ adminPage: page }) => {
      ...
      await resetCalMock(request);
      await assembleOpenAPI(page, request, {
        seam: 'calendar', spec: APIKEY_CAL_SPEC, binding: CAL_BINDING,
  ...
  test('openapi mail + bearer: assemble → bearer form → MailContract.Send delivers',
    async ({ adminPage: page }) => {
      const { csrf } = await login(request, OWNER.email, OWNER.password);
      await assembleOpenAPI(page, request, {
        seam: 'mail', spec: BEARER_MAIL_SPEC, binding: MAIL_BINDING,
