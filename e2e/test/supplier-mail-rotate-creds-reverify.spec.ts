// supplier-mail-rotate-creds-reverify.spec.ts —— §three, row D-5:
// "change an identity field: mail's SMTP from-address/host/password"
// (edit-config) → connected=false → the smtp-dependent block
...
// host is an identity change → backend resets connected=false (UpsertSupplierCredentials
// clears connected_at) → the smtp-dependent block (can_deliver_codes on
...
import { configureMailSupplier, saveMailCreds, connectMail } from '@/fixtures/mail';
...
test.describe('supplier · mail rotate SMTP identity → re-verify (§3 D-5)', () => {
  ...
  test('connected → change from_address → connected resets, can_deliver_codes false; re-connect restores',
    async ({ playwright }) => {
      ...
      // smtp-dependent block drops until re-verified.
      expect(await canDeliverCodes(request)).toBe(false);

      // re-run the connection test → restores the block (slot still active).
