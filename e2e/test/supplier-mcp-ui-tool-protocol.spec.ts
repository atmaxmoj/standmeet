// supplier-mcp-ui-tool-protocol.spec.ts -- Section 1 (mcp-ui:tool protocol)
//
...
test.describe('supplier · mcp-ui:tool host protocol round-trip (§1)', () => {
  let seed: CodedSeed;
  ...
  test('sandbox card posts mcp-ui:tool → host dispatches → tool runs (observable effect) → tool-result re-renders card',
    async ({ browser }) => {
      ...
      // tool deleted that event through the supplier proxy (the side effect
