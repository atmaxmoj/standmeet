// supplier-op-calendar-check-ui.spec.ts — F-C-16: the calendar card must offer a way to check
// "is it still live right now?"
//
// The mail supplier has one (`suppliers.mail_test_send`, declared in smtp's manifest); the
...
// OP — the operation the calendar declares in its own manifest; strip the `suppliers.` prefix
...
test.describe.serial('suppliers · the calendar card can answer "is it live?" (F-C-16)', () => {
  // The instance is reset only here — the two cases after this mutate the supplier's state
  ...
  test('a live check reports what the calendar actually said', async ({ adminPage }) => {
    ...
    // enough).
    await op.getByTestId('supplier-op-field-days').fill('120');
    await op.getByTestId('supplier-op-run').click();
    ...
    expect(
      (await op.getByTestId('supplier-op-result').innerText()).toLowerCase(),
