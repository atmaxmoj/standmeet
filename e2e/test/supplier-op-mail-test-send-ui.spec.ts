// supplier-op-mail-test-send-ui.spec.ts —— F-C-12: a supplier's **own declared owner op** must have a face on the card.
//
// why this guard exists, rather than yet another endpoint-hitting test: `suppliers.mail_test_send` has long worked ——
// declared in smtp's manifest, implemented in axisconn/impls.go, routed at /suppliers/ops/mail_test_send,
...
//
// five existing tests touch this op (supplier-happy-matrix / supplier-openapi-mail /
// supplier-provider-agnostic / owner-mcp-parity-suppliers / norm-outward-toolset),
// each one a `request.post(.../ops/…)` or an MCP callTool ——**not one goes through the browser**.
// a suite that only drives the block, not the face, stays all green even when the face doesn't exist at all. So this one goes only through the GUI:
...
//
// F-C-34 —— failure classification has three branches, and all three are guarded here: no supplier / relay rejects / **unreachable**.
...
// "you haven't configured a mail supplier yet". **This guard can't reproduce that cell** —— while the supplier is still in the active slot, the product
...
// is exactly the condition that maps to "not configured yet" (`supplier/slots.go:260`). Producing that state is F-C-30's job,
// the two share one root. What's left here is the regression guard for "the unreachable branch is alive".

import { claim, login } from '@/fixtures/admin';
import { resetInstance, findSetupToken } from '@/fixtures/instance';
import {
  armSMTPFault, clearMailpit, configureMailSupplier, connectMailOutcome,
...
// OP —— the op smtp declares in its own manifest; strip the `suppliers.` prefix and it's the route segment,
...
// serial —— the first test wants the "no mail supplier yet" state, the second connects it. The order is part of the tests.
test.describe.serial('suppliers · a declared owner op has a face on the card (F-C-12)', () => {
  ...
  test('with no mail supplier, the card tells the owner what to do next', async ({ adminPage }) => {
    await gotoAdminSection(adminPage, 'suppliers');
    const op = adminPage.getByTestId(`supplier-op-${OP}`);
    ...
    await op.getByTestId('supplier-op-field-to').fill('nobody@standmeet.test');
    ...
    await expect(
      op.getByTestId('supplier-op-result'),
      'a failure must name the next step, not merely report failure',
    ).toHaveText('no mail supplier is set up yet — connect one first');
  });
  ...
  test('a test-send from the card really leaves the building', async ({ adminPage, playwright }) => {
    ...
    await op.getByTestId('supplier-op-field-to').fill(to);
    await op.getByTestId('supplier-op-run').click();
  ...
  test('a relay that rejects the message says to change the recipient, not to wait',
    async ({ adminPage, playwright }) => {
      const request = await playwright.request.newContext();
      // configure the mail supplier ourselves, not relying on the state left by the previous test: running this one alone would turn that sentence into
      // "no mail supplier yet" —— a red, but a red on assembly, proving nothing about classification.
      await configureMailSupplier(request, OWNER.email, OWNER.password);
...
// the third failure: the supplier **is** there (configured, connected, occupying the category slot), but unreachable. See the F-C-34 paragraph in the file header.
// a separate describe: it shares the file-level ownerCredentials and the already-claimed instance with the group above,
// but configures its own supplier and makes its own failures, not relying on the state left above.
...
test.describe('suppliers · a configured-but-unreachable relay names its own class (F-C-34)', () => {
  ...
  // `use-supplier-op.ts`'s `.catch(() => ({ reached: false }))` —— any rejection counts as didn't-go-through,
