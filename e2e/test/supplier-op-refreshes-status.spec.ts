// supplier-op-refreshes-status.spec.ts —— F-C-45: **a card must not say two contradictory
// things at once.**
//
...
test.describe('F-C-45 · an op that finds the grant revoked updates the card it sits on', () => {
  ...
  test('the probe says revoked, and the status beside it stops saying connected',
    async ({ adminPage: page }) => {
      ...
      const card = await openSupplierCard(page, 'google-calendar');
      ...
      await expect(card.getByTestId('supplier-status'),
