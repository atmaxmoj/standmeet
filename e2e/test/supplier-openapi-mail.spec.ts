// supplier-openapi-mail.spec.ts -- #155 §8 zone F (consumption closure), filling the
// **missing diagonal**: openapi × mail.
//
// The existing suite covers two diagonals:
//   - openapi × calendar  -> supplier-binding-jsonata.spec.ts (SaaS spec + JSONata binding)
//   - protocol × mail      -> supplier-protocol-smtp.spec.ts / supplier-provider-agnostic.spec.ts (built-in SMTP)
...
// bearer) -> the backend assembles a kind=openapi mail supplier -> connect (apiKey, no
...
// body shape.
//
// Aligned with docs/design/supplier.md: §1 (mail = SendGrid(openapi) or SMTP(protocol),
...
// JSONata only) + §8 target interface (REST POST /api/admin/suppliers {spec,binding};
...
//     direction's construction block).
...
// POST /api/admin/suppliers -- builds an openapi supplier from spec+binding. 201 → {id}; 4xx → {error}.
async function createSupplier(
  request: APIRequestContext, csrf: string,
  body: { spec: unknown; binding: unknown },
): Promise<CreateResult> {
  // eslint-disable-next-line e2e-local/no-direct-mutating-api -- action under test: bespoke openapi supplier build; returns status+id+error so callers inspect both 201 and 4xx
...
async function connectApiKey(
  ...
  const credRes = await request.post(
    `${BACKEND}/api/admin/suppliers/${encodeURIComponent(id)}/credentials`,
  ...
  const connectRes = await request.post(
    `${BACKEND}/api/admin/suppliers/${encodeURIComponent(id)}/connect`,
    { headers: { 'X-Csrftoken': csrf }, data: {} },
  );
  if (connectRes.status() !== 200) throw new Error(`mail connect: ${connectRes.status()}`);
  const stRes = await request.get(`${BACKEND}/api/admin/suppliers/${encodeURIComponent(id)}/status`);
...
async function disconnectSupplier(
  ...
  const res = await request.post(
    `${BACKEND}/api/admin/suppliers/${encodeURIComponent(id)}/disconnect`,
    { headers: { 'X-Csrftoken': csrf }, data: {} },
  );
...
// assembleAndConnectMail -- assembles one openapi mail supplier (SendGrid spec+binding)
...
async function assembleAndConnectMail(
  ...
  const r = await createSupplier(request, csrf, {
...
// runFormEncodedSend -- assembles a supplier that declares form-encoding, sends one
...
async function runFormEncodedSend(request: APIRequestContext, csrf: string): Promise<void> {
  ...
  expect(st.connected, 'precondition: the key saves and the supplier connects').toBe(true);
...
async function runNonSendingFlagged(request: APIRequestContext, csrf: string): Promise<void> {
  ...
  if (r.status >= 400 && r.status < 500) {
    ...
    expect(r.id, 'supplier not created').toBeFalsy();
    return;
  }
...
// runDepGating -- disconnect the openapi mail supplier -> mail.send re-gates.
async function runDepGating(request: APIRequestContext, csrf: string): Promise<void> {
  const st = await assembleAndConnectMail(request, csrf);
  const before = await findBlock(request, csrf, 'mail.send');
  expect(before?.dependency?.connected, 'connected → mail slot connected').toBe(true);

  await disconnectSupplier(request, csrf, st.id);
  const after = await findBlock(request, csrf, 'mail.send');
  expect(after?.dependency?.connected, 'disconnected → mail slot disconnected → mail.send re-gated').toBe(false);
}

test.describe('supplier · openapi mail (SendGrid-style, kind=openapi fills the mail slot, §8 area F diagonal)', () => {
  ...
  // Reset the instance + owner + SendGrid mock per test (suppliers don't accumulate
  ...
  // happy: assemble an openapi mail supplier -> connect(apiKey) -> mail.send cap gate
  // opens -> MailContract sends through it -> mock receives it.
  test('assemble openapi mail supplier → connect (apiKey, no dance) → mail.send un-gates → MailContract.Send goes through it → mock received',
  ...
  //
  // Seen while driving mail-supplier check 5: that field wants "the message id read
  ...
  // dep-gating: disconnect the openapi mail supplier -> mail.send re-gates. Implemented,
  // via "mail as a visitor block".
  test('disconnect the openapi mail supplier → mail.send re-gates',
    async () => { await runDepGating(request, csrf); });
});

// This file used to have an inline diagInvoke -- hitting the owner-authed supplier
