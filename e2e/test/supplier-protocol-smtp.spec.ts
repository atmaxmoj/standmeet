// supplier-protocol-smtp.spec.ts — #155 §8 section E (protocol SMTP) target contract (RED).
//
// Business story: the owner installs a kind=protocol SMTP supplier to fill the "mail"
// category slot. Unlike an openapi supplier (whose form is derived from the spec's
...
// docs/design/supplier.md §2 smtpForm), never derived from any spec. Owner picks "SMTP
...
//
// Covers "protocol supplier UI assembly + fixed form + connection test" (§8 section E).
...
//   testid: supplier-add-open / supplier-card-smtp / supplier-scheme-select
//           / supplier-field-{key} / supplier-connect-button
//           / supplier-status(connected|not) / supplier-config-save
//   REST  : POST /api/admin/suppliers (built from the built-in protocol)
...
// The SMTP protocol supplier's fixed credential fields (hand-defined, docs §2
...
test.describe('supplier · protocol SMTP (kind=protocol, fixed credential form)', () => {
  // Covers protocol(SMTP) supplier UI assembly + the fixed form + the connection test
  // (supplier.md §8 section E). Implemented, green.
  ...
  test('pick SMTP(protocol) → 6 fixed fields render (not spec-derived) → fill → connect → Connected',
    async ({ adminPage }) => {
      ...
      await adminPage.getByTestId('supplier-connect-button').click();
      await expect(adminPage.getByTestId('supplier-status')).toHaveText(/connected|已连接/i);
    });
  ...
  test('wrong host/port → connection test fails → friendly error, status stays not connected',
    async ({ adminPage }) => {
      ...
      const err = adminPage.getByTestId('supplier-error');
      ...
      await expect(adminPage.getByTestId('supplier-status')).not.toHaveText(/connected|已连接/i);
    });
  ...
  test('bad SMTP auth config (handshake cannot complete) → connection test fails → not connected',
    async ({ playwright }) => {
      ...
      const id = await createSMTPSupplier(request);
...
// ─── helpers (inline; promote to fixtures/supplier-protocol.ts once the implementation
// turns green) ───
...
// openSMTPForm — go to suppliers -> open the add modal -> click the SMTP(protocol) card
...
async function openSMTPForm(page: Page): Promise<void> {
  ...
  // In the catalog, the protocol supplier is the "SMTP" card; clicking it renders the
  // fixed form.
  await page.getByTestId('supplier-card-smtp').click();
}

// assertFixedFormNotDerived — a protocol supplier has **no spec input / no scheme
...
async function assertFixedFormNotDerived(page: Page): Promise<void> {
  await expect(page.getByTestId('supplier-spec-input')).toHaveCount(0);
  await expect(page.getByTestId('supplier-scheme-select')).toHaveCount(0);
  ...
  await expect(page.getByTestId('supplier-field-password')).toHaveAttribute('type', 'password');
  await expect(page.getByTestId('supplier-field-tls')).toHaveJSProperty('tagName', 'SELECT');
}

async function fillSMTPForm(page: Page, fields: Record<string, string>): Promise<void> {
  for (const [key, value] of Object.entries(fields)) {
    const field = page.getByTestId(`supplier-field-${key}`);
    if (key === 'tls') { await field.selectOption(value); continue; }
    await field.fill(value);
  }
}

// createSMTPSupplier — POST /api/admin/suppliers to create a row from the built-in
// protocol(SMTP), returning the supplier id (§8 generalizes the gcal pattern to {id}).
// kind=protocol carries no spec.
async function createSMTPSupplier(request: APIRequestContext): Promise<string> {
  ...
  const res = await request.post(`${BACKEND}/api/admin/suppliers`, {
    ...
    data: { kind: 'protocol', protocol: 'smtp', seam: 'mail' },
  });
  if (res.status() !== 201) throw new Error(`create smtp supplier: ${res.status()}`);
