// supplier-provider-agnostic.spec.ts -- #155 §8 area F (consumption loop), focused on
// provider-agnostic behavior.
//
// ⭐ the linchpin (seam unification): install a **non-Google** calendar supplier
// (CalDAV, kind=protocol; or a test provider) into the "calendar" seam slot -> a session
...
// (docs/design/supplier.md §2 "the contract abstracts kind away" + §5.3 consumption flow).
//
// Also covers:
//   - SMTP (kind=protocol) filling the mail seam -> mailer sends via MailContract.Send, kind-agnostic.
//   - err: supplier connected but runtime API 5xx -> friendly degrade (no crash, no leaked stack, no event created).
//   - dep-gating: calendar supplier connect/disconnect -> calendar.book cap un-gates / re-gates
//     (via DepRegistry's global single gate, supplier.md §6 + blocks.ts dependency.connected).
//
// Covers "seam unification lets any provider feed booker". Implemented, actually
// compiles, actually runs, actually goes green (booker consumes any provider through the
// seam contract, no longer only recognizing a hand-rolled gcal-specific supplier;
...
// MailContract.Send; blocks.ts dependency.connected; gcal-setup's connected-owner is
...
// events / fail / reset, structurally identical to the gcal mock but through the provider-agnostic "calendar" seam endpoint.
...
test.describe('supplier · provider-agnostic consumer loop (area F)', () => {
  // #155 §8-F: seam unification -- non-Google (CalDAV protocol) calendar feeds booker,
  // without changing a line of code. The SMTP mail case sends via "mail as a visitor
  // block" (sandboxed mail-sender plugin), already implemented.
  ...
  test('non-Google (CalDAV) calendar connects → calendar_book assembles + booking works (booker unchanged)',
    async () => {
      ...
      // Install a non-gcal calendar supplier into the "calendar" seam slot and connect it (no OAuth dance).
      const conn = await connectCalDAVCalendar(request, csrf);

      // dep-gating: the calendar seam slot is now "connected" -> the calendar.book cap un-gates.
      const cap = await findBlock(request, csrf, 'calendar.book');
      expect(cap?.dependency?.connected, 'CalDAV connected → calendar seam slot connected').toBe(true);
      ...
      // calendar_book gets assembled (seam unification: the consumer doesn't know CalDAV is behind it).
  ...
  // dep-gating: calendar supplier disconnect -> cap re-gates; reconnect -> un-gates.
  test('calendar supplier disconnect → calendar.book re-gated; reconnect → un-gated',
    async () => { await runDepGating(request); });
});

// ─── helpers (inline; promote to fixtures/supplier-providers.ts once implementation goes green) ───
...
// runDepGating -- calendar supplier connect -> un-gate, disconnect -> re-gate, reconnect -> un-gate.
// Goes through DepRegistry's global single gate (driven by blocks.ts dependency.connected).
async function runDepGating(request: APIRequestContext): Promise<void> {
  ...
  await disconnectSupplier(request, csrf, conn.id);
  const capGated = await findBlock(request, csrf, 'calendar.book');
  expect(capGated?.dependency?.connected, 'disconnected → seam slot disconnected').toBe(false);
  await expectCalendarBookExposed(request, (await newSession('B')).session_token, false);

  // Reconnect -> un-gates.
  await reconnectSupplier(request, csrf, conn.id);
...
// sendViaMailContract -- triggers MailContract.Send through the supplier's self-test send
// endpoint (verifies mailer goes through the seam contract and doesn't care what kind is
...
async function callBook(
  ...
  const res = await request.post(
    `${BACKEND}/api/v1/sessions/${visitor.conversation_id}/tools/calendar_book`,
    {
      ...
      // carries a TTL that lives in the host's blockstore (the mock's reset can't clear it).
