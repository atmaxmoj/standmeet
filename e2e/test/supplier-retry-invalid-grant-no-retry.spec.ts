// supplier-retry-invalid-grant-no-retry.spec.ts — §5 retry matrix · non-retryable
...
//
// Distinct from supplier-revoked-degrades (which asserts the degrade) and
// supplier-dep-drop-mid-turn (mid-turn drop): THIS one asserts the retry layer
...
test.describe('supplier retry · invalid_grant is non-retryable → fails fast, token endpoint hit once', () => {
  let seed: CodedSeed;
