// supplier-retry-send-idempotent.spec.ts — §5 retry matrix R4 · confirmation email
// is never sent twice
//
...
// supplier-retry-insert-idempotent (events.insert is never double-booked).
//
...
import {
  configureMailSupplier, clearMailpit, waitForMailTo, countMailpitMessages,
} from '@/fixtures/mail';
import {
...
test.describe('supplier retry · confirmation send under transient error does not double-send (R4)', () => {
  let seed: CodedSeed;
