// supplier-scope-gates-block.spec.ts — F-B-8 ⭐⭐.
// **The scope that was granted and the block that gets exposed have never been cross-checked
// against each other.**
//
...
//      `supplier-scope-shortfall` line ("this grant can't do X, go add Y"). That UI was never
...
test.describe('F-B-8 · a read-only grant must not put booking in front of a visitor', () => {
  ...
  test('the owner is told the grant cannot book, on the card that claims to be connected',
    async ({ adminPage: page }) => {
      ...
      // exactly here.
      await expect(card.getByTestId('supplier-scope-shortfall'),
...
async function connectReadOnly(page: Parameters<typeof openSupplierCard>[0]) {
