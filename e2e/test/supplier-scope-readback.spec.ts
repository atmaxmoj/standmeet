// supplier-scope-readback.spec.ts —— F-C-33. **After connecting, can the owner still see what they granted?**
//
// `supplier-connect-flow.spec.ts` has a case guarding the **write** half: the checked scope subset enters the dance unchanged.
...
test.describe('F-C-33 · a connection shows the scopes it was granted', () => {
  ...
  test('the granted scopes are still visible after leaving and coming back',
    async ({ adminPage: page }) => {
      ...
      const back = await openSupplierCard(page, OAUTH2_SUPPLIER_ID);
      await expectConnected(back);
      await expect(back.getByTestId(`supplier-scope-${SCOPE_READ}`),
        'the scope this supplier was actually granted still reads as granted')
        .toBeChecked();
      await expect(back.getByTestId(`supplier-scope-${SCOPE_WRITE}`),
