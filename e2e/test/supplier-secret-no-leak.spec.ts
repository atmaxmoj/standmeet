// supplier-secret-no-leak.spec.ts —— Phase B / secret-scan: a supplier credential must
// **never** appear in anything the visitor can see. Booking / listing slots goes through the
// GCal supplier (which holds the secret and calls on the owner's behalf), but the owner's
// client_secret / token value must never appear in a tool_result or in the admin
// conversation transcript.
//
// This pairs with supplier-credential-arch (the go-arch-lint structural gate) to pincer
// "credentials never leave the vault" from both sides: structurally, block code can't
...
// supplier proxy and **never** crosses the plugin boundary — assert the secret doesn't end
...
test.describe('Phase B · supplier credential never leaks to the visitor', () => {
  let seed: CodedSeed;
  ...
  test('calendar_list_slots goes through the GCal supplier but never echoes the secret',
    async ({ playwright }) => {
      ...
      // exercises the supplier (FreeBusy via the stored token).
