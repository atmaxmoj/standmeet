// supplier-security.spec.ts -- #155 §8 zone H (security). Implemented, green
// (originally a RED contract).
//
// "The owner uploads an arbitrary OpenAPI spec" forces out three security gates; these
// three must hold in a spec-driven supplier implementation, and this file pins them
...
//   2. Credentials never leak -- once a user-uploaded openapi supplier is connected,
...
//      handle_contract_test.go + supplier-secret-no-leak.spec.ts to "uploaded" suppliers.
//   3. Per-owner isolation -- an uploaded supplier and its connection belong to one
...
//   POST   /api/admin/suppliers            (create from a spec)
//   POST   /api/admin/suppliers/{id}/credentials
//   POST   /api/admin/suppliers/{id}/connect      (start oauth)
//   GET    /api/admin/suppliers/{id}/status
//   GET    /api/admin/suppliers                    (list)
//   DELETE /api/admin/suppliers/{id}/disconnect
//
// The "create any supplier via POST from a spec" upload path is implemented; this
...
} from '@/fixtures/supplier-security-specs';

const BACKEND = process.env['BACKEND_URL'] ?? 'http://localhost:8000';

const OWNER = {
  email: 'sec-supplier@example.com',
  password: 'correct-horse-battery-staple',
  handle: 'secconn',
  fullName: 'Security Supplier Owner',
};

test.use({ ownerCredentials: { email: OWNER.email, password: OWNER.password } });

// uploadSpec —— POST /api/admin/suppliers from a raw spec string. Returns the
// new supplier id (assigned by the backend). Throws on non-2xx so callers that
...
async function uploadSpec(
  ...
  if (!body.id) throw new Error('uploadSpec: response missing supplier id');
  return body.id;
}

test.describe('supplier · §8 area H security (SSRF / no credential leak / per-owner isolation)', () => {
  // #155 §8 H implemented: security gates for spec-driven supplier uploads
  ...
  test('SSRF · no leftover supplier after rejection (list excludes the rejected spec)',
    ({ playwright }) => ssrfRejectLeavesNoSupplier(playwright));

  // -- 2. credentials never leak (extends handle_contract / secret-no-leak to user-uploaded) --
  test('no credential leak · an uploaded supplier api key is masked in status/list, raw never returned',
    ({ playwright }) => secretMaskedInAdminReads(playwright));

  test('no credential leak · an uploaded supplier secret never appears on any visitor-visible surface',
  ...
  test('rotation · one unreadable supplier does not take the whole list down',
  ...
  test('isolation · an unauthenticated request cannot read an owner-uploaded supplier',
    ({ playwright }) => unauthCannotReadSupplier(playwright));

  test('isolation · unauthenticated cannot disconnect or change an owner-uploaded supplier credentials',
    ({ playwright }) => unauthCannotMutateSupplier(playwright));
});
...
async function ssrfServerUrlRejected(playwright: Playwright): Promise<void> {
  ...
  for (const url of INTERNAL_SERVER_URLS) {
    ...
    // contract; it must NOT 2xx and silently hold an internal-pointing supplier
...
async function ssrfOAuthUrlRejected(playwright: Playwright): Promise<void> {
  ...
  const upload = await request.post(`${BACKEND}/api/admin/suppliers`, {
  ...
  const connect = await request.post(`${BACKEND}/api/admin/suppliers/${id}/connect`, {
...
async function ssrfRejectLeavesNoSupplier(playwright: Playwright): Promise<void> {
  const request = await playwright.request.newContext();
  const { csrf } = await login(request, OWNER.email, OWNER.password);
  ...
  await request.post(`${BACKEND}/api/admin/suppliers`, {
    headers: { 'X-Csrftoken': csrf },
    data: { spec: JSON.parse(specWithServerURL('http://169.254.169.254/latest/meta-data/')) },
  });
  // a rejected SSRF spec must not have been persisted as a supplier row.
  const list = await request.get(`${BACKEND}/api/admin/suppliers`, {
    headers: { 'X-Csrftoken': csrf },
  });
  expect(list.status()).toBe(200);
  expect(await list.text(), 'rejected internal spec left no supplier behind')
    .not.toContain('169.254.169.254');
  await request.dispose();
}

// ── 1b. consume-time SSRF ──
// Upload + credential a supplier whose *static* URLs are benign, then drive the
...
async function ssrfOAuthDanceRedirectRejected(playwright: Playwright): Promise<void> {
  ...
  // the supplier must end up NOT connected (the internal hop never completed).
...
async function secretMaskedInAdminReads(playwright: Playwright): Promise<void> {
  ...
  const credRes = await request.post(`${BACKEND}/api/admin/suppliers/${id}/credentials`, {
  ...
  const status = await request.get(`${BACKEND}/api/admin/suppliers/${id}/status`, {
    headers: { 'X-Csrftoken': csrf },
  });
  ...
  expect(list.status()).toBe(200);
  expect(await list.text(), 'secret not in suppliers list').not.toContain(BENIGN_API_KEY_SECRET);
  await request.dispose();
}
...
// card on `/admin/suppliers` said `not connected`, the credentials box was empty, and
...
// crash". The reverse case (a supplier that was genuinely never connected still just
...
async function unreadableCredentialAsksReconnect(playwright: Playwright): Promise<void> {
  ...
  // supplier was never configured (a false red).
  const before = await request.get(`${BACKEND}/api/admin/suppliers/${id}/status`, {
    headers: { 'X-Csrftoken': csrf },
  });
  expect(before.status(), 'the supplier must really be configured first').toBe(200);

  corruptStoredCredential(id);

  const after = await request.get(`${BACKEND}/api/admin/suppliers/${id}/status`, {
    headers: { 'X-Csrftoken': csrf },
  });
  expect(
    after.status(),
...
// prod: **one** unreadable supplier took `suppliers.list` down with a 500, so
...
async function unreadableCredentialDoesNotSinkTheList(playwright: Playwright): Promise<void> {
  ...
  await request.post(`${BACKEND}/api/admin/suppliers/${broken}/credentials`, {
    headers: { 'X-Csrftoken': csrf }, data: { api_key: BENIGN_API_KEY_SECRET },
  });
  ...
  expect(
    list.status(),
    'one unreadable row must not take the whole suppliers surface down',
  ).toBe(200);
  expect(
    await list.text(),
...
async function secretNotInVisitorSurface(playwright: Playwright): Promise<void> {
  ...
  // the public instance descriptor is the visitor-facing supplier surface; it
  // may advertise *blocks* but never the owner's stored secret.
...
async function unauthCannotReadSupplier(playwright: Playwright): Promise<void> {
  const request = await playwright.request.newContext();
  const { csrf } = await login(request, OWNER.email, OWNER.password);
  const id = await uploadSpec(request, csrf, SPEC_BENIGN_APIKEY);
  await request.dispose();
  ...
  // outsider" stand-in for single-owner v1: admin supplier routes are
  // owner-scoped, so this must be refused, not served.
  const anon = await playwright.request.newContext();
  const list = await anon.get(`${BACKEND}/api/admin/suppliers`);
  expect([401, 403], 'unauthenticated list refused').toContain(list.status());
  const status = await anon.get(`${BACKEND}/api/admin/suppliers/${id}/status`);
  expect([401, 403], 'unauthenticated status refused').toContain(status.status());
  await anon.dispose();
}

async function unauthCannotMutateSupplier(playwright: Playwright): Promise<void> {
  const request = await playwright.request.newContext();
  const { csrf } = await login(request, OWNER.email, OWNER.password);
  const id = await uploadSpec(request, csrf, SPEC_BENIGN_APIKEY);
  await request.dispose();
  ...
  const cred = await anon.post(`${BACKEND}/api/admin/suppliers/${id}/credentials`, {
  ...
  const dis = await anon.delete(`${BACKEND}/api/admin/suppliers/${id}/disconnect`);
...
// diagInvoke -- hits the owner-authed supplier diag endpoint. **This is a back door
// that bypasses the real path** (the real path is visitor chat -> agent -> booker
// sandbox -> supplier.invoke), so it's **deliberately** kept inline here rather than
