// supplier-send-confirmation-tool.spec.ts — §1 (send_confirmation as a tool) + D-4
//
// Today, sending the confirmation email goes through REST. After the refactor it becomes
// a supplier-backed **tool**: clicking "use profile / other address / skip" on the
...
// `send_confirmation` tool with session context -> the tool sends via the mail supplier
...
test.describe('supplier · send_confirmation as a tool (§1 + D-4 recipient hardening)', () => {
  let seed: CodedSeed;
...
async function quoteFlow(browser: Browser, seed: CodedSeed): Promise<void> {
  ...
  // Observable: the tool, via the mail supplier proxy, sent one email to session-email;
  // exactly one send.
  ...
  // **block assembly (spinning up the sandbox)**, roughly 1s when idle, but measured
