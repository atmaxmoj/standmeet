// supplier-single-gate-consistency.spec.ts -- §1 the single gate is consistent across all three walks
//
// D-2: supplier gating is collapsed into `enabledCaps` (the global single
// gate). All three visitor "walks" read that one gate, so an unconnected
// supplier must produce the SAME answer on all three surfaces:
//   1. the issued session's `tool_specs`         (no calendar_book)
//   2. the session's `blocks` states       (no calendar.book entry)
...
// The booking block id (dotted) + its tool name (snake_case) +
// its prompt fragment id.
const BOOK_CAP_ID = 'calendar.book';
const BOOK_TOOL_NAME = 'calendar_book';
const BOOK_FRAGMENT_ID = 'blocks/calendar.book';
...
test.describe('supplier dep · one unconnected state → consistent absence on all three walks', () => {
  ...
  test('calendar NOT connected → no calendar_book tool, no calendar.book cap entry, no booking fragment',
    async () => {
      ...
      // walk 2 — blocks states (VisitorStates). Single-gate means an
      ...
      const bookCap = (sess.blocks ?? []).find((c) => c.id === BOOK_CAP_ID);
      expect(bookCap, 'walk 2: calendar.book cap not present when unconnected')
        .toBeUndefined();
      const diagBookCap = diag.blocks.find((c) => c.id === BOOK_CAP_ID);
      ...
      // consistency: session.blocks and diag.blocks are the same
      // Registry source, so they must agree on calendar.book's absence too.
      const sessHasBook = (sess.blocks ?? []).some((c) => c.id === BOOK_CAP_ID);
      const diagHasBook = diag.blocks.some((c) => c.id === BOOK_CAP_ID);
