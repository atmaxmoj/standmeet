// supplier-spec-fetch-names-the-refusal.spec.ts -- F-C-23: a fetch blocked by the SSRF
// gate must not be reported as "is it unreachable?".
//
// Found by driving the real environment: on prod's add-supplier panel, filling the spec
...
async function fetchSpecFrom(page: Page, url: string): Promise<void> {
  await page.getByTestId('supplier-spec-url-input').fill(url);
  await page.getByTestId('supplier-spec-fetch-button').click();
}

test.describe('supplier · spec fetch names which refusal it is', () => {
