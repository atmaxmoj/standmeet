// supplier-spec-from-url-assembles.spec.ts —— F-C-25 + F-C-26.
//
...
// be assembled into a supplier" was never walked by anyone.
//
...
// SUPPLIER_EGRESS_ALLOW, which is what lets the assemble-time egress static check pass.
...
test.describe('a spec fetched from a URL can actually be assembled', () => {
  ...
  test('fetch by URL → supply the base URL → assemble leaves a supplier behind',
    async ({ adminPage: page }) => {
      ...
      await expect(page.getByTestId('supplier-spec-error')).toContainText(/servers|base url/i);
      ...
      await page.getByTestId('supplier-field-token').fill('vendor-test-token');
      ...
      expect(await newSupplierID(page, before), 'a URL-fetched spec must assemble')
        .not.toBe('');
    });
  ...
  test('a fetched spec that is merely too large says so, not "invalid JSON" (F-C-52)',
    async ({ adminPage: page }) => {
      await openSupplierAdd(page);
      await page.getByTestId('supplier-spec-url-input').fill(TOO_BIG_SPEC_URL);
      await page.getByTestId('supplier-spec-fetch-button').click();

      const said = page.getByTestId('supplier-spec-error');
  ...
  // valid (validation passes, a candidate appears), but at supplier-creation time the
  ...
  test('a refused assemble says so inside the modal', async ({ adminPage: page }) => {
    ...
    const err = page.getByTestId('supplier-assemble-error');
