// supplier-spec-ingest.spec.ts —— #155 area A (spec ingest) RED contract.
//
// Story: the owner clicks "add" on /admin/suppliers → pastes / uploads an OpenAPI
// spec → the backend parses it → a "supplier candidate" appears (category + derived
...
// silent swallowing.
//
// Aligned with docs/design/supplier.md §8 area A + the target interface sketch:
//   testid: supplier-add-open / supplier-spec-input (paste or upload) /
//           supplier-spec-submit / supplier-spec-error / supplier-candidate /
//           supplier-spec-url-input / supplier-spec-fetch-button
//   REST:   POST /api/admin/suppliers (builds from a spec)
//
...
test.describe('supplier · area A spec ingest · happy', () => {
  // Covers the spec-driven ingest UI/backend (docs/design/supplier.md §8 area A).
  ...
  // candidate appears.
  test('valid 3.0 spec pasted → parsed → shows calendar supplier candidate', async ({
    adminPage: page,
  }) => {
    ...
    const candidate = page.getByTestId('supplier-candidate');
    ...
    await expect(page.getByTestId('supplier-spec-error')).toHaveCount(0);
  });
  ...
  test('valid 3.0 spec file uploaded → parsed → candidate appears', async ({ adminPage: page }) => {
    ...
    // recognizes input[type=file]; paste uses the supplier-spec-input textarea).
    // onChange reads the file → the same validation path → candidate.
    await page.getByTestId('supplier-spec-file').setInputFiles({
      name: 'calendar.openapi.json',
      mimeType: 'application/json',
      buffer: Buffer.from(validCalendarSpec(), 'utf-8'),
    });
    await expect(page.getByTestId('supplier-candidate')).toContainText(/calendar/i);
  });
});

test.describe('supplier · area A spec ingest · err', () => {
  // Covers the spec-driven ingest UI/backend (docs/design/supplier.md §8 area A).
  // Already implemented, green.
  ...
  test('malformed spec → rejected + human-readable parse error (no candidate shown)', async ({ adminPage: page }) => {
    await openSpecPaste(page);
    await page.getByTestId('supplier-spec-input').fill('{ "openapi": "3.0.0", ');
    await page.getByTestId('supplier-spec-submit').click();

    const err = page.getByTestId('supplier-spec-error');
    await expect(err).toBeVisible();
    await expect(err).toContainText(/parse|invalid|无法解析|格式/i);
    await expect(page.getByTestId('supplier-candidate')).toHaveCount(0);
  });
  ...
  test('non-3.0 (Swagger 2.0) → rejected + version hint', async ({ adminPage: page }) => {
    await openSpecPaste(page);
    await page.getByTestId('supplier-spec-input').fill(swagger20Spec());
    await page.getByTestId('supplier-spec-submit').click();
  ...
  test('valid 3.1 spec pasted → parsed → shows supplier candidate (F-H-1)',
    async ({ adminPage: page }) => {
      await openSpecPaste(page);
      await page.getByTestId('supplier-spec-input').fill(openapi31Spec());
      await page.getByTestId('supplier-spec-submit').click();
  ...
  test('missing servers → rejected + points out missing servers', async ({ adminPage: page }) => {
    await openSpecPaste(page);
    await page.getByTestId('supplier-spec-input').fill(specMissingServers());
    await page.getByTestId('supplier-spec-submit').click();
  ...
  test('no operations (empty paths) → rejected + points out missing operations', async ({ adminPage: page }) => {
    await openSpecPaste(page);
    await page.getByTestId('supplier-spec-input').fill(specMissingOperations());
    await page.getByTestId('supplier-spec-submit').click();
  ...
  // supplier-spec-fetch-names-the-refusal; this test only covers the latter.
  test('spec URL fetch fails → human-readable fetch error', async ({ adminPage: page }) => {
    ...
    await page.getByTestId('supplier-spec-url-input')
...
// ──────────────────────────────────────────────────────────────────────────
test.describe('supplier · area A spec ingest corner · err', () => {
  // Covers the spec-driven ingest UI/backend (docs/design/supplier.md §8 area A).
  // Already implemented, green.
  ...
  test('oversized spec (over the size limit) → rejected + size-limit hint', async ({ adminPage: page }) => {
    await openSpecPaste(page);
    await page.getByTestId('supplier-spec-input').fill(oversizedSpec());
    await page.getByTestId('supplier-spec-submit').click();
  ...
  test('duplicate operationId → rejected/flagged (cannot bind uniquely)', async ({ adminPage: page }) => {
    await openSpecPaste(page);
    await page.getByTestId('supplier-spec-input').fill(duplicateOperationIdSpec());
    await page.getByTestId('supplier-spec-submit').click();
  ...
  test('operation missing operationId → flagged (binding has no anchor)', async ({ adminPage: page }) => {
    await openSpecPaste(page);
    await page.getByTestId('supplier-spec-input').fill(specMissingOperationId());
    await page.getByTestId('supplier-spec-submit').click();
  ...
  test('external/unresolved $ref → rejected + points out unresolvable reference', async ({ adminPage: page }) => {
    await openSpecPaste(page);
    await page.getByTestId('supplier-spec-input').fill(specExternalRef());
    await page.getByTestId('supplier-spec-submit').click();
...
test.describe('supplier · area A spec ingest corner · happy', () => {
  ...
  test('valid 3.0 YAML spec → parsed via the same path → candidate appears', async ({ adminPage: page }) => {
    await openSpecPaste(page);
    await page.getByTestId('supplier-spec-input').fill(validCalendarSpecYaml());
    await page.getByTestId('supplier-spec-submit').click();
  ...
  test('internal $ref (same document) → resolves → candidate appears', async ({ adminPage: page }) => {
    await openSpecPaste(page);
    await page.getByTestId('supplier-spec-input').fill(specInternalRef());
    await page.getByTestId('supplier-spec-submit').click();
...
// openSupplierAdd / openSpecPaste + sample spec strings).
// ──────────────────────────────────────────────────────────────────────────

// openSupplierAdd —— navigates into the suppliers area from the known entry point
