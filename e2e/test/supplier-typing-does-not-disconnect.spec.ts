// supplier-typing-does-not-disconnect.spec.ts —— F-C-46。
//
// **Typing a character into a credential field must not take down a supplier
// that's still in use.**
//
// Hit in prod: while driving mail-supplier check 4, my script typed into
// `from_address` just once and then broke — **CONNECT was never clicked**. The
// log showed two `POST /suppliers/smtp/credentials` calls, `connected_at`
...
// defect:
//   · `use-supplier-card.ts`'s `setField` — saves to the server on every keystroke;
...
import { openSupplierCard, expectConnected } from '@/fixtures/supplier-card';
...
// bearer-api — a non-dance built-in supplier: one token field, saving means
...
test.describe('supplier · typing is not committing (F-C-46)', () => {
  ...
  test('typing a new credential does NOT take the live supplier down', async ({ adminPage: page }) => {
    const card = await openSupplierCard(page, SUPPLIER_ID);
    await card.getByTestId('supplier-field-token').fill('the-working-token');
    await card.getByTestId('supplier-connect-button').click();
    await expectConnected(card);
    // Precondition: it's connected right now.
    expect((await status(page)).connected, 'precondition: the supplier is live').toBe(true);

    // Owner starts editing this field — just typing, nothing pressed yet.
    await card.getByTestId('supplier-field-token').fill('half-typed-new-tok');
    ...
    // second time today).
    await openSupplierCard(page, 'smtp');
    const back = await openSupplierCard(page, SUPPLIER_ID);
  ...
  test('pressing Connect after a credential change still re-verifies (D-5 holds)',
    async ({ adminPage: page }) => {
      ...
      await card.getByTestId('supplier-field-token').fill('a-different-token');
      await card.getByTestId('supplier-connect-button').click();
      await expectConnected(card);
...
async function status(page: Page): Promise<SupplierStatus> {
  const res = await page.request.get(`${BACKEND}/api/admin/suppliers/${SUPPLIER_ID}/status`);
  if (res.status() !== 200) throw new Error(`supplier status: ${res.status()}`);
