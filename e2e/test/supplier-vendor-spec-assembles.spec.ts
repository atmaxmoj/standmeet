// supplier-vendor-spec-assembles.spec.ts -- supplier-assembly checks 2 + 3,
...
// them into a supplier. The form that can submit is a different, differently
...
//   (2) after clicking assemble, `GET /api/admin/suppliers` **actually gains
...
// used" is distinguishable later when checking the supplier, not a
// coincidental collision.
const BASE_URL = 'https://api.vendor-supplied.test/v2';

test.use({ ownerCredentials: { email: OWNER.email, password: OWNER.password } });

test.describe('a real vendor spec (no servers, no auth) assembles into a supplier', () => {
  ...
  test('the missing base URL has a place to be supplied, and the candidate then appears',
    async ({ adminPage: page }) => {
      ...
      await page.getByTestId('supplier-spec-input').fill(vendorSpecNoServers());
      ...
      await page.getByTestId('supplier-spec-base-url').fill(BASE_URL);
      await page.getByTestId('supplier-spec-submit').click();

      // Assert the candidate **appears**, not that the error disappears: the
      // latter also holds when the request just dies.
      await expect(page.getByTestId('supplier-candidate')).toContainText(/vendor scheduling/i);
  ...
  test('the form that collected spec and scheme can actually assemble a supplier',
    async ({ adminPage: page }) => {
      const before = await supplierIDs(page);

      await openSupplierAdd(page);
      await page.getByTestId('supplier-spec-input').fill(vendorSpecWithServers());
      await page.getByTestId('supplier-spec-submit').click();
      ...
      await page.getByTestId('supplier-scheme-select').selectOption('manual:bearer');
      ...
      await expect(page.getByTestId('supplier-assemble-useless')).toBeVisible();
      ...
      // The evidence lives in the supplier list, not on the button.
      const created = await newSupplierID(page, before);
      expect(created, 'assembling must leave a supplier behind').not.toBe('');
    });
});
...
async function supplierIDs(page: Page): Promise<Set<string>> {
  const res = await page.request.get(`${BACKEND}/api/admin/suppliers`);
  if (res.status() !== 200) throw new Error(`list suppliers: ${res.status()}`);
  const rows = (await res.json() as { suppliers?: ConnRow[] }).suppliers ?? [];
  return new Set(rows.map((c) => c.id));
}

// newSupplierID -- polls until an openapi supplier appears that wasn't in
...
// built-in suppliers.
async function newSupplierID(page: Page, before: Set<string>): Promise<string> {
  let found = '';
