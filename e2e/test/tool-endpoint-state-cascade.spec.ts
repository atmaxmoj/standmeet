...
// block_state carried in a per-tool endpoint's response is always the **current**
...
// block_state (max_bookings is full → ErrHidden). The frontend's zustand syncs from
...
function findCap(caps: SessionBlock[] | undefined, id: string): SessionBlock | undefined {
  return caps?.find(c => c.id === id);
}

test.describe('tool endpoint · block_state cascade is cross-tool', () => {
  let seed: CodedSeed;
  ...
  test('before burn: corpus_search response shows calendar.book enabled + quota_remaining=1',
    async () => {
      ...
      const cal = findCap(body.block_state, 'calendar.book');
  ...
  test('burn calendar_book (max_bookings=1) → cap immediately gated out in same response',
    async () => {
      ...
      expect(
        findCap(body.block_state, 'calendar.book'),
  ...
  test('after burn: corpus_search response shows calendar.book gated out (cross-tool invariant)',
    async () => {
      ...
      expect(
        findCap(body.block_state, 'corpus.retrieval')?.enabled,
      ).toBe(true);
    });

  test('after burn: re-calling calendar_book → 404 + block_state still excludes it',
    async () => {
      const { status, body } = await callTool(
        seed.request, seed.visitor, 'calendar_book', {
          topic: 'Tries again',
