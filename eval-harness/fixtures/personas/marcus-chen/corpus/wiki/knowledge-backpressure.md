...
wiki://project/notification-pipeline), and so was the event bus migration (see
wiki://work/orbit/event-bus).
...
- **Blast-radius caps** — one bad producer shouldn't starve everyone; per-event
  ...
  wiki://project/notification-pipeline/fan-out).
- **Poison messages** — one un-processable item shouldn't wedge a worker forever;
  that's what dead-letter queues are for (see wiki://knowledge/retries-backoff).
