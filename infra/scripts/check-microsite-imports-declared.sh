#!/usr/bin/env sh
# check-microsite-imports-declared —— what the panel says you can import, you must really be able to import.
#
...
PANEL="app/src/lib/admin/microsite-imports.ts"
VENDOR="builder/vendor/@standmeet"

[ -f "$PANEL" ] || { echo "check-microsite-imports-declared: $PANEL is gone; the gate has no object"; exit 2; }
[ -d "$VENDOR" ] || { echo "check-microsite-imports-declared: $VENDOR is gone; the gate has no object"; exit 2; }
...
[ -n "$advertised" ] || {
  echo "check-microsite-imports-declared: $PANEL mentions no @standmeet/* at all."
...
for pkg in $advertised; do
  ...
  if [ ! -d "$VENDOR/$name" ]; then
    echo "check-microsite-imports-declared: the panel advertises $pkg, but builder didn't vendor it"
...
if [ "$probe_hits" != "@standmeet/definitely-not-vendored" ]; then
  echo "check-microsite-imports-declared: SELF-TEST FAILED —— the extractor didn't read the package name in the probe"
  rm -rf "$probe"
  exit 2
fi
if [ -d "$VENDOR/definitely-not-vendored" ]; then
  echo "check-microsite-imports-declared: SELF-TEST FAILED —— the probe package actually exists"
  rm -rf "$probe"
  exit 2
fi
rm -rf "$probe"
...
[ "$fail" -eq 0 ] || exit 1
echo "check-microsite-imports-declared: every @standmeet/* the panel advertises is in builder/vendor (self-test passed)."
